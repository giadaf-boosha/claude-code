...
> Ultimo aggiornamento: **17 settembre 2026, 07:00 CEST**.
> Versione CLI di riferimento: **v2.1.274** · Modello default **Sonnet 5** · Premium **Fable 5.1 / Opus 5** (Max plan; Opus 4.8 rimane disponibile via `/model`).
...
- **Claude Mods** (Early Access, 15 set): nuovo sistema di estensione "Function Hooks" — un mod e' un plugin il cui comportamento vive in un modulo hooks con un unico entry point `register(on, options)` che intercetta gli eventi del motore agentico, puo' modificare una tool call prima che parta o aggiungere componenti interattivi all'interfaccia, non solo osservare/bloccare come i hook `command` classici. API instabile, breaking change attesi prima della GA. Fonte: [@bcherny](https://x.com/bcherny/status/2099551291601248485) · [repo `mods/`](https://github.com/anthropics/claude-code/tree/main/mods). Doc: [docs/11-plugins-marketplace.md](./docs/11-plugins-marketplace.md), [docs/07-hooks.md](./docs/07-hooks.md), [docs/19-changelog.md](./docs/19-changelog.md).

Il resto delle release v2.1.270-274 (12-17 set: fix su permessi git read-only dopo sessioni lunghe, fast mode nelle sessioni Remote, `allowed_domains` per-comando in Bash/PowerShell/Monitor, auto-archiviazione sessioni VS Code, self-healing dei transcript corrotti su errori "unexpected tool_use_id", header di richiesta gateway per monitoring LLM) resta sotto la soglia editoriale. Nessun annuncio Anthropic blog dedicato su Claude Code nelle ultime 24 ore.
