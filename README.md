...
## What's new today (2026-09-17)

> _Aggiornamento automatico dalle 07:00 Europe/Rome. Vedi [archive](./docs/whats-new-archive.md) per i giorni precedenti._

- **CLI v2.1.274** (17 set): warning visibile quando la memoria e' critica, sessioni che si autoripristinano da transcript corrotti ("unexpected tool_use_id") invece di restare bloccate in loop, negoziazione protocollo **MCP 2026-07-28** estesa di default a Bedrock/Vertex/Foundry, `/code-review` passa a prompt inline piu' snelli invece di spawnare piu' subagent di review. Fonte: [GitHub Releases v2.1.274](https://github.com/anthropics/claude-code/releases/tag/v2.1.274). Vedi [docs/10-mcp.md § 10.2b](./docs/10-mcp.md#102b-spec-mcp-2026-07-28-core-stateless) e [docs/19-changelog.md](./docs/19-changelog.md).
- **CLI v2.1.273** (15 set): le sessioni **Remote Control** si possono ora biforcare in sessioni locali in background, notifica quando un server MCP si disconnette a meta' sessione, fix al context meter che raddoppiava il conteggio dei turni advisor-tool (faceva scattare l'auto-compact a meta' della finestra reale). Fonte: [GitHub Releases v2.1.273](https://github.com/anthropics/claude-code/releases/tag/v2.1.273). Vedi [docs/17-ide-surface.md § 17.6](./docs/17-ide-surface.md#remote-control) e [docs/19-changelog.md](./docs/19-changelog.md).
- **CLI v2.1.271** (14 set): **fast mode** arriva nelle sessioni Remote (cloud e self-hosted runner), nuovo `allowed_domains` per-comando su Bash/PowerShell/Monitor in auto mode con sandboxing, `omitClaudeMd` nel frontmatter agent per far girare subagent senza CLAUDE.md. Fonte: [GitHub Releases v2.1.271](https://github.com/anthropics/claude-code/releases/tag/v2.1.271). Vedi [docs/04-modalita-permessi.md § 4.4](./docs/04-modalita-permessi.md#sandbox), [docs/08-subagents.md § 8.2](./docs/08-subagents.md), [docs/05-fast-mode-1m-context.md](./docs/05-fast-mode-1m-context.md) e [docs/19-changelog.md](./docs/19-changelog.md).
- **Claude Cowork e chat si fondono in un unico Claude** (16 set): Anthropic elimina l'interfaccia Cowork separata e la integra nella chat principale, con Claude Docs/Slides in beta e Claude Design incluso nelle conversazioni; Claude Code resta un prodotto a se' ma ora supporta gli artifact. Rollout su Pro/Max (web/desktop/mobile) nelle prossime settimane, Team/Free a seguire. Fonte: [Anthropic blog](https://claude.com/blog/cowork-is-now-claude). Dove in repo: n/a (annuncio ecosistema, fuori scope CLI).
- **Blog engineering: come Anthropic ha scalato il test impact analysis della CI** (14 set): la crescita 25x dei job CI (80% del codice scritto da Claude, suite di test 10x piu' grande) ha imposto di riscrivere il servizio da singleton a sistema distribuito con journal e consumer separati; Claude Code ha generato lo sharding iniziale e poi eseguito da solo il tuning finale (journal sizing, config worker) — case study di compound engineering interno. Fonte: [Anthropic blog](https://claude.com/blog/agentic-coding-is-straining-ci-heres-how-we-scaled-test-impact-analysis-at-anthropic). Vedi [docs/22-compound-engineering.md](./docs/22-compound-engineering.md).

Il resto delle release v2.1.270/272 (12 e 15 set: fix della regressione sui permessi Bash su comandi git read-only introdotta in v2.1.269, hardening su skill sync/OAuth/permessi) resta sotto la soglia editoriale. L'aumento permanente **+25%** dei limiti settimanali (gia' coperto nell'edizione del 12 set) e' entrato in vigore come previsto il 14 settembre. Nota: la routine non ha girato dal 13 al 16 settembre; questa edizione copre l'intera finestra 12→17 settembre per evitare buchi nel changelog.
