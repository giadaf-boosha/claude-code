...
- **CLI v2.1.269** (11 set): introduce `claude plugin eval` per valutare un plugin/skill con risultati riproducibili (report JSON+HTML), il comando `/output-style` per elencare e cambiare stile di output, e il nuovo `/focus` per una vista essenziale del turno (solo prompt, riassunto one-line, risposta). Aggiunge anche il diff del tool Bash sui file modificati e tag repository per OpenTelemetry. Fonte: [GitHub Releases v2.1.269](https://github.com/anthropics/claude-code/releases/tag/v2.1.269). Vedi [docs/09-skills.md](./docs/09-skills.md), [docs/03-slash-commands.md](./docs/03-slash-commands.md), [docs/11-plugins-marketplace.md § 11.1](./docs/11-plugins-marketplace.md), [docs/19-changelog.md § Fase 7](./docs/19-changelog.md).
- **`claude plugin eval` sotto la lente**: la eval suite confronta l'output di un plugin/skill contro una baseline "senza plugin" con 6 tipi di grader, pensata come CI gate per skill che smettono di attivarsi dopo un edit o un cambio modello — risponde a "la skill si attiva davvero?", "sopravvive a un edit?", "batte il modello nudo?". Fonte: [MarkTechPost](https://www.marktechpost.com/2026/09/11/anthropic-adds-plugin-evals-to-claude-code-6-grader-types-a-no-plugin-baseline-and-a-ci-gate-for-skills/). Vedi [docs/11-plugins-marketplace.md § 11.1](./docs/11-plugins-marketplace.md), [docs/09-skills.md](./docs/09-skills.md).

Il resto delle release v2.1.263/265/266/267/268 (6-10 set: bug fix di affidabilita', telemetria gateway e `--plugin-dir` per cartelle di plugin, fix critico della regressione gateway introdotta in 2.1.265, `maxEffortLevel` e `--system-prompt-snapshot off`, pricing gateway allineato a `/cost`, timeout di 300s per `WebFetch`) resta sotto la soglia editoriale. Nessun annuncio Anthropic blog dedicato su Claude Code nelle ultime 24 ore; nessun post datato oggi sugli account X monitorati oltre a quanto gia' coperto dal changelog.
