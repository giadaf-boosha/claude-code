...
> Ultimo aggiornamento: **19 settembre 2026, 07:00 CEST**.
...
## What's new today (2026-09-19)

> _Aggiornamento automatico dalle 07:00 Europe/Rome. Vedi [archive](./docs/whats-new-archive.md) per i giorni precedenti._

- **AGENTS.md come alternativa a CLAUDE.md** (v2.1.277, 18 set): quando un progetto non ha un CLAUDE.md, Claude Code legge ora AGENTS.md al suo posto — preferenza configurabile in `/config` sotto "Project instructions" (non ancora su Bedrock, Vertex o Foundry). Stessa release: `CLAUDE_GATEWAY_PROXY_IS_EGRESS_BOUNDARY=1` per gateway con unico egress via forward proxy, header statici opzionali sugli upstream gateway, oltre 70 fix di affidabilita' (hang di `claude -p`/Agent SDK dopo errori interni, logout inatteso con build multiple sulla stessa macchina, avvio sessione piu' rapido per SDK/headless). Fonte: [GitHub Releases v2.1.277](https://github.com/anthropics/claude-code/releases/tag/v2.1.277). Doc: [docs/06-claude-md-memory.md](./docs/06-claude-md-memory.md), [docs/19-changelog.md](./docs/19-changelog.md).
- **Auto mode: classifier lato server di default per API/Enterprise e provider cloud** (v2.1.278, 19 set): per utenti Claude API ed Enterprise, e su Bedrock/Vertex/Foundry/gateway, l'auto mode passa di default al classifier lato server, che non fattura piu' l'overhead di classificazione; opt-out su Bedrock/Vertex/Foundry/gateway con `CLAUDE_CODE_AUTO_MODE_SERVER=0` (warning se si ricade sul fallback fatturato), nuova riga "Auto mode server" in `/status`. Fonte: [GitHub Releases v2.1.278](https://github.com/anthropics/claude-code/releases/tag/v2.1.278). Doc: [docs/04-modalita-permessi.md](./docs/04-modalita-permessi.md), [docs/19-changelog.md](./docs/19-changelog.md).

Il resto della release v2.1.276 (fix critico: richieste che fallivano con errore `400 ... Input tag 'advisor_20260301'` quando `ANTHROPIC_BASE_URL` punta a un proxy/gateway, regressione della v2.1.275) resta sotto la soglia editoriale. Nessun nuovo annuncio Anthropic blog dedicato su Claude Code nelle ultime 24 ore (gli ultimi post su `claude.com/blog`, del 17 set, sono gia' coperti nell'aggiornamento di ieri); nessun post nuovo rilevato negli account team tracciati (@bcherny, @_catwu, @noahzweben, @trq212, @claudeai, @ClaudeDevs, @alistaiir, @ClaudeCodeLog) nella finestra 18-19 settembre — verifica via WebSearch su x.com, non esaustiva. `anthropic.com/news` non raggiungibile dal proxy di rete durante la ricerca odierna.
