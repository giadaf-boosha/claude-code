...
## What's new today (2026-09-18)

> _Aggiornamento automatico dalle 07:00 Europe/Rome. Vedi [archive](./docs/whats-new-archive.md) per i giorni precedenti._

- **v2.1.276** (18 set): fix critico — corregge l'errore `400 ... Input tag 'advisor_20260301'` che bloccava ogni richiesta quando `ANTHROPIC_BASE_URL` punta a un proxy o gateway (regressione introdotta da v2.1.275). Fonte: [GitHub Releases v2.1.276](https://github.com/anthropics/claude-code/releases/tag/v2.1.276). Doc: [docs/18-settings-auth.md](./docs/18-settings-auth.md), [docs/19-changelog.md](./docs/19-changelog.md).
- **v2.1.275** (17 set): release corposa — nuovo tasto "send-now" (`ctrl+enter` o `ctrl+x ctrl+s`) che interrompe il turno corrente e spedisce subito tutti i messaggi in coda, sync automatico di skill e plugin abilitati sull'account claude.ai verso le sessioni terminal (opt-out con `syncClaudeAiSkills`/`syncClaudeAiPlugins`), nuovo `/plugin install <plugin> --marketplace <source>`. Fonte: [GitHub Releases v2.1.275](https://github.com/anthropics/claude-code/releases/tag/v2.1.275). Doc: [docs/03-slash-commands.md](./docs/03-slash-commands.md), [docs/09-skills.md](./docs/09-skills.md), [docs/19-changelog.md](./docs/19-changelog.md).
- **Projects ridisegnato**: passa da modello a cartelle a modello conversazionale — Claude scompone la richiesta, delega a thread paralleli su sessioni cloud, coordina il lavoro e assembla il risultato finale, con memoria condivisa che cresce nel tempo e una libreria di file/artifact. Beta per Pro/Max su sessioni cloud di Claude Code, in espansione verso Team ed Enterprise. Fonte: [Anthropic blog](https://claude.com/blog/projects-redesigned) · [@ClaudeDevs](https://x.com/ClaudeDevs/status/2100633571543367691). Doc: [docs/13-routines-cloud.md](./docs/13-routines-cloud.md), [docs/19-changelog.md](./docs/19-changelog.md).

Il resto delle release v2.1.273-274 (16-17 set: attributi telemetria OTel per `effort` e per le sorgenti dei managed settings, client MCP v2 di default su Bedrock/Vertex/Foundry, `CLAUDE_CODE_MCP_STARTUP_WAIT_MS`, avviso di memoria critica, self-healing di sessioni con transcript corrotto) resta sotto la soglia editoriale. Nessun altro annuncio Anthropic blog dedicato a Claude Code nelle ultime 24 ore oltre a "Projects redesigned"; nessun post nuovo rilevante dagli altri account team monitorati.
