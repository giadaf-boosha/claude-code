...
> Nessuna novita' significativa nelle ultime 24 ore. L'unica release (v2.1.247, 26 ago) resta sotto la soglia editoriale: tool `SendFeedback` per bozze di segnalazione problemi, comando `/claude-api cost-optimize`, copertura Admin API nella skill `/claude-api`, personalizzazione spinner tip via `spinnerTipsOverride` — nessun nuovo slash command, tool primitivo o feature di sistema paragonabile a routines/`/loop`/Monitor. Nessun annuncio Anthropic rilevante nelle ultime 24 ore. Prossimo aggiornamento domani 07:00.
