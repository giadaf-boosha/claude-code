...
- **Claude Code legge AGENTS.md se manca CLAUDE.md** (v2.1.277, 18 set): niente piu' workaround con `@AGENTS.md` — quando una cartella non ha un CLAUDE.md, Claude Code usa direttamente AGENTS.md (lo standard cross-tool ormai adottato in 60mila+ repository), comportamento configurabile in `/config` → "Project instructions"; se esiste gia' un CLAUDE.md quello resta prioritario. Non ancora disponibile su Bedrock, Vertex e Foundry. Fonte: [changelog v2.1.277](https://code.claude.com/docs/en/changelog) · [@trq212](https://x.com/trq212/status/2101009392611278961). Doc: [docs/06-claude-md-memory.md § 6.3](./docs/06-claude-md-memory.md), [docs/19-changelog.md](./docs/19-changelog.md).
- **Auto mode: classifier lato server senza sovrapprezzo** (v2.1.278, 19 set): su API diretta, Enterprise, Bedrock, Vertex, Foundry e sessioni Claude apps gateway, i controlli di sicurezza di auto mode girano ora di default lato server invece che come richieste extra al modello, e non vengono piu' fatturati come overhead classifier; nuova riga "Auto mode server" in `/status`, opt-out via `CLAUDE_CODE_AUTO_MODE_SERVER=0`. Fonte: [changelog v2.1.278](https://code.claude.com/docs/en/changelog) · [GitHub Releases v2.1.278](https://github.com/anthropics/claude-code/releases/tag/v2.1.278). Doc: [docs/04-modalita-permessi.md § 4.3](./docs/04-modalita-permessi.md), [docs/19-changelog.md](./docs/19-changelog.md).

Il resto delle release v2.1.276-277 (18-19 set: fix regressione header `advisor_20260301` su proxy/gateway introdotta da v2.1.275, tasto "invia subito" `ctrl+enter`/`ctrl+x ctrl+s` per interrompere il turno e inviare i messaggi in coda, mouse support nel pannello `/config` fullscreen, 95+ fix minori su VS Code, Claude Tag e Code Review) resta sotto la soglia editoriale. Nessun annuncio Anthropic blog dedicato su Claude Code nelle ultime 24 ore; nessun altro post rilevante dai profili X del team oltre all'annuncio AGENTS.md gia' citato.
