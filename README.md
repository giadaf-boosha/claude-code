...
- **Claude Security plugin** (beta pubblica, 22 lug): nuovo plugin ufficiale Anthropic per Claude Code — scansione multi-agente del codice che mappa l'architettura, costruisce un threat model, individua vulnerabilita' (injection, bypass di autenticazione, memory corruption, logic error) e fa verificare ogni finding da un agente indipendente prima di proporre patch revisionabili con `git apply`. Si installa con `/plugin install claude-security@claude-plugins-official` e si usa con il nuovo comando `/claude-security` (scan dell'intero repo, di un diff/PR/commit, o generazione patch). Fonte: [Claude Code Docs](https://code.claude.com/docs/en/claude-security) · [@claudeai](https://x.com/claudeai/status/2079990597973057691). Doc: [docs/11-plugins-marketplace.md](./docs/11-plugins-marketplace.md), [docs/03-slash-commands.md](./docs/03-slash-commands.md), [docs/19-changelog.md](./docs/19-changelog.md).
