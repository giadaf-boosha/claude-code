...
- **v2.1.265** (8 set): introduce un cap di **1 GB sui tool result salvati su disco** (il preview in conversazione segnala quando e' troncato) e risolve piu' bug di prompt-cache reuse per subagent foreground ripresi e teammate/agent-team che riprendono una sessione. Fonte: [GitHub Releases v2.1.265](https://github.com/anthropics/claude-code/releases/tag/v2.1.265). Vedi [docs/08-subagents.md](./docs/08-subagents.md), [docs/19-changelog.md](./docs/19-changelog.md).
- **`--plugin-dir` su cartella multipla** (v2.1.265, 8 set): il flag ora accetta una cartella che contiene piu' plugin — ogni sotto-cartella con un manifest si carica, e aggiunte/rimozioni a runtime vengono rilevate senza riavvio. Fonte: [GitHub Releases v2.1.265](https://github.com/anthropics/claude-code/releases/tag/v2.1.265). Vedi [docs/11-plugins-marketplace.md](./docs/11-plugins-marketplace.md), [docs/19-changelog.md](./docs/19-changelog.md).
- **VS Code: archiviazione automatica sessioni inattive** (v2.1.265, 8 set): nuovo setting che archivia le sessioni ferme oltre un periodo configurabile (default 14 giorni). Fonte: [GitHub Releases v2.1.265](https://github.com/anthropics/claude-code/releases/tag/v2.1.265). Vedi [docs/17-ide-surface.md](./docs/17-ide-surface.md), [docs/19-changelog.md](./docs/19-changelog.md).
- **`/workflows` mostra lo stato dei tool call** (v2.1.265, 8 set): ogni chiamata tool appare come in corso/fallita/completata, la task list del subagent e' visibile e Invio espande input/output della singola chiamata. Fonte: [GitHub Releases v2.1.265](https://github.com/anthropics/claude-code/releases/tag/v2.1.265). Vedi [docs/24-workflows.md](./docs/24-workflows.md), [docs/19-changelog.md](./docs/19-changelog.md).
- **v2.1.266** (9 set): risolve una regressione introdotta in 2.1.265 che rompeva tutte le richieste nelle configurazioni LLM-gateway/proxy con `CLAUDE_CODE_USE_GATEWAY` impostata insieme a API key, `apiKeyHelper` o auth header custom (forzava il sign-in al Cloud gateway anche quando non richiesto). Fonte: [GitHub Releases v2.1.266](https://github.com/anthropics/claude-code/releases/tag/v2.1.266). Vedi [docs/18-settings-auth.md](./docs/18-settings-auth.md), [docs/19-changelog.md](./docs/19-changelog.md).

Il resto della release v2.1.265 (oltre 50 fix minori su Remote Control, fallback MCP legacy HTTP+SSE, sandbox Windows, VS Code) e la v2.1.263 (6 set, solo bug fix e affidabilita') restano sotto la soglia editoriale. Nessun annuncio Anthropic blog dedicato su Claude Code nelle ultime 24 ore; i post X dei team monitorati non hanno prodotto novita' aggiuntive verificabili in questa finestra.
