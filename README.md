...
- **Setting `managedMcpServers`** (v2.1.259, 2 set): le organizzazioni possono distribuire server MCP HTTP/SSE a tutti gli utenti tramite settings gestiti (server-managed settings, `managed-settings.json`, MDM), senza piu' dover deployare il file `managed-mcp.json` macchina per macchina; stessa forma di `.mcp.json`, ma solo transport remoti (le entry con un `command` da eseguire vengono ignorate). Fonte: [GitHub Releases v2.1.259](https://github.com/anthropics/claude-code/releases/tag/v2.1.259) · [docs managed-mcp](https://code.claude.com/docs/en/managed-mcp). Doc: [docs/10-mcp.md](./docs/10-mcp.md), [docs/19-changelog.md](./docs/19-changelog.md).

Il resto della release v2.1.259 (2 set: `--permission-prompts none` per host headless, riconoscimento GitLab merge request `MR !N`, `--json` su `claude plugin validate`, fix di sessioni concorrenti che si sovrascrivevano `~/.claude.json`) resta sotto la soglia editoriale. Nessun annuncio Anthropic blog dedicato su Claude Code nelle ultime 24 ore; il digest settimanale ufficiale resta fermo alla Week 34 (17-21 ago, gia' coperta).
