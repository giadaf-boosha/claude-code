...
> Ultimo aggiornamento: **18 settembre 2026, 07:00 CEST**.
...
- **Supporto ad AGENTS.md** (v2.1.277, 18 set): Claude Code legge ora anche **AGENTS.md** — se una cartella non contiene un CLAUDE.md, usa il primo AGENTS.md che trova per le istruzioni di progetto, con toggle in `/config` → "Project instructions" (non ancora su Bedrock, Vertex, Foundry). Allinea Claude Code allo standard cross-tool AGENTS.md adottato da oltre 30 altri coding agent (Cursor, GitHub Copilot, OpenAI Codex, Google Jules, Amp). Fonte: [@trq212](https://x.com/trq212/status/2101009392611278961) · [GitHub Releases v2.1.277](https://github.com/anthropics/claude-code/releases/tag/v2.1.277). Doc: [docs/06-claude-md-memory.md](./docs/06-claude-md-memory.md), [docs/19-changelog.md](./docs/19-changelog.md).

Il resto della release v2.1.277 (18 set: headers statici sui gateway Claude apps, `CLAUDE_GATEWAY_PROXY_IS_EGRESS_BOUNDARY`, ampi fix su VS Code/Web/Code Review) e la v2.1.278 (19 set: classificatore auto mode lato server di default per API/Enterprise/gateway, senza sovraccosto di token, con riga dedicata in `/status`) restano sotto la soglia editoriale — cambi di billing/infrastruttura, non feature utente. Nessun annuncio Anthropic blog dedicato su Claude Code nelle ultime 24 ore; nessun post rilevante da @bcherny, @_catwu, @noahzweben, @claudeai, @ClaudeDevs oltre a quello citato.
