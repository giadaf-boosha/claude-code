...
- **CLI v2.1.251 — hook `PreModelSwitch`/`PostModelSwitch`**: nuovi eventi hook bloccano, confermano o annotano un cambio modello; i `SessionStart` hook di resume ricevono ora la staleness della sessione e il costo stimato di re-cache. Fonte: [CHANGELOG.md](https://github.com/anthropics/claude-code/blob/main/CHANGELOG.md). Vedi [docs/07 § Eventi](./docs/07-hooks.md) e [docs/19 § Fase 7](./docs/19-changelog.md).
- **Live streaming subagent in Remote Control**: i tool call e i risultati di un subagent in foreground si vedono ora in diretta sul client Remote Control (i subagent in background restano a solo stato). Fonte: [CHANGELOG.md](https://github.com/anthropics/claude-code/blob/main/CHANGELOG.md). Vedi [docs/17 § 17.6 Remote Control](./docs/17-ide-surface.md) e [docs/19](./docs/19-changelog.md).
- **Spend limit bar in `/usage`**: nuovo campo `rate_limits.spend_limit` in status line per chi lavora dietro un Claude apps gateway con limiti di spesa configurati. Fonte: [CHANGELOG.md](https://github.com/anthropics/claude-code/blob/main/CHANGELOG.md). Vedi [docs/18 § Campi principali](./docs/18-settings-auth.md) e [docs/19](./docs/19-changelog.md).
- **Metriche prompt-cache per sessione in `/cost`**: hit ratio, cache miss, token ri-cachati, warm/cold, disponibili anche via status line (`prompt_cache`). Fonte: [CHANGELOG.md](https://github.com/anthropics/claude-code/blob/main/CHANGELOG.md). Vedi [docs/18](./docs/18-settings-auth.md) e [docs/19](./docs/19-changelog.md).
- **Fix sicurezza multipli**: Read/Write/Edit non seguono piu' un symlink scambiato dopo il controllo permessi, i comandi plugin dichiarati in marketplace non possono piu' puntare fuori dalla directory plugin (path traversal), Grep/Glob applicano ora le regole deny anche su path raggiunti via symlink. Fonte: [CHANGELOG.md](https://github.com/anthropics/claude-code/blob/main/CHANGELOG.md). Vedi [docs/19 § Fase 7](./docs/19-changelog.md).
- **Hardening sandbox**: i settings gestiti che terminano il TLS della sandbox, instradano il traffico su un proxy custom, iniettano credenziali o indeboliscono l'isolamento richiedono ora approvazione esplicita prima di applicarsi. Fonte: [CHANGELOG.md](https://github.com/anthropics/claude-code/blob/main/CHANGELOG.md). Vedi [docs/18 § Sandbox](./docs/18-settings-auth.md) e [docs/19](./docs/19-changelog.md).
- **`CLAUDE_CODE_SUBAGENT_MODEL` diventa default, non override**: il `model:` di un agent definition o un modello passato esplicitamente allo spawn ora prevalgono sulla env var. Fonte: [CHANGELOG.md](https://github.com/anthropics/claude-code/blob/main/CHANGELOG.md). Vedi [docs/08 § Subagent](./docs/08-subagents.md) e [docs/19](./docs/19-changelog.md).
- **Modello default Enterprise seat-based → Opus 5**: allineato agli altri piani premium (Max, Team Premium). Fonte: [CHANGELOG.md](https://github.com/anthropics/claude-code/blob/main/CHANGELOG.md). Vedi [docs/01 § Snapshot](./docs/01-snapshot.md) e [docs/19](./docs/19-changelog.md).
- **`/effort` salva il default per modello**: cambiando modello, l'effort level scelto resta specifico per quel modello invece di essere condiviso tra tutti. Fonte: [CHANGELOG.md](https://github.com/anthropics/claude-code/blob/main/CHANGELOG.md). Vedi [docs/05 § 5.3 Effort](./docs/05-fast-mode-1m-context.md) e [docs/19](./docs/19-changelog.md).
- **Fix affidabilita'**: risolto il blocco "text content blocks must be non-empty" dopo un turno con solo thinking; Opus 5 con effort xhigh/max e thinking disattivato non fallisce piu' (l'effort scende automaticamente a `high`). Fonte: [CHANGELOG.md](https://github.com/anthropics/claude-code/blob/main/CHANGELOG.md). Vedi [docs/19](./docs/19-changelog.md) e [docs/05](./docs/05-fast-mode-1m-context.md).
