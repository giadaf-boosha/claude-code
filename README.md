...
> Nessuna novita' significativa nelle ultime 24 ore. Le release piu' recenti (v2.1.263, v2.1.265, v2.1.266, v2.1.267, 6-9 set) restano sotto la soglia editoriale: setting `maxEffortLevel` per limitare l'effort su tutti i provider, `--system-prompt-snapshot off`, `--plugin-dir` esteso a cartelle di plugin, telemetria `user.email`/`user.groups` per Desktop/Cowork, cap 1 GB sui risultati dei tool su disco, fix di una regressione sul Cloud gateway e vari fix di affidabilita'. Nessun annuncio Anthropic blog dedicato su Claude Code nelle ultime 24 ore. Prossimo aggiornamento domani 07:00.
