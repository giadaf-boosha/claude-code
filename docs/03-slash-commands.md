...
| `/branch [name]` | built-in | Branch della conversazione: entri in una copia per esplorare per conto tuo |
| `/fork` | built-in | Copia la conversazione in una nuova **sessione background** (riga propria in `claude agents`) mentre continui a lavorare in primo piano — prima dello split di v2.1.212 era alias di `/branch` e lanciava un subagent in-sessione (comportamento ora su `/subtask`) |
...
| `/doctor` (alias `/checkup`) | built-in | Diagnostica install + `f` per fix automatico. Da v2.1.205 e' un setup checkup completo: propone skill/MCP/plugin inutilizzati da rimuovere (in base al costo di context), deduplica `CLAUDE.md` locali contro le versioni committate e segnala hook lenti — riporta prima i risultati, chiede conferma prima di modificare |
...
| `/subtask` | built-in | Lancia un subagent nella sessione corrente (comportamento in-session che prima apparteneva a `/fork`, ora spostato qui da v2.1.212 — `/fork` spawna invece una sessione background) |
