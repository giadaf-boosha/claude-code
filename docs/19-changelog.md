...
Cronologia completa di Claude Code dalla research preview (24 febbraio 2025, v0.2.0) all'ultima versione (15 settembre 2026, v2.1.272). 7 fasi storiche + tabella versione per versione + post-mortem aprile 2026.
...
| 12 set 2026 | v2.1.270 | Fix regressione: comandi git read-only in Bash tornavano a chiedere permesso dopo una sessione lunga (introdotta in 2.1.269). |
| 14 set 2026 | **v2.1.271** | **`allowed_domains` per-comando in auto mode** per Bash/PowerShell/Monitor — vedi [4.3](./04-modalita-permessi.md#sandbox-allowed_domains-per-comando-in-auto-mode-v21271). **Subagent hand-back mechanism**: i subagent riportano l'esito al chiamante tramite una call dedicata rivista dal safety classifier, non piu' solo review dell'ultimo messaggio — vedi [8.6d](./08-subagents.md#86d-subagent-hand-back-mechanism-da-v21271). Fast mode esteso alle sessioni Remote (cloud e self-hosted runner). Mouse support in `/config` a schermo intero. `omitClaudeMd` nel frontmatter agent. Markdown pubblicato come artifact renderizzato come pagina stilizzata. Watch limit artifact portato a 10 sessioni concorrenti. Release enorme: decine di fix su org policy, MCP OAuth, Bash permission, terminal rendering, VS Code, web e Claude Tag. |
| 15 set 2026 | v2.1.272 | Bug fix e miglioramenti di affidabilita'. |

<sub>Aggiornato 2026-09-15 via daily what's new. Fonte: [GitHub Releases v2.1.270](https://github.com/anthropics/claude-code/releases/tag/v2.1.270) · [v2.1.271](https://github.com/anthropics/claude-code/releases/tag/v2.1.271) · [v2.1.272](https://github.com/anthropics/claude-code/releases/tag/v2.1.272) · [code.claude.com/docs/en/changelog](https://code.claude.com/docs/en/changelog). Due voci editoriali, entrambe da v2.1.271: `allowed_domains` per-comando in auto mode (feature di sistema, sandbox) e il subagent hand-back mechanism (feature di sistema, authority). Il resto della release (fast mode su Remote, mouse support in `/config`, `omitClaudeMd`, Markdown artifact stilizzati, watch limit a 10) e la v2.1.272 (solo bug fix) restano sotto la soglia editoriale. Nessun annuncio Anthropic blog dedicato nelle ultime 24 ore; digest ufficiale settimanale ancora fermo alla Week 34 (17-21 ago). La proposta di aumento limiti settimanali del 14 set (gia' coperta il 2026-09-12) e' entrata in vigore come da annuncio.</sub>
