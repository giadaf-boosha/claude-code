...
Cronologia completa di Claude Code dalla research preview (24 febbraio 2025, v0.2.0) all'ultima versione (3 settembre 2026, v2.1.260). 7 fasi storiche + tabella versione per versione + post-mortem aprile 2026.
...
| 2 set 2026 | **v2.1.259** | **`managedMcpServers`**: le organizzazioni possono fornire server MCP HTTP/SSE a tutti gli utenti senza configurazione locale — vedi [10.4](./10-mcp.md#104-scope-settings). **`--permission-prompts none`**: nega automaticamente i permission prompt su host headless unattended invece di bloccare la sessione — vedi [16.1](./16-headless-agent-sdk.md#161-cli-headless--p). Riconoscimento comandi `glab mr` con badge `MR !N` nei tool summary. `--json` per `claude plugin validate`. Fix: sessioni concorrenti che si sovrascrivevano a vicenda le config in `~/.claude.json`, invalidazione prompt cache al refresh OAuth, auto mode che girava su modelli non supportati, errori di connessione MCP in sessioni cloud. |
| 3 set 2026 | **v2.1.260** | **Pannello diff** in modalita' fullscreen (`/diff`) con le modifiche non committate aggiornate live — vedi [3.1](./03-slash-commands.md#31-tabella-completa). **Diagnostica prompt cache** in `/cost` e nel campo `prompt_cache` della status line, con le cause probabili di un cache miss. Forma testuale di `/advisor` per Desktop e sessioni headless. Fix estesi: regole permessi file con parentesi scartate come invalide nel sandbox Bash, auto-approvazione di comandi zsh con command substitution nascosta in variabili, model discovery Bedrock che falliva dietro CA aziendali, `/rewind` con file-read tracking non aggiornato. |

<sub>Aggiornato 2026-09-04 via daily what's new. Fonte: [GitHub Releases v2.1.259](https://github.com/anthropics/claude-code/releases/tag/v2.1.259) · [v2.1.260](https://github.com/anthropics/claude-code/releases/tag/v2.1.260) · [@bcherny](https://x.com/bcherny/status/2095590515765060076) (anteprima Function Hooks). Quattro voci editoriali: diff panel, diagnostica prompt cache, `managedMcpServers`, `--permission-prompts none` (tutte feature/setting di sistema). Il resto (badge GitLab MR, fix di v2.1.259/260) resta sotto soglia. Nessun annuncio Anthropic blog dedicato nelle ultime 24 ore.</sub>
