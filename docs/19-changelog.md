...
| 26 giu 2026 | v2.1.195 | `CLAUDE_CODE_DISABLE_MOUSE_CLICKS` per fullscreen senza click/drag/hover (mantiene scroll); hook matchers con trattini ora exact-match (usa `mcp__brave-search__.*` per wildcard); fix voice dictation macOS su cambio device e per lingue senza spazi (JA/ZH/TH); fix plugin enable/disable quando name differisce da marketplace; fix background agent daemon su socket failure; fix crashed task con schermata bianca. |
| 29 giu 2026 | **v2.1.196** | **Organization default models** configurabili dall'admin in org console (visualizzati come "Org default"/"Role default" in `/model`); **session names leggibili** e cliccabili all'avvio; **allegati cliccabili** in chat (Cmd/Ctrl-click apre in Finder/Explorer); security: `claude mcp list`/`get` non avvia server da `.mcp.json` non fidati; `/code-review` -25% token usage; stream watchdog default per tutti i provider; Remote Control disabilitato per host non-Anthropic; fix PowerShell `git diff`/`git grep` exit code 1; fix `claude agents` panel (focus, tipi, status, apertura con `←`); fix session recovery mid-turn + `/cd` da `/resume`. |
