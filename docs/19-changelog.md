...
| 18 set 2026 | v2.1.276 | Fix critico: regressione 2.1.275 che faceva fallire ogni richiesta con errore `400 … Input tag 'advisor_20260301'` quando `ANTHROPIC_BASE_URL` punta a un proxy o gateway. |
| 18 set 2026 | **v2.1.277** | **Supporto ad AGENTS.md**: se una cartella non contiene un CLAUDE.md, Claude Code legge ora il primo AGENTS.md che trova per le istruzioni di progetto — toggle in `/config` → "Project instructions", non ancora su Bedrock/Vertex/Foundry — vedi [6.3](./06-claude-md-memory.md#63-imports). Allinea Claude Code allo standard cross-tool adottato da 30+ coding agent (Cursor, GitHub Copilot, Codex, Jules). `CLAUDE_GATEWAY_PROXY_IS_EGRESS_BOUNDARY=1` per gateway Claude apps con unico egress via forward proxy; header statici opzionali (`headers:`) sugli upstream gateway. Fix: blocchi di testo vuoti che facevano fallire ogni richiesta della conversazione, logout inatteso quando build CLI diverse girano sulla stessa macchina, avvio bloccato con `customApiKeyResponses` malformato. Ampi fix su VS Code, Web, Claude Tag, Code Review. |
| 19 set 2026 | v2.1.278 | Auto mode per utenti API/Enterprise e su Bedrock/Vertex/Foundry/gateway passa di default al classificatore lato server, senza sovraccosto di token (`CLAUDE_CODE_AUTO_MODE_SERVER=0` per l'opt-out, con avviso se si ricade sul billing a consumo). Nuova riga "Auto mode server" in `/status`. |

<sub>Aggiornato 2026-09-19 via daily what's new. Fonte: [@trq212](https://x.com/trq212/status/2101009392611278961) · [GitHub Releases v2.1.277](https://github.com/anthropics/claude-code/releases/tag/v2.1.277) · [v2.1.278](https://github.com/anthropics/claude-code/releases/tag/v2.1.278) · [v2.1.276](https://github.com/anthropics/claude-code/releases/tag/v2.1.276) · [code.claude.com/docs/en/changelog](https://code.claude.com/docs/en/changelog). Unica voce editoriale: supporto AGENTS.md (v2.1.277, feature di sistema — cross-tool standard). Il resto di v2.1.277 (header gateway, fix vari) e tutta la v2.1.278 (classificatore auto mode lato server) restano sotto la soglia editoriale: cambi di billing/infrastruttura, non feature utente. Nessun annuncio Anthropic blog dedicato su Claude Code nelle ultime 24 ore.</sub>
