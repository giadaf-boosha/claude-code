...
| 26 giu 2026 | **v2.1.195** | **Fix hook matcher hyphenated ids**: identificatori con trattini (es. `code-reviewer`, `mcp__brave-search`) usano ora exact-match invece di substring-match — bug critico per chi usava nomi composti. **`CLAUDE_CODE_DISABLE_MOUSE_CLICKS`**: nuova env var disabilita click/drag/hover mouse in fullscreen mantenendo scroll con wheel (utile per tmux/screen). **Fix voice dictation macOS**: non cattura piu' silenzio in sessioni lunghe dopo cambio input device. **Fix voice auto-submit CJK/Thai**: auto-submit ora funziona per lingue senza spazi (giapponese, cinese, thai). **Fix plugin consent**: plugin abilitati solo da `.claude/settings.json` non richiedono piu' consenso ad ogni caricamento. **Fix `/plugin` Enable/Disable**: funziona quando `name` in `plugin.json` differisce dal nome marketplace. **Fix background jobs**: jobs non scompaiono piu' da `claude agents` con versioni nuove, daemon sempre raggiungibili, niente blank screen su task in crash. Remote session startup mostra checklist di provisioning. Voice mode Linux: diagnostica "no microfono" vs "SoX non installato". `claude agents` completed list riempie spazio verticale disponibile. |

<sub>Aggiornato 2026-06-27 via daily what's new. Fonte: [GitHub Releases v2.1.195](https://github.com/anthropics/claude-code/releases/tag/v2.1.195).</sub>
