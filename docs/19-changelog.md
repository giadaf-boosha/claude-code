...
| 8 set 2026 | **v2.1.265** | Cap di **1 GB sui tool result salvati su disco** (preview segnala il troncamento). **`--plugin-dir` su cartella multipla**: ogni sotto-cartella con manifest si carica, aggiunte/rimozioni a runtime rilevate senza riavvio — vedi [11](./11-plugins-marketplace.md). `/workflows` mostra lo stato di ogni tool call (in corso/fallita/completata) e la task list del subagent, espandibile — vedi [24](./24-workflows.md). VS Code: archiviazione automatica delle sessioni inattive (default 14 giorni) — vedi [17](./17-ide-surface.md). Fix estesi su prompt-cache reuse per subagent foreground ripresi e agent-team, fallback MCP legacy HTTP+SSE, sandbox Windows (AppContainer), Remote Control e plugin. |
| 9 set 2026 | v2.1.266 | Fix di una regressione di 2.1.265 che rompeva tutte le richieste nelle configurazioni LLM-gateway/proxy con `CLAUDE_CODE_USE_GATEWAY` impostata insieme a API key, `apiKeyHelper` o auth header custom — vedi [18](./18-settings-auth.md). |

<sub>Aggiornato 2026-09-09 via daily what's new. Fonte: [GitHub Releases v2.1.263](https://github.com/anthropics/claude-code/releases/tag/v2.1.263) · [v2.1.265](https://github.com/anthropics/claude-code/releases/tag/v2.1.265) · [v2.1.266](https://github.com/anthropics/claude-code/releases/tag/v2.1.266) · [CHANGELOG.md](https://github.com/anthropics/claude-code/blob/main/CHANGELOG.md). Quattro voci editoriali: cap 1GB tool result + fix prompt-cache, `--plugin-dir` multi-cartella, `/workflows` stato tool call, archiviazione automatica sessioni VS Code, fix regressione gateway/proxy in v2.1.266. Il resto di v2.1.265 (oltre 50 fix minori) e v2.1.263 (solo bug fix) resta sotto soglia. Nessun annuncio Anthropic blog dedicato su Claude Code nelle ultime 24 ore. Nota: fetch di anthropic.com e dev.classmethod.jp bloccati dal proxy di rete, ricerca completata via WebSearch/GitHub changelog raw/GitHub releases sulle altre fonti; nessun post X aggiuntivo verificabile nella finestra 24h oltre a contenuti gia' coperti (Fable 5.1, limiti settimanali) o non pertinenti a Claude Code CLI.</sub>
