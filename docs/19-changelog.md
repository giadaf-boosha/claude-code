...
| 20 lug 2026 | v2.1.216 | Nuova setting `sandbox.filesystem.disabled` per disattivare l'isolamento filesystem mantenendo il controllo di rete. Fix: rallentamento quadratico di normalizzazione messaggi nelle sessioni lunghe, sessioni background resumed che perdevano prompt e restrizioni tool dell'agent originale, subagent worktree-isolated che reindirizzavano git nella checkout condivisa (`git -C`, `GIT_DIR`), sessioni cloud che perdevano il messaggio in-flight al restart del container. |
| 21 lug 2026 | **v2.1.217** | **Cap subagent concorrenti**: limite default di 20 subagent in esecuzione simultanea per messaggio (`CLAUDE_CODE_MAX_CONCURRENT_SUBAGENTS`), i subagent non spawnano piu' automaticamente subagent annidati (`CLAUDE_CODE_MAX_SUBAGENT_SPAWN_DEPTH` per abilitare nesting piu' profondo), e `--max-budget-usd` ora ferma anche i subagent background gia' in esecuzione quando il tetto e' raggiunto. Autocomplete emoji nel prompt (`:heart:`). Fix: memory leak su output MCP troncati, auto-update Windows che poteva cancellare `claude.exe`, impostazioni mTLS/proxy aziendali ignorate su Desktop, isolamento sessioni background su working directory symlink-ate. |
| 22 lug 2026 | **v2.1.218** | **`/code-review` gira in background**: la skill non riempie piu' la conversazione principale; per estensione, tutte le skill con `context: fork` girano ora in background di default (opt-out per singola skill con `background: false`). `/deep-research` non si auto-invoca piu' — solo su richiesta esplicita. Screen-reader announcement sulle cancellazioni di testo in `--ax-screen-reader`. Fix: path Windows con prefisso `\u` corrotti in caratteri CJK, freccia sinistra che scartava la conversazione senza conferma, `/ultrareview` che falliva su argomenti descrittivi, `/code-review ultra` che girava in locale invece che in cloud nelle sessioni non interattive. |
