...
Cronologia completa di Claude Code dalla research preview (24 febbraio 2025, v0.2.0) all'ultima versione (14 agosto 2026, v2.1.233). 7 fasi storiche + tabella versione per versione + post-mortem aprile 2026.
...
| 14 ago 2026 | **v2.1.233** | **Merge request GitLab** in `--worktree` e nella vista `claude agents` (mostrate come `!N`) — vedi [02 § Worktree](./02-cli-installazione.md#worktree--tmux). **Tool di task-tracking (Todo) deprecati di default** su Opus 4.8, Sonnet 5 e modelli piu' recenti (si ripristinano con `CLAUDE_CODE_ENABLE_TODO_TOOLS=1`). Nuova env `CLAUDE_CODE_TOOL_MEMORY_LIMIT` per limiti cgroup Linux sul tool Bash, `CLAUDE_CODE_WEBFETCH_CACHE_TTL_MS` per la cache di `WebFetch`, setting opt-in `forward_user_identity` per attribuzione spesa per-utente sull'apps gateway. Fix: loop di conferme ripetute in auto mode su Windows per `cd` e redirect ordinari — vedi [4.3](./04-modalita-permessi.md#43-auto-mode-da-v2183-default-promaxteam-da-14-ago-2026); sessioni cloud marcate erroneamente come perse durante i permission prompt; CPU al 100% su sessioni Linux idle con sandboxing; alias skill bundled che riportavano "Unknown command" in modalita' `-p`. Fix sicurezza: path Windows NT `\??\` che bypassavano la validazione UNC. |

<sub>Aggiornato 2026-08-16 via daily what's new. Fonte: [GitHub Releases v2.1.233](https://github.com/anthropics/claude-code/releases/tag/v2.1.233).</sub>
