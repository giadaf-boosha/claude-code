...
| 22 lug 2026 | — | **Claude Security plugin** (beta pubblica): scansione multi-agente del codice — un team di agenti mappa l'architettura, costruisce un threat model, individua vulnerabilita' (injection, bypass di autenticazione, memory corruption, logic error) e fa verificare ogni finding da un agente indipendente prima di scrivere il report. Aggiunge il comando `/claude-security` (scan repo/diff/PR/commit, poi "Suggest patches"); le patch finiscono in `patches/*.patch` e vanno applicate a mano con `git apply`, mai in automatico. Si installa con `/plugin install claude-security@claude-plugins-official`. Fonte: [Claude Code Docs](https://code.claude.com/docs/en/claude-security) · [@claudeai](https://x.com/claudeai/status/2079990597973057691). |
| 22 lug 2026 | v2.1.218 | `/code-review` gira come subagent in background invece di occupare la conversazione. Annunci screen reader per cancellazioni di testo in modalita' `--ax-screen-reader`. Fix path Windows con segmenti `\u` corrotti in caratteri CJK, incolla multi-riga collassato nei terminali che codificano newline come Ctrl+J, vari fix su `/ultrareview` e sessioni background. |
...
<sub>Aggiornato 2026-07-24 via daily what's new. Fonte: [Claude Code Docs — Claude Security](https://code.claude.com/docs/en/claude-security) · [@claudeai](https://x.com/claudeai/status/2079990597973057691) · [GitHub Releases v2.1.218](https://github.com/anthropics/claude-code/releases/tag/v2.1.218).</sub>
