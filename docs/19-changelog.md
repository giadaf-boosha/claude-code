...
| 14 ago 2026 | v2.1.233 | Aggiunto supporto URL GitLab merge request al flag `--worktree`. Nuova impostazione gateway `forward_user_identity` per attribuzione utente. Supporto opzionale cgroup di memoria per comandi Bash tool su Linux. Todo/task-tracking tool disabilitati di default sui modelli piu' recenti (Opus 4.8+, Sonnet 5+). Fix sicurezza: bypass permessi PowerShell su parametri che scrivono variabili, prefix device Windows NT (`\??\`) che aggirava la validazione dei path UNC. |
| 17 ago 2026 | **v2.1.234** | **Badge GitLab merge request** in footer/statusline per repo con remote GitLab e `glab` autenticato. **Auto-continue sessione** al reset di un limite di utilizzo claude.ai (disattivabile da `/config`). Claude ora usa l'email dell'account solo per identificarti, mai per invii a servizi terzi non richiesti. **Hardening sicurezza**: letture remote, restore sessione, include CLAUDE.md, workflow script e upload rifiutano ora i path Windows NT-namespace (`\??\`), contro il vettore di leak credenziali NTLM. Remote Control sincronizza l'effort level scelto da telefono/web anche su terminale e Desktop/VS Code — vedi [17.6](./17-ide-surface.md#remote-control). `/goal` si autopulisce su errori irrecuperabili e fa check-in dopo 30+ min di attesa su task in background — vedi [25](./25-goal.md). Costo di contesto della skill `claude-api` ridotto da ~200k a ~25k token. Numerosi fix su Remote Control, cross-session messaging, permission preview e rendering markdown. |

<sub>Aggiornato 2026-08-18 via daily what's new. Fonte: [GitHub Releases v2.1.233](https://github.com/anthropics/claude-code/releases/tag/v2.1.233) · [v2.1.234](https://github.com/anthropics/claude-code/releases/tag/v2.1.234).</sub>
