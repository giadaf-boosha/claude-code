...
| 11 ago 2026 | **v2.1.228** | **Hardening skill sincronizzate da claude.ai**: non oscurano piu' comandi locali o prompt MCP (i comandi locali hanno precedenza), le loro descrizioni vengono sanificate ed etichettate, e sulla macchina locale il loro body non esegue piu' comandi `!` ne' espande file `@` — chiude una superficie di attacco reale. 18 fix totali: redraw bloccato su layout error interne, `git`/Git Bash non trovato su Windows se lanciato da una cartella padre dell'installazione git, `/tui` che tornava a un modello precedente dopo `/model`, cross-session messaging senza inbox al primo avvio post-install, leak di titolo/history in Remote Control `/resume`, cleanup sessione che cancellava contenuti della memory folder di progetto. Write tool: i modelli piu' recenti possono sovrascrivere un file non letto in sessione, come gia' l'Edit tool. |
| 11 ago 2026 | — | **Compliance API estesa a Claude Cowork e Claude Code** (beta Enterprise): copertura su desktop, web, mobile e CLI con la stessa Compliance Access Key gia' in uso, nessuna integrazione separata da costruire — vedi [18.6](./18-settings-auth.md#186-claude-for-teams--enterprise). |

<sub>Aggiornato 2026-08-12 via daily what's new. Fonte: [GitHub Releases v2.1.228](https://github.com/anthropics/claude-code/releases/tag/v2.1.228) · [@ClaudeCodeLog](https://x.com/ClaudeCodeLog/status/2087274049709707404) · [Compliance API docs](https://support.claude.com/en/articles/13015708-access-the-compliance-api). Nota: fetch diretto di anthropic.com e claude.com bloccato dal proxy di rete, ricerca completata via WebSearch/GitHub releases/X.</sub>
