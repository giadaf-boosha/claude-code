...
| 14 lug 2026 | **v2.1.208** | **Screen reader mode** (`--ax-screen-reader` / `axScreenReader`): rendering plain-text opt-in per utenti screen reader. `vimInsertModeRemaps`: mappa sequenze insert-mode a due tasti (es. `jj`) a Escape. `CLAUDE_CODE_PROCESS_WRAPPER`: forza ogni self-spawn attraverso un wrapper eseguibile per launcher aziendali. Performance: fino a 7x piu' veloce nei round di tool call con molti tool MCP (print/SDK); transcript ridotti fino a 79x in sessioni edit-intensive; bounded memory leak (fino a 64MB/server MCP stdio, LSP LRU 50-doc cap). |
| 14 lug 2026 | v2.1.209 | Fix rapido: `/model` e altri dialog non piu' bloccati nelle sessioni background di `claude agents` (revert di un guard troppo ampio). |
| 14 lug 2026 | **v2.1.210** | Fix isolamento worktree: subagenti `isolation: 'worktree'` non possono piu' eseguire comandi git-mutating sul repo principale invece che sul proprio worktree. Contatore elapsed-time live sulla riga tool summary collassata. Warning startup per regole permission `Write(path)`/`NotebookEdit(path)`/`Glob(path)` (usare `Edit(path)`/`Read(path)`). Fix `ultracode` keyword opt-in che scattava su input non-umano (webhook, PR comment relayati). Auto mode: classifier di default Sonnet 5 per sessioni esterne. |
| 15 lug 2026 | **v2.1.211** | `--forward-subagent-text` / `CLAUDE_CODE_FORWARD_SUBAGENT_TEXT`: include testo e thinking dei subagenti nello stream-json output. Fix sicurezza: le anteprime permessi neutralizzano ora caratteri bidirezionali/zero-width/quote look-alike che potevano falsificare visivamente il prompt di approvazione. Fix: auto mode non sovrascrive piu' la decisione `ask` di un hook PreToolUse; regression prompt-caching su Bedrock/Vertex/Mantle/Foundry che fatturava come nuovo il blocco di contesto finale ad ogni richiesta. |
| 17 lug 2026 | **v2.1.212** | **`/fork`** copia ora la conversazione in una nuova sessione background (riga propria in `claude agents`) invece di lanciare un subagente in-sessione; il vecchio comportamento diventa **`/subtask`**. **`/resume`** in agent view apre un picker di sessioni passate (incluse quelle eliminate). Nuovi limiti anti-loop: WebSearch max 200 chiamate/sessione (`CLAUDE_CODE_MAX_WEB_SEARCHES_PER_SESSION`), subagent spawn max 200/sessione (`CLAUDE_CODE_MAX_SUBAGENTS_PER_SESSION`, reset da `/clear`). MCP tool call oltre 2 minuti vanno automaticamente in background. `claude auto-mode reset` per ripristinare la config di default. Fix: plan mode eseguiva comandi Bash file-modifying (`touch`, `rm`) senza prompt; worktree creation seguiva symlink committati fuori dal repo. |
| 18 lug 2026 | **v2.1.214** | Fix sicurezza permessi: bypass PowerShell 5.1, comandi Bash oltre 10.000 caratteri, redirect form parsati diversamente da bash, comandi `docker`/Podman con flag daemon-redirect (`--url`, `--connection`, `--identity`) ora richiedono conferma. **`EndConversation`** tool: Claude puo' terminare sessioni con utenti fortemente abusivi o tentativi di jailbreak (gia' su claude.ai dal 2025). Heartbeat di progresso periodico per tool call di lunga durata. Fix: `/ultrareview` rifiutava repo senza merge base. |

<sub>Aggiornato 2026-07-18 via daily what's new. Fonte: [GitHub Releases v2.1.205](https://github.com/anthropics/claude-code/releases/tag/v2.1.205) · [v2.1.206](https://github.com/anthropics/claude-code/releases/tag/v2.1.206) · [v2.1.207](https://github.com/anthropics/claude-code/releases/tag/v2.1.207) · [v2.1.208](https://github.com/anthropics/claude-code/releases/tag/v2.1.208) · [v2.1.209](https://github.com/anthropics/claude-code/releases/tag/v2.1.209) · [v2.1.210](https://github.com/anthropics/claude-code/releases/tag/v2.1.210) · [v2.1.211](https://github.com/anthropics/claude-code/releases/tag/v2.1.211) · [v2.1.212](https://github.com/anthropics/claude-code/releases/tag/v2.1.212) · [v2.1.214](https://github.com/anthropics/claude-code/releases/tag/v2.1.214) · [@ClaudeDevs](https://x.com/ClaudeDevs/status/2075635283211772279).</sub>
