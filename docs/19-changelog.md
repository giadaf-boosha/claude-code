...
| 22 lug 2026 | **v2.1.218** | **`/code-review` gira come subagent in background**: la review non riempie piu' la conversazione principale e resta valido target per slash command impilati. `/ultrareview` accetta argomenti descrittivi ("review my auth changes") applicati come nota ai findings; `/code-review ultra` non gira piu' in locale su sessioni non interattive. Skill con `context: fork` girano in background di default (opt-out per skill con `background: false`). Fix di sicurezza: hook di agent frontmatter da cartelle non fidate ora richiedono trust della cartella dell'agent; percorsi Windows con segmenti `\u`-prefixed non piu' corrotti in caratteri CJK. |
| 24 lug 2026 | **v2.1.219** | **Claude Opus 5**: nuovo modello Opus di default (sostituisce Opus 4.8), context 1M, effort `low`/`medium`/`high`/`xhigh`, fast mode $10/$50 per MTok. Opus 4.7 rimosso dal fast mode. Nuovo hook **`DirectoryAdded`** (si attiva dopo `/add-dir` o `register_repo_root` SDK). Subagent annidati fino a profondita' 3 di default (era 1; `CLAUDE_CODE_MAX_SUBAGENT_SPAWN_DEPTH=1` per disattivare). `workflowSizeGuideline` configurabile da settings file. Vedi [5.9 Claude Opus 5](./05-fast-mode-1m-context.md#59-claude-opus-5--nuovo-modello-premium-da-v2119). |
| 25 lug 2026 | v2.1.220 | Bug fix e miglioramenti di affidabilita' (nessuna feature nuova). |
