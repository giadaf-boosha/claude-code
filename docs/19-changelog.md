...
| 20 lug 2026 | v2.1.216 | Nuova opzione `sandbox.filesystem.disabled` per disattivare l'isolamento filesystem del sandbox mantenendo il controllo dell'egress di rete. Bundle esteso di fix di affidabilita' e sicurezza: costo di normalizzazione dei messaggi non piu' quadratico nelle sessioni lunghe (stalli multi-secondo su resume), subagent isolati in worktree che non possono piu' reindirizzare git verso il checkout condiviso (`git -C`, `--git-dir`, `GIT_DIR`/`GIT_WORK_TREE`), `/rewind` non attraversa piu' symlink/hard link nei path tracciati, `claude daemon stop --any` non termina piu' processi non correlati via lockfile legacy stale. Nessuna feature abbastanza rilevante per la sezione "What's new today" (release quasi interamente di bug fix). |
...
<sub>Aggiornato 2026-07-21 via daily what's new. Fonte: [GitHub Releases v2.1.216](https://github.com/anthropics/claude-code/releases/tag/v2.1.216).</sub>
