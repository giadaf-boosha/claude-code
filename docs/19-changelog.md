...
| 4 ago 2026 | **v2.1.221** | **Focus view** su VS Code (`Ctrl+Alt+F`): nasconde l'attivita' dei tool dietro un riepilogo espandibile per turno con indicatore live del tool in corso. **`mode: "mask"`** per i file di credenziali sandbox su Linux/WSL: i comandi sandboxati leggono una copia sentinella mentre il proxy sandbox sostituisce il valore reale in uscita (su macOS fallback a `deny`). `prompt-audit` subcommand nella skill `claude-api`. Fix sicurezza: bypass del permission-check Bash in condizionali regex zsh `[[ ]]`; MCP server da `--mcp-config` non connessi al primo turno in print mode; file @-mentioned persi su Esc + resubmit. Vedi [17.1](./17-ide-surface.md#171-vs-code), [4.4](./04-modalita-permessi.md#44-sandbox-mode-sandbox). |
| 4 ago 2026 | **v2.1.222** | **Rimossa la feature `/ultraplan`** — vedi [15.1](./15-ultraplan-ultrareview.md#151-ultraplan-research-preview-da-v2191). Fix sicurezza: sessioni/subagent isolati in worktree potevano eseguire comandi git distruttivi contro il checkout principale (l'isolamento ora copre file edit e Bash in ogni tipo di sessione); hook `PreToolUse` auto-allow bypassavano le restrizioni sui tool nei task in background. Altri fix: `/usage` sovra-attribuiva utilizzo ai server MCP; sessioni non collegate alle PR create dopo il push del branch; Remote Control non piu' attivabile da settings repo-local (solo user scope). |
