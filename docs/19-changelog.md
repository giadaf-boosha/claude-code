...
| 18 ago 2026 | v2.1.235 | Nuovo setting opzionale `spellcheck` (supporto aspell/hunspell/ispell). Fix: invalidazione dell'intera prompt cache in alcuni scenari. Migliorato il rendering delle liste markdown, i permission dialog e l'uso di memoria/CPU delle sessioni cloud in background. |
| 19 ago 2026 | v2.1.236 | Nuova env var `ANTHROPIC_DEFAULT_MODEL` per impostare il modello di partenza delle nuove sessioni (`/model` resta prioritario e persiste). `notify_when_idle` su `SendMessage` cross-session. Migliorie sandbox per le regole wildcard read-deny su macOS. Fix: rendering del picker `/model`, display dei permission dialog, supporto screen reader del transcript in VS Code. |
| 20 ago 2026 | v2.1.237 | Fix: prompt caching rotto nelle sessioni con LLM gateway o base URL custom. Nuovo output style built-in **"Concise"**: Claude apre con i risultati, salta preamboli e narrazione pur lavorando con lo stesso livello di accuratezza (selezionabile da `/config` → Output style). |
| 20 ago 2026 | v2.1.238 | Setting `keybindingFlavor: "readline"` per il comportamento Ctrl+W in stile Bash. `headersHelper` nei plugin marketplace per generare header HTTP (es. token a breve scadenza) sulle fetch di catalog/archive. `claude self-hosted-runner --defer-shutdown-max-min <minuti>` per uno shutdown piu' morbido dei runner. Supporto `--proxy-authorization-command`/`--proxy-authorization-file` per proxy di egress con header `Proxy-Authorization` a rotazione. Fix: memory leak nelle sessioni lunghe (risultati tool dei subagent non piu' rilasciati fuori dalla finestra di display), output style personalizzati che tornavano al default a meta' sessione, numerosi fix su Remote Control e cross-session messaging. |

<sub>Aggiornato 2026-08-21 via daily what's new. Fonte: [GitHub Releases v2.1.235](https://github.com/anthropics/claude-code/releases/tag/v2.1.235) · [v2.1.236](https://github.com/anthropics/claude-code/releases/tag/v2.1.236) · [v2.1.237](https://github.com/anthropics/claude-code/releases/tag/v2.1.237) · [v2.1.238](https://github.com/anthropics/claude-code/releases/tag/v2.1.238). Nessuna voce editoriale: solo fix, setting e feature minori, sotto soglia.</sub>
