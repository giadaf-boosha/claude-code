...
| 20 lug 2026 | v2.1.216 | **`sandbox.filesystem.disabled`**: nuova opzione salta l'isolamento filesystem mantenendo il controllo dell'egress di rete. Fix rallentamento quadratico nella normalizzazione messaggi delle sessioni lunghe (stalli multi-secondo, resume lenti). Fix riautenticazione MCP che revocava credenziali funzionanti prima che il nuovo login si completasse, worktree-isolated subagent che redirigevano git verso il checkout condiviso (`git -C`, `--git-dir`, `GIT_DIR`/`GIT_WORK_TREE`), `claude daemon stop --any` che poteva terminare un processo non correlato via lockfile stale. |
| 21 lug 2026 | v2.1.217 | **Autocomplete emoji shortcode** nel prompt (`:heart:` → ❤️). **Tetto concorrenza subagent** default 20 (`CLAUDE_CODE_MAX_CONCURRENT_SUBAGENTS`); i subagent non spawnano piu' subagent annidati by default (`CLAUDE_CODE_MAX_SUBAGENT_SPAWN_DEPTH` per abilitare nesting piu' profondo). Fix `--max-budget-usd` che non fermava i subagent in background al raggiungimento del cap, memory leak su output MCP troncati, auto-update Windows che poteva lasciare `claude.exe` mancante. |
| 22 lug 2026 | **v2.1.218** | **`/code-review` gira come subagent in background** — la review non riempie piu' la conversazione principale. Aggiunto HTTP status/error text a `claude mcp list`/`/mcp` su fallimento connessione. Fix path Windows con segmenti `\u` corrotti in caratteri CJK, `/ultrareview` che falliva su argomenti descrittivi, `/code-review ultra` che eseguiva silenziosamente una review locale in sessioni non interattive invece che cloud. |

<sub>Aggiornato 2026-07-24 via daily what's new. Fonte: [GitHub Releases v2.1.216](https://github.com/anthropics/claude-code/releases/tag/v2.1.216) · [v2.1.217](https://github.com/anthropics/claude-code/releases/tag/v2.1.217) · [v2.1.218](https://github.com/anthropics/claude-code/releases/tag/v2.1.218).</sub>
