...
Cronologia completa di Claude Code dalla research preview (24 febbraio 2025, v0.2.0) all'ultima versione (14 luglio 2026, v2.1.210). 7 fasi storiche + tabella versione per versione + post-mortem aprile 2026.
...
| 14 lug 2026 | **v2.1.208** | **Screen reader mode**: rendering plain-text opt-in per tecnologie assistive (`--ax-screen-reader` / `CLAUDE_AX_SCREEN_READER=1` / `"axScreenReader": true`). `vimInsertModeRemaps`: sequenze a due tasti in Vim insert mode (es. `jj` → Escape). `CLAUDE_CODE_PROCESS_WRAPPER`: self-spawn CC rilanciato attraverso wrapper richiesto da launcher aziendali. Mouse click su menu multi-select in fullscreen. Cache tool-pool assembly (fino a 7x piu' rapido per tool call). Vedi [17.13](./17-ide-surface.md#1713-accessibilita--screen-reader-mode-v21208). |
| 14 lug 2026 | v2.1.209 | Fix: `/model` e altri dialog bloccati nelle sessioni background di `claude agents` (revertito un guard troppo ampio). |
| 14 lug 2026 | v2.1.210 | Contatore tempo trascorso live sulle righe di tool summary collassate. Warning a startup per permission rule deprecate (`Write`, `NotebookEdit`, `Glob` a livello path → usa `Edit`/`Read`). Auto mode: classifier di default su Sonnet 5 per sessioni esterne. Fix: subagent `isolation: 'worktree'` che eseguivano comandi git contro il repo principale invece del worktree isolato; keyword `ultracode` attivata da input non-umano (webhook, commenti PR); hardening Agent tool contro prompt injection indiretta via contenuto subagent. |

<sub>Aggiornato 2026-07-15 via daily what's new. Fonte: [GitHub Releases v2.1.205](https://github.com/anthropics/claude-code/releases/tag/v2.1.205) · [v2.1.206](https://github.com/anthropics/claude-code/releases/tag/v2.1.206) · [v2.1.207](https://github.com/anthropics/claude-code/releases/tag/v2.1.207) · [v2.1.208](https://github.com/anthropics/claude-code/releases/tag/v2.1.208) · [v2.1.209](https://github.com/anthropics/claude-code/releases/tag/v2.1.209) · [v2.1.210](https://github.com/anthropics/claude-code/releases/tag/v2.1.210) · [@ClaudeDevs](https://x.com/ClaudeDevs/status/2075635283211772279).</sub>
