...
| 18 ago 2026 | v2.1.235 | Nuovo setting opzionale `spellcheck` (sottolinea parole errate nel prompt via `aspell`/`hunspell`/`ispell`). Fix: invalidazione totale della prompt cache, rendering liste markdown annidate, permission prompt e tool Agent, dialog di approvazione notebook. Migliora uso memoria/CPU nelle sessioni cloud. |
| 19 ago 2026 | v2.1.236 | Nuova env var **`ANTHROPIC_DEFAULT_MODEL`**: fissa il modello di partenza delle nuove sessioni, un `/model` esplicito la sovrascrive e persiste al restart (diversamente da `ANTHROPIC_MODEL`) — vedi [05 § Aggiornamento modello](./05-fast-mode-1m-context.md). **`notify_when_idle`** su `SendMessage` cross-session: avviso one-shot opt-in quando un'altra sessione locale torna idle, senza polling (macOS/Linux) — vedi [17.6](./17-ide-surface.md#remote-control). Sandbox macOS: le regole read-deny con wildcard (es. `**/.env`) hanno ora precedenza dentro le region di lettura consentite e coprono il contenuto delle directory match, non aggirabili rinominando il file negato. Fix: housekeeping/log MCP rotti dopo rimozione della directory in cui una sessione era entrata, renderer fullscreen che restava bloccato dopo un avvio fallito. |
| 20 ago 2026 | **v2.1.237** | **Nuovo output style built-in "Concise"**: Claude va dritto ai risultati saltando preamboli e narrazione, a parita' di qualita' del lavoro — attivabile da `/config` → Output style, vedi [18 § Advanced settings](./18-settings-auth.md). Fix: prompt caching rotto per sessioni con LLM gateway o base URL custom. |

<sub>Aggiornato 2026-08-20 via daily what's new. Fonte: [GitHub Releases v2.1.235](https://github.com/anthropics/claude-code/releases/tag/v2.1.235) · [v2.1.236](https://github.com/anthropics/claude-code/releases/tag/v2.1.236) · [v2.1.237](https://github.com/anthropics/claude-code/releases/tag/v2.1.237).</sub>
