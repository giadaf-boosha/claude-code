...
| 2 set 2026 | **v2.1.259** | Setting gestito `managedMcpServers` per server MCP HTTP/SSE configurati centralmente — vedi [10](./10-mcp.md). `allowedMcpServers` ora governa solo i server aggiunti dall'utente. Flag `--permission-prompts none` per host headless unattended — vedi [4.4](./04-modalita-permessi.md). Le routine cloud riconoscono le merge request GitLab nel formato `MR !N` — vedi [13](./13-routines-cloud.md). Fix: sessioni concorrenti che si sovrascrivevano a vicenda le modifiche a `~/.claude.json`, regole deny `Read(...)` su Bash che non coprivano valori di opzione e operandi file, cache invalidata al refresh del token OAuth. |
| 3 set 2026 | **v2.1.260** | Pannello diff (`/diff`) affiancato alla conversazione in fullscreen mode — vedi [17](./17-ide-surface.md). `/cost` e la status line indicano ora la causa probabile di un cache miss (tool/system prompt cambiati, idle oltre il TTL). Workflow tool: overhead ridotto da ~5.7k a ~1k token — vedi [24](./24-workflows.md). `/reload-plugins` e forma testuale di `/advisor` anche in sessioni headless/desktop. Fix: regole permission con parentesi scartate, permission check Bash che auto-approvava alcune command substitution zsh, discovery modelli Bedrock dietro CA aziendale, prompt caching su Fable 5.1. |
| 4 set 2026 | **v2.1.261** | `/skill-doctor`: mostra quali skill caricate non vengono usate e quanto costano in context, per fare pruning — vedi [9](./09-skills.md). Setting `bashOutputMaxChars`/`taskOutputMaxChars` (fino a 128K caratteri) per l'output inline di comandi e task in background — vedi [18](./18-settings-auth.md). `--append-subagent-system-prompt-file` per system prompt di subagent troppo lunghi da riga di comando — vedi [8](./08-subagents.md). Riga "Organization policy" in `/status`/`claude doctor`. Fix: caratteri digitati/incollati fuori ordine o persi durante input veloce, `/add-dir <subdir>` su automount `/net`, setup wizard Bedrock che restava appeso senza risposta AWS. |

<sub>Aggiornato 2026-09-05 via daily what's new. Fonte: [GitHub Releases v2.1.259](https://github.com/anthropics/claude-code/releases/tag/v2.1.259) · [v2.1.260](https://github.com/anthropics/claude-code/releases/tag/v2.1.260) · [v2.1.261](https://github.com/anthropics/claude-code/releases/tag/v2.1.261) · [code.claude.com/docs/en/changelog](https://code.claude.com/docs/en/changelog). Nessun annuncio Anthropic blog ne' post X dei team dedicato su Claude Code nelle ultime 24 ore.</sub>
