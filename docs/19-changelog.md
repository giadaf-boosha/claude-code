...
Cronologia completa di Claude Code dalla research preview (24 febbraio 2025, v0.2.0) all'ultima versione (9 luglio 2026, v2.1.206). 7 fasi storiche + tabella versione per versione + post-mortem aprile 2026.
...
| 8 lug 2026 | **v2.1.205** | **Auto mode** blocca il tampering dei file di transcript di sessione e chiede conferma prima di un `rm -rf` su una variabile non risolvibile dal contesto. `/doctor` diventa full setup checkup con fix automatici (alias `/checkup`). Notifiche background task dichiarano esplicitamente "nessun input umano ricevuto", per prevenire l'esecuzione di approvazioni fabbricate nel transcript. Auto-update binario ora streamma su disco (-~400MB picco memoria). Agent view: sessioni che editano/mergiano/pushano su una PR esistente ora la linkano. Fix: `--json-schema` con schema invalido, messaggi persi a `--max-turns`, worktree removal su Windows con junction NTFS, `claude mcp add-from-claude-desktop` bloccato su nomi server non validi. |
| 9 lug 2026 | **v2.1.206** | `/cd` suggerisce percorsi directory (come `/add-dir`). `/doctor` propone di sfoltire i `CLAUDE.md` versionati. `/commit-push-pr` auto-approva il push sul remote configurato (`remote.pushDefault` o l'unico remote presente). Background agents si auto-aggiornano in background subito dopo un update CLI. `/code-review` migliora la qualita' dei finding su claude-opus-4-8 a tutti gli effort level. Fix: login scaduto con errore modello fuorviante, `--resume`/`--continue` senza input da tastiera all'avvio, MCP `request_timeout_ms` per-server ignorato, OAuth MCP re-auth loop dopo refresh fallito, prezzi errati nel picker `/model`. |

<sub>Aggiornato 2026-07-10 via daily what's new. Fonte: [GitHub Releases v2.1.202](https://github.com/anthropics/claude-code/releases/tag/v2.1.202) · [v2.1.203](https://github.com/anthropics/claude-code/releases/tag/v2.1.203) · [v2.1.204](https://github.com/anthropics/claude-code/releases/tag/v2.1.204) · [v2.1.205](https://github.com/anthropics/claude-code/releases/tag/v2.1.205) · [v2.1.206](https://github.com/anthropics/claude-code/releases/tag/v2.1.206).</sub>
