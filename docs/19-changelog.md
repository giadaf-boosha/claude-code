...
Cronologia completa di Claude Code dalla research preview (24 febbraio 2025, v0.2.0) all'ultima versione (10 settembre 2026, v2.1.268). 7 fasi storiche + tabella versione per versione + post-mortem aprile 2026.
...
| 6 set 2026 | v2.1.263 | Bug fix e miglioramenti di affidabilita' (nessun dettaglio pubblicato). |
| 8 set 2026 | v2.1.265 | `--plugin-dir` accetta ora anche cartelle intere di plugin (non solo un singolo plugin o `.zip`). Nuovo tetto di 1 GB sui risultati dei tool salvati su disco. Fix su prompt cache per subagent avviati in foreground/ripresi, resume dopo morte del processo precedente, sintassi `/model opusplan[1m]`, jump del transcript in fullscreen, permessi file su Windows AppContainer. |
| 8 set 2026 | **v2.1.266** | **Fix critico**: la variabile non documentata `CLAUDE_CODE_USE_GATEWAY`, introdotta in v2.1.265, forzava il sign-in al Cloud gateway quando combinata con API key o header di autenticazione custom, rompendo quelle configurazioni. Ripristinato: la sola variabile torna a essere ignorata — vedi [13.7b](./13-routines-cloud.md#137b-self-hosted-runner-beta-da-v21224-7-ago-2026). |
| 9 set 2026 | **v2.1.267** | **Setting `maxEffortLevel`** (globale o per modello): impone un tetto massimo di effort su tutti i provider inclusi Bedrock/Vertex/Foundry, gli utenti restano liberi di scegliere un livello piu' basso — vedi [5.3](./05-fast-mode-1m-context.md#53-opus-47--effort-xhigh). Flag `--system-prompt-snapshot off` per rendere il system prompt da zero a ogni richiesta invece di riusare quello registrato. 40+ fix su Cowork scheduled tasks, prompt cache/tool list su resume, Remote Control, GitHub Enterprise Server. |
| 10 set 2026 | **v2.1.268** | **Pricing sul Claude apps gateway**: i client autenticati ricevono le stesse tariffe dell'API diretta tramite i managed settings dell'org, con `/cost` e telemetria allineati al contatore di spesa — vedi [13.7b](./13-routines-cloud.md#137b-self-hosted-runner-beta-da-v21224-7-ago-2026). Nuovo flag `claude self-hosted-runner --remove-session-state`. Fix: `WebFetch` restava appeso indefinitamente su server che non chiudevano la risposta (ora timeout 300s, configurabile con `CLAUDE_CODE_WEBFETCH_DEADLINE_MS`); prompt cache/extended thinking che si rompevano a meta' sessione per sessioni SDK con `excludeDynamicSections`. Icone browser-tab per gli artifact pubblicati. 60+ fix aggiuntivi su permessi, MCP OAuth, Remote Control e Claude in Slack. |

<sub>Aggiornato 2026-09-11 via daily what's new. Fonte: [GitHub Releases v2.1.263](https://github.com/anthropics/claude-code/releases/tag/v2.1.263) · [v2.1.265](https://github.com/anthropics/claude-code/releases/tag/v2.1.265) · [v2.1.266](https://github.com/anthropics/claude-code/releases/tag/v2.1.266) · [v2.1.267](https://github.com/anthropics/claude-code/releases/tag/v2.1.267) · [v2.1.268](https://github.com/anthropics/claude-code/releases/tag/v2.1.268) · [code.claude.com/docs/en/changelog](https://code.claude.com/docs/en/changelog). La routine daily era ferma dal 5 set: questa edizione recupera 6-10 settembre in un colpo solo. Quattro voci editoriali (pricing gateway, fix critico `CLAUDE_CODE_USE_GATEWAY`, `maxEffortLevel`, `--plugin-dir` su cartelle); il resto (fix WebFetch/prompt-cache, icone artifact, decine di fix minori) resta sotto soglia. Nessun post dedicato dai team account tracciati ne' annuncio Anthropic blog specifico su Claude Code nella finestra. Digest ufficiale settimanale fermo alla Week 34 (17-21 ago, gia' coperta).</sub>
