...
| 8 lug 2026 | **v2.1.205** | **`/doctor` diventa full setup checkup**: non solo report read-only — diagnostica installazione, skill/MCP/plugin inutilizzati vs costo di context, deduplica `CLAUDE.md` locali vs quelli committati, propone di snellire `CLAUDE.md` di contenuto derivabile dal codice, segnala hook lenti. Mostra i findings e chiede conferma prima di modificare; alias `/checkup`. Auto mode blocca ora il tampering dei file di transcript di sessione. Fix: `--json-schema` con `format` produceva output non strutturato; rimozione worktree su Windows che cancellava file fuori dal worktree se presente junction/symlink NTFS. |
| 9 lug 2026 | v2.1.206 | `/cd` suggerisce percorsi directory mentre digiti (come `/add-dir`). `/commit-push-pr` auto-consente `git push` anche verso il remote configurato del repo, non solo `origin`. Gateway: `/login` supporta endpoint gateway pubblici gestiti da Anthropic. `EnterWorktree` chiede conferma prima di entrare in un worktree fuori da `.claude/worktrees/`. Background agent si aggiornano in background subito dopo un update di Claude Code, senza upgrade lento al re-attach. Fix: 15+ bug su MCP, Bedrock, agent view, `/model` picker. |
| 6-10 lug 2026 | — | **In-app browser su Desktop**: Claude Code su Desktop ha ora un browser integrato — Claude apre documentazione, design o qualsiasi sito e legge, clicca, interagisce con le pagine come gia' fa con le preview del dev server locale. Sandboxato e configurabile: si sceglie se le sessioni di navigazione persistono, e classificatori di sicurezza rivedono le azioni su siti esterni. Fonte: [Week 28 digest](https://code.claude.com/docs/en/whats-new/2026-w28). Doc: [docs/17-ide-surface.md](./17-ide-surface.md). |
| 11 lug 2026 | v2.1.207 | Auto mode disponibile senza opt-in `CLAUDE_CODE_ENABLE_AUTO_MODE` su Bedrock, Vertex AI e Foundry (disattivabile con `disableAutoMode`). Bedrock/Vertex/Claude Platform on AWS passano a Opus 4.8 come default. Fix: freeze terminale su stream di liste/tabelle molto lunghe; falsi warning prompt-injection; auto-updater che sovrascriveva launcher script custom; crash loop agent teams su messaggi mailbox malformati. |

<sub>Aggiornato 2026-07-11 via daily what's new. Fonte: [GitHub Releases v2.1.205](https://github.com/anthropics/claude-code/releases/tag/v2.1.205) · [v2.1.206](https://github.com/anthropics/claude-code/releases/tag/v2.1.206) · [v2.1.207](https://github.com/anthropics/claude-code/releases/tag/v2.1.207) · [Week 28 whats-new digest](https://code.claude.com/docs/en/whats-new/2026-w28).</sub>
