...
| 18 ago 2026 | v2.1.235 | Nuovo setting opzionale `spellcheck` (supporto aspell/hunspell/ispell) per sottolineare gli errori di battitura nei prompt. Fix: invalidazione dell'intera prompt cache alla disconnessione del language server, dialoghi di permesso e rendering di liste markdown annidate migliorati. |
| 19 ago 2026 | v2.1.236 | Nuova env var `ANTHROPIC_DEFAULT_MODEL`. Aggiunto `notify_when_idle` a `SendMessage` cross-session. Migliorate le regole wildcard read-deny del sandbox macOS. Fix: clipboard/housekeeping rotto se la cartella di sessione viene rimossa, connessioni MCP v2 che riaprivano all'infinito le subscription, hook `Notification` che non scattava per i permission prompt sotto Claude Desktop/VS Code. |
| 19 ago 2026 | v2.1.237 | Nuovo output style integrato **"Concise"** (risultati diretti, salta il preambolo). Fix: prompt caching rotto nelle sessioni con LLM gateway o base URL custom. |
| 20 ago 2026 | v2.1.238 | Nuovo setting `keybindingFlavor` per cancellazione parola in stile readline (Ctrl+W). Marketplace plugin: `headersHelper` per mint di header HTTP. Nuovi flag `--proxy-authorization-command/file` per proxy egress. Fix: memory leak (crescita illimitata di memoria) nelle sessioni interattive lunghe, output style custom che si resettava a meta' sessione, resilienza riconnessione Remote Control (~30 min). |
| 21 ago 2026 | v2.1.239 | Le stime di costo includono ora un premium 1.1x inferenza US-only per i workspace con residenza dati. Nuovo `/claude-api upgrade` per migrare progetti Python da `anthropic` 0.x a 1.x. Sessioni cloud: i plugin sincronizzati da claude.ai mostrano ora `nome@synced`. Build Alpine/musl: paste immagini native, clipboard e cattura audio ora funzionano. Fix: server MCP remoti che restavano falliti dopo errori 5xx transitori, prompt del renderer fullscreen che ricompariva inutilmente. |

<sub>Aggiornato 2026-08-22 via daily what's new. Fonte: [GitHub Releases v2.1.235](https://github.com/anthropics/claude-code/releases/tag/v2.1.235) · [v2.1.236](https://github.com/anthropics/claude-code/releases/tag/v2.1.236) · [v2.1.237](https://github.com/anthropics/claude-code/releases/tag/v2.1.237) · [v2.1.238](https://github.com/anthropics/claude-code/releases/tag/v2.1.238) · [v2.1.239](https://github.com/anthropics/claude-code/releases/tag/v2.1.239). Nessuna voce editoriale: solo fix e setting minori, sotto soglia.</sub>
