...
| 7 ago 2026 | **v2.1.224** | **Self-hosted environments**: `claude self-hosted-runner` fa girare le sessioni web/mobile/desktop sull'infrastruttura del team (Team/Enterprise, beta pubblica) — vedi [18](./18-settings-auth.md). **Cross-session messaging**: le sessioni Claude Code si scambiano messaggi tra loro su macOS/Linux, scoperte via `ListAgents` — vedi [12](./12-agent-teams.md). Archive plugin source (zip via HTTPS, pinning SHA-256). Sandbox credential-masking: `extract`/`onExtractNoMatch`, `decode: "jwt"` con `maskClaims`, `awsPairs`/`sigv4`. Rimosso il tetto di 200 subagent per sessione. Fix sicurezza: regole `denyRead`/`denyWrite` con trailing slash bypassabili su Linux/macOS, `SendMessage` che riportava "Message sent" anche a scrittura fallita. |
| 8 ago 2026 | **v2.1.225** | **Gateway spend-limit**: il messaggio di limite raggiunto mostra tetto, orario di reset e messaggio dell'operatore. Trust prompt per `claude agents` su directory non fidate. Cross-session messaging esteso: `SendMessage` avvia conversazioni con sessioni Remote Control su altre macchine chiamandole per nome (`ListAgents` le mostra come `nome [ref]`). Fix: token OAuth long-lived sovrascritto da uno short-lived (sessioni headless), fallimenti MCP OAuth su macOS per timeout keychain, fold Focus view VS Code che nascondeva la to-do list. |
| 8 ago 2026 | v2.1.226 | Bug fix e miglioramenti di affidabilita'. |

<sub>Aggiornato 2026-08-08 via daily what's new. Fonte: [Anthropic blog](https://claude.com/blog/run-claude-code-sessions-on-your-own-compute) · [@ClaudeDevs](https://x.com/ClaudeDevs/status/2085817074816070014) · [GitHub Releases v2.1.224](https://github.com/anthropics/claude-code/releases/tag/v2.1.224) · [v2.1.225](https://github.com/anthropics/claude-code/releases/tag/v2.1.225) · [v2.1.226](https://github.com/anthropics/claude-code/releases/tag/v2.1.226).</sub>
