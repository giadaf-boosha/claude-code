...
Cronologia completa di Claude Code dalla research preview (24 febbraio 2025, v0.2.0) all'ultima versione (15 luglio 2026, v2.1.211). 7 fasi storiche + tabella versione per versione + post-mortem aprile 2026.
...
| 14 lug 2026 | v2.1.208 | **Screen reader mode** (`claude --ax-screen-reader` / `CLAUDE_AX_SCREEN_READER=1` / `axScreenReader` setting): rendering plain-text opt-in per screen reader. `vimInsertModeRemaps`: mappa sequenze insert-mode tipo `jj`→Escape. `CLAUDE_CODE_PROCESS_WRAPPER`: onora un launcher aziendale richiesto per ogni self-spawn del processo. Performance: fino a 7x piu' veloci i round di tool ad alto conteggio MCP (tool-pool caching), transcript ridotti fino a 79x in sessioni edit-heavy, memory leak risolti (MCP stdio stderr, LSP documents, async hook output). ~40 fix di affidabilita' (fast mode, background agent, stream-json, Bedrock streaming, Grep/Glob/Edit, `/usage`, `/mcp`). |
| 14 lug 2026 | v2.1.209 | Fix: `/model` e altri dialog erano bloccati nelle sessioni background `claude agents` (revert di una guard troppo ampia). |
| 14 lug 2026 | v2.1.210 | Contatore tempo trascorso live sulla riga tool summary collassata. Warning di startup per regole permesso `Write(path)`/`NotebookEdit(path)`/`Glob(path)` (usare `Edit(path)`/`Read(path)`). **Fix sicurezza**: subagent con `isolation: 'worktree'` potevano eseguire comandi git-mutanti sul checkout principale invece che sul proprio worktree isolato. Regole permesso "always allow" ora salvate a livello di repository root (persistono tra worktree). Auto mode: classifier di default passa a Sonnet 5 per sessioni esterne. Hardening dell'Agent tool contro indirect prompt injection via contenuto letto da un subagent. |
| 15 lug 2026 | **v2.1.211** | Nuovo flag `--forward-subagent-text` (+ `CLAUDE_CODE_FORWARD_SUBAGENT_TEXT`): include testo e thinking dei subagent nello stream-json output. **Fix sicurezza**: permission preview inoltrate a chat esterne ora neutralizzano caratteri bidirectional-override, zero-width e virgolette look-alike che potevano alterare visivamente il messaggio di approvazione. Fix: auto mode non sovrascrive piu' la decisione `ask` di un hook PreToolUse su Bash non sandboxato; env var numeriche (timeout, token budget, retry count) accettano notazione scientifica e digit-separator (`1e6`, `64_000`); sessioni parallele non si disconnettono piu' tutte insieme dopo wake-from-sleep condividendo un credential store; subagent con model override esplicito non perdono piu' il proprio modello al resume. |

<sub>Aggiornato 2026-07-16 via daily what's new. Fonte: [changelog](https://code.claude.com/docs/en/changelog) · [GitHub Releases v2.1.208](https://github.com/anthropics/claude-code/releases/tag/v2.1.208) · [v2.1.209](https://github.com/anthropics/claude-code/releases/tag/v2.1.209) · [v2.1.210](https://github.com/anthropics/claude-code/releases/tag/v2.1.210) · [v2.1.211](https://github.com/anthropics/claude-code/releases/tag/v2.1.211).</sub>
