...
Cronologia completa di Claude Code dalla research preview (24 febbraio 2025, v0.2.0) all'ultima versione (15 settembre 2026, v2.1.273). 7 fasi storiche + tabella versione per versione + post-mortem aprile 2026.
...
| 12 set 2026 | v2.1.270 | Fix: comandi Bash git read-only (es. `git status`, `git log`) che tornavano a chiedere permesso dopo una sessione lunga — regressione introdotta in v2.1.269. |
| 14 set 2026 | **v2.1.271** | **Fast mode nelle sessioni Claude Code Remote** (cloud e self-hosted runner). **Auto mode**: `allowed_domains` per-comando su Bash/PowerShell/Monitor, per aprire l'accesso di rete solo agli host che un comando dichiara di usare — vedi [4.x](./04-modalita-permessi.md). `omitClaudeMd` in agent frontmatter per subagent senza CLAUDE.md ereditato. Supporto mouse in `/config` fullscreen. Setting `multiplier` (1-10) per pricing gateway con markup. 150+ fix, tra cui `.git/config.lock` stantio che rompeva git per il resto della sessione e `/resume` che perdeva il context window 1M al cambio famiglia di modello. |
| 15 set 2026 | v2.1.272 | Bug fix e miglioramenti di affidabilita'. |
| 15 set 2026 | **v2.1.273** | **Notifiche di disconnessione MCP** con riconnessione automatica segnalata via `/mcp` — vedi [10](./10-mcp.md). Nuovi header opzionali per gateway LLM (`x-claude-code-request-class` e affini, opt-in `CLAUDE_CODE_GATEWAY_HINT_HEADERS=1`). Le sessioni avviate con `--remote-control` possono ora fare **fork** per girare in background sul proprio computer — vedi [13](./13-routines-cloud.md). 60+ fix, tra cui il context meter che contava i turni degli advisor-tool a ~2x il peso reale e i sub-agent/background agent segnalati come falliti quando la risposta finale ometteva l'uso token. |

<sub>Aggiornato 2026-09-16 via daily what's new. Fonte: [GitHub Releases v2.1.270](https://github.com/anthropics/claude-code/releases/tag/v2.1.270) · [v2.1.271](https://github.com/anthropics/claude-code/releases/tag/v2.1.271) · [v2.1.272](https://github.com/anthropics/claude-code/releases/tag/v2.1.272) · [v2.1.273](https://github.com/anthropics/claude-code/releases/tag/v2.1.273) · [claude.com/blog](https://claude.com/blog/agentic-coding-is-straining-ci-heres-how-we-scaled-test-impact-analysis-at-anthropic). Tre voci editoriali: fast mode + `allowed_domains` per-comando in auto mode (v2.1.271), riconnessione MCP + fork sessioni Remote Control (v2.1.273), e il post Anthropic sulla scalata del test impact analysis interno (CI cresciuta 25x in 6 mesi). Il resto (gateway hint headers, `multiplier` pricing, fix vari) resta sotto la soglia editoriale. Nessun post X nuovo e rilevante nelle ultime 24 ore dai team account monitorati; digest ufficiale settimanale fermo alla Week 37 (7-11 set, gia' coperta).</sub>
