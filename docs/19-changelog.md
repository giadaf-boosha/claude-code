...
| 18 ago 2026 | v2.1.235 | Setting opzionale `spellcheck`: sottolinea le parole non riconosciute nel prompt input mentre digiti, usando aspell/hunspell/ispell installato in locale. Fix invalidazione prompt cache su disconnessione/riconnessione language server, allineamento Vim mode e dialog di permesso. |
| 19 ago 2026 | **v2.1.236** | **`ANTHROPIC_DEFAULT_MODEL`**: nuova env var che imposta il modello di partenza per le nuove sessioni — un pick esplicito da `/model` la sovrascrive e persiste comunque tra i restart. `notify_when_idle` su `SendMessage` cross-session: una sessione puo' chiedere a un'altra sulla stessa macchina di avvisarla al prossimo ritorno idle. Fix fullscreen renderer, `/model` picker, sandbox macOS. |
| 19 ago 2026 | **v2.1.237** | **Concise output style**: nuovo stile di risposta built-in — Claude apre col risultato e salta preamboli/narrazione, con lo stesso livello di completezza dello stile Default quando chiedi dettaglio; error report, warning di sicurezza e conferme di azioni distruttive restano invariati. Attivabile da `/config` → Output style o `"outputStyle": "Concise"` in `settings.json` — vedi [18.2](./18-settings-auth.md#182-campi-principali-selezione). Descritto da Boris Cherny come soluzione ponte, con altro in arrivo. Fix invalidazione prompt cache su gateway LLM/base URL custom. |
| 20 ago 2026 | v2.1.238 | `keybindingFlavor` per keybinding in stile readline nel prompt (es. `Ctrl+W` si ferma sullo spazio come in Bash). Plugin marketplace: `headersHelper` esegue comandi che generano header HTTP dinamici (token) a install/update. `claude self-hosted-runner --defer-shutdown-max-min <minuti>` per drenare le sessioni collegate dopo SIGTERM. Fix memory leak su sessioni interattive lunghe. |
| 21 ago 2026 | **v2.1.239** | **`/design`** (research preview): porta il workflow ad artboard di Claude Design dentro CLI e Claude Code Desktop, basato su artifacts — descrivi cosa vuoi progettare, Claude legge la codebase, pubblica un canvas di artboard modificabili nello stesso stile UI, scegli e implementa. Disponibile su Pro, Max, Team, Enterprise (richiede v2.1.233+) — vedi [03.1](./03-slash-commands.md#31-tabella-completa). **Remote Control esce da research preview**: qualsiasi macchina con `claude remote-control` attivo appare ora come device card in cima al tab Code dell'app mobile, da cui avviare al volo una sessione — vedi [17.6](./17-ide-surface.md#remote-control). `/claude-api upgrade` migra progetti Python da `anthropic` 0.x a 1.x. Stima costi (`/cost`, statusline, `--max-budget-usd`) include ora il premio 1.1x US-only-inference per i workspace data-residency. |
| 22 ago 2026 | v2.1.240 | Bug fix e miglioramenti di affidabilita'. |
| 23 ago 2026 | v2.1.241 | Bug fix e miglioramenti di affidabilita'. |

<sub>Aggiornato 2026-08-23 via daily what's new. Fonte: [code.claude.com/docs/en/whats-new/2026-w34](https://code.claude.com/docs/en/whats-new/2026-w34) · [@ClaudeDevs](https://x.com/ClaudeDevs/status/2089471692762673408) (`/design`) · [@ClaudeDevs](https://x.com/ClaudeDevs/status/2090245922685063634) (Concise) · [GitHub Releases v2.1.235](https://github.com/anthropics/claude-code/releases/tag/v2.1.235)–[v2.1.241](https://github.com/anthropics/claude-code/releases/tag/v2.1.241).</sub>
