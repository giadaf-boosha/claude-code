...
| 14 ago 2026 | **v2.1.233** | **`--worktree` supporta URL merge request GitLab** come sorgente branch, visibile anche in `claude agents` — vedi [2.3](./02-cli-installazione.md#23-flag-cli-selezionati-al-21119). **Tool di task-tracking disattivati di default** su Opus 4.8, Sonnet 5, Fable 5 e modelli successivi (`TodoWrite`, `TaskCreate/Get/Update/List`) — riattivabili con `CLAUDE_CODE_ENABLE_TODO_TOOLS=1` — vedi [8.3](./08-subagents.md#83-subagents-built-in). Nuova opzione gateway `forward_user_identity` per spend attribution per-utente, supporto opzionale a cgroup di memoria per i comandi Bash tool su Linux, env `CLAUDE_CODE_WEBFETCH_CACHE_TTL_MS`. Fix sicurezza: path Windows con prefisso NT device che bypassava la validazione UNC. Fix: sessioni cloud marcate erroneamente "lost" durante lo shutdown, connessioni MCP v2 che riaprivano subscription all'infinito, sessioni idle su Linux al 100% CPU, alias skill che riportavano "Unknown command". Revertite le modifiche ai permessi Bash di v2.1.232 per symlink Cygwin-style. |

<sub>Aggiornato 2026-08-17 via daily what's new. Fonte: [GitHub Releases v2.1.233](https://github.com/anthropics/claude-code/releases/tag/v2.1.233) · [changelog ufficiale](https://code.claude.com/docs/en/changelog). Nessuna release CLI ulteriore rilevata nella finestra 15-17 ago (incidente transitorio Claude.ai/Code/Cowork risolto il 16 ago non riportato: nessun impatto sul prodotto, fuori soglia editoriale).</sub>
