...
Cronologia completa di Claude Code dalla research preview (24 febbraio 2025, v0.2.0) all'ultima versione (7 agosto 2026, v2.1.224). 7 fasi storiche + tabella versione per versione + post-mortem aprile 2026.
...
| 7 ago 2026 | **v2.1.224** | **Cross-session `SendMessage`**: le sessioni Claude Code si scambiano messaggi tra loro (locali su macOS/Linux, o cloud), individuandosi con `ListAgents` — generalizza ad ogni sessione indipendente il pattern lead/teammate finora riservato ad [Agent Teams](./12-agent-teams.md). **`claude self-hosted-runner`**: nuovo comando per far girare sessioni web/mobile/desktop su macchine o container proprietari (Team/Enterprise) — vedi [02](./02-cli-installazione.md#22-comandi-cli-principali). Marketplace: installazione plugin da archivio `.zip` via HTTPS con pinning SHA-256 opzionale — vedi [11.3](./11-plugins-marketplace.md#113-comandi-plugin). Rimosso il tetto di 200 subagent spawnabili per sessione (restano i limiti di concorrenza/depth) — vedi [08](./08-subagents.md). Fix sicurezza: sandbox filesystem deny-entry con trailing slash bypassabili su Linux/macOS, `SendMessage` che riportava successo anche a scrittura fallita. |
