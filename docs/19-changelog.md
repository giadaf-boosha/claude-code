...
| 18 ago 2026 | v2.1.235 | Nuovo setting opzionale `spellcheck`: sottolinea parole errate nel prompt mentre si scrive, usando `aspell`/`hunspell`/`ispell` installati localmente. Fix: invalidazione dell'intera prompt cache alla disconnessione/riconnessione di un language server, disallineamento di liste markdown annidate a profondita' 3+, highlight del prompt sfasati su input multi-riga, dialoghi permessi disallineati col testo del grant. `SendMessage` rifiuta ora subito i messaggi troppo grandi invece di scartarli in silenzio. |
| 19 ago 2026 | v2.1.236 | Nuova env var `ANTHROPIC_DEFAULT_MODEL` (modello di partenza per nuove sessioni, sovrascritta e persistita da una scelta `/model`). `notify_when_idle` su `SendMessage` cross-session: chiede a un'altra sessione sulla stessa macchina un avviso quando torna idle (opt-in, one-shot, macOS/Linux). Sandbox macOS: le regole wildcard di read-deny (es. `**/.env`) hanno ora precedenza dentro le region di lettura consentite e non sono piu' aggirabili rinominando il file negato. Auto mode: le regole allow di `Monitor` vengono accantonate mentre l'auto mode e' attivo (stessa revisione dei comandi Bash); classifier allineato a Bedrock/Vertex/Foundry e con telemetria disattivata. Fix: sessioni/clipboard/log MCP rotti dopo rimozione della directory corrente (regressione 2.1.229), fullscreen renderer che restava bloccato dopo un avvio fallito, `/model` picker piu' alto dello schermo, crash su chiusura tag malformata in `SendMessage`, subprocess falliti senza gestione (es. PowerShell su WSL, regressione 2.1.234). VS Code: screen reader sul transcript. |
| 20 ago 2026 | v2.1.237 | Nuovo output style built-in **"Concise"**: Claude apre con il risultato, salta preamboli e narrazione mantenendo la stessa profondita' di lavoro (attivabile da `/config` → Output style o `"outputStyle": "Concise"` in settings.json). Fix prompt caching per sessioni dietro LLM gateway o base URL custom. |

<sub>Aggiornato 2026-08-20 via daily what's new. Fonte: [GitHub Releases v2.1.235](https://github.com/anthropics/claude-code/releases/tag/v2.1.235) · [v2.1.236](https://github.com/anthropics/claude-code/releases/tag/v2.1.236) · [v2.1.237](https://github.com/anthropics/claude-code/releases/tag/v2.1.237). Nessuna voce editoriale: solo fix e feature minori (spellcheck, `ANTHROPIC_DEFAULT_MODEL`, `notify_when_idle`, output style "Concise", hardening sandbox macOS), sotto soglia.</sub>
