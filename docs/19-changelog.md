...
Cronologia completa di Claude Code dalla research preview (24 febbraio 2025, v0.2.0) all'ultima versione (12 agosto 2026, v2.1.229). 7 fasi storiche + tabella versione per versione + post-mortem aprile 2026.
...
| 11 ago 2026 | v2.1.228 | Fix: sessioni interattive che smettevano di ridisegnarsi dopo un errore di layout interno raro, rilevamento git/Git Bash rotto su Windows in alcuni scenari di installazione, `/tui` che ripristinava il modello di sessione precedente dopo un cambio con `/model`, problemi di cross-session messaging all'avvio, self-hosted runner che saltavano repository con checkout falliti. Hardening delle skill sincronizzate da claude.ai contro lo shadowing di comandi locali. Messaggi cross-session migliorati con visualizzazione inline di mittente e corpo. |
| 12 ago 2026 | v2.1.229 | **Plugin marketplace `command` sources**: un comando locale (es. l'IDE) stampa la directory del plugin, ri-risolta a ogni sessione e applicata senza restart (`mode: "link"` la usa al posto). SSE keepalive ping sulle risposte di streaming del gateway durante pause di thinking lunghe, per evitare disconnessioni da idle-timeout su Vertex/Bedrock. `ListAgents` marca ora le sessioni Remote Control disconnesse come `offline` e le sessioni cloud come `cloud`. `/commit-push-pr` non auto-approva piu' comandi git/gh con flag pericolosi (`--force`, `--amend`, `--no-verify`, ecc). Numerosi fix di crash (glob/file_path/command non-stringa nei tool call, RangeError su terminali stretti, path Windows estesi/UNC) e di stabilita' (self-hosted runner, IDE extension, workflow fan-out). |

<sub>Aggiornato 2026-08-13 via daily what's new. Fonte: [GitHub Releases v2.1.228](https://github.com/anthropics/claude-code/releases/tag/v2.1.228) · [v2.1.229](https://github.com/anthropics/claude-code/releases/tag/v2.1.229). Nessuna voce editoriale: solo bug fix/hardening/rifiniture VS Code, sotto soglia.</sub>
