...
| 23 giu 2026 | **v2.1.187** | **`sandbox.credentials`**: impostazione per bloccare nei comandi sandboxed l'accesso a file credenziali e variabili d'ambiente segrete. **Org model restrictions**: restrizioni configurate a livello org appaiono nel picker con messaggio "restricted by your organization's settings" (`--model`, `/model`, `ANTHROPIC_MODEL`). **Mouse nei menu fullscreen**: clic del mouse nei menu di selezione (permission prompts, `/model`, `/config`). Fix `--resume` no-model-turn ("No conversation found"); fix StructuredOutput loop; fix MCP remote timeout (configurabile via `CLAUDE_CODE_MCP_TOOL_IDLE_TIMEOUT`); fix Remote Control -2.7s startup; fix CJK mojibake; fix VSCode su sessioni grandi. `/btw` ←/→ navigation; `/install-github-app` workflow opzionale; `/plugin` mostra plugin non utilizzati. |
