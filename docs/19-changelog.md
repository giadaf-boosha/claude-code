...
| 14 lug 2026 | **v2.1.208** | **Screen reader mode** (`claude --ax-screen-reader` / `CLAUDE_AX_SCREEN_READER=1` / `axScreenReader` in settings): rendering plain-text opt-in per screen reader. `vimInsertModeRemaps`: remap due tasti in insert mode (es. `jj` → Escape). `CLAUDE_CODE_PROCESS_WRAPPER` per launcher aziendali che richiedono wrapper obbligatorio sui self-spawn. Mouse click su menu multi-select e riga "Other" in fullscreen. Performance: fino a **7x piu' veloce** nei tool round di sessioni print/SDK con molti server MCP (cache del tool-pool assembly); transcript ridotti fino a **79x** nelle sessioni edit-heavy; diversi memory leak risolti (MCP stdio stderr, LSP documents, hook output, tool-result payload). |
| 14 lug 2026 | v2.1.209 | Fix (hotfix): `/model` e altri dialog non piu' bloccati nelle sessioni background di `claude agents` — reverte un guard troppo ampio introdotto in v2.1.208. |
| 14 lug 2026 | **v2.1.210** | Warning di avvio per le regole permission `Write(path)`, `NotebookEdit(path)` e `Glob(path)` (usa `Edit(path)`/`Read(path)`). Auto mode: il classifier di permission usa ora **Sonnet 5 di default per sessioni esterne**, validato alla prima richiesta e fissato per la sessione. Fix: subagent con `isolation: 'worktree'` potevano eseguire comandi git-mutanti sul repo principale invece che sul proprio worktree isolato; keyword `ultracode` che si attivava su input non generati da un umano (webhook, commenti PR relayati); scritture di memoria oltre il limite di lettura di `MEMORY.md` producono ora un errore esplicito invece di un troncamento silenzioso. Contatore elapsed-time live sulle tool call lunghe collassate. |

<sub>Aggiornato 2026-07-15 via daily what's new. Fonte: [GitHub Releases v2.1.208](https://github.com/anthropics/claude-code/releases/tag/v2.1.208) · [v2.1.209](https://github.com/anthropics/claude-code/releases/tag/v2.1.209) · [v2.1.210](https://github.com/anthropics/claude-code/releases/tag/v2.1.210).</sub>
