...
Cronologia completa di Claude Code dalla research preview (24 febbraio 2025, v0.2.0) all'ultima versione (17 settembre 2026, v2.1.274). 7 fasi storiche + tabella versione per versione + post-mortem aprile 2026.
...
| 12 set 2026 | v2.1.270 | Fix: regressione (introdotta in v2.1.269) per cui i comandi git read-only in Bash chiedevano permesso inaspettatamente dopo che una sessione girava da un po'. |
| 14 set 2026 | **v2.1.271** | **Fast mode nelle sessioni Remote** (cloud e self-hosted runner) — vedi [5.1](./05-fast-mode-1m-context.md#51-fast-mode-opus-48-di-default-da-v21154). **`allowed_domains` per-comando** su Bash/PowerShell/Monitor in auto mode con sandboxing: gli host necessari a un comando vengono rivisti insieme al comando e aperti solo per lui — vedi [4.4](./04-modalita-permessi.md#sandbox). **`omitClaudeMd`** nel frontmatter agent per far girare subagent custom/plugin senza CLAUDE.md utente/progetto/locale — vedi [8.2](./08-subagents.md#82-frontmatter-completo). Moltiplicatore `modelPricing` fino a 10x per tariffe di chargeback interno. Ampi fix su Bash permission checks, MCP OAuth, cross-session messaging, resume/teleport. |
| 15 set 2026 | v2.1.272 | Bug fix e miglioramenti di affidabilita'. |
| 15 set 2026 | **v2.1.273** | **Gateway hint headers** (`x-claude-code-request-class` e affini) per LLM gateway, opt-in `CLAUDE_CODE_GATEWAY_HINT_HEADERS=1`. **Fork di sessioni Remote Control** (`claude --remote-control`/`/remote-control`) come sessioni locali in background — vedi [17.6](./17-ide-surface.md#remote-control). Notifica quando un server MCP si disconnette a meta' sessione e la riconnessione automatica fallisce. Fix: context meter/auto-compact che contava i turni advisor-tool a circa il doppio del reale (auto-compact scattava a meta' finestra). |
| 17 set 2026 | **v2.1.274** | Warning visibile per memoria critica con istruzioni per liberarla o riavviare in sicurezza. Sessioni che si autoripristinano da transcript corrotti ("unexpected tool_use_id") invece di restare bloccate in retry infinito. Negoziazione protocollo **MCP 2026-07-28** estesa di default a Bedrock/Vertex/Foundry e installazioni con telemetria disattivata — vedi [10.2b](./10-mcp.md#102b-spec-mcp-2026-07-28-core-stateless). `/code-review` passa a prompt di review inline piu' snelli per i modelli senza settings dedicati, invece di spawnare piu' subagent. `CLAUDE_CODE_MCP_STARTUP_WAIT_MS` per limitare l'attesa dei server MCP al primo turno non interattivo. |

<sub>Aggiornato 2026-09-17 via daily what's new. Fonte: [GitHub Releases v2.1.270](https://github.com/anthropics/claude-code/releases/tag/v2.1.270) · [v2.1.271](https://github.com/anthropics/claude-code/releases/tag/v2.1.271) · [v2.1.272](https://github.com/anthropics/claude-code/releases/tag/v2.1.272) · [v2.1.273](https://github.com/anthropics/claude-code/releases/tag/v2.1.273) · [v2.1.274](https://github.com/anthropics/claude-code/releases/tag/v2.1.274) · [Anthropic blog — Cowork+chat merge](https://claude.com/blog/cowork-is-now-claude) · [Anthropic blog — test impact analysis](https://claude.com/blog/agentic-coding-is-straining-ci-heres-how-we-scaled-test-impact-analysis-at-anthropic). Voci editoriali: v2.1.274 (MCP 2026-07-28 esteso, self-healing transcript, memory warning), v2.1.273 (fork Remote Control, fix context meter), v2.1.271 (fast mode Remote, sandboxing per-comando, `omitClaudeMd`), il merge Cowork/chat e il case study CI. Il resto (v2.1.270, v2.1.272, hardening minori) resta sotto la soglia editoriale. Nota: routine ferma dal 13 al 16 set, questa edizione copre 12→17 settembre.</sub>
