...
Cronologia completa di Claude Code dalla research preview (24 febbraio 2025, v0.2.0) all'ultima versione (11 settembre 2026, v2.1.269). 7 fasi storiche + tabella versione per versione + post-mortem aprile 2026.
...
## 19.9 Tabella versione per versione (v0.2.0 → v2.1.274)
...
| 12 set 2026 | v2.1.270 | Fix: comandi git read-only in Bash chiedevano permesso a torto dopo che una sessione girava da tempo (regressione v2.1.269). |
| 14 set 2026 | v2.1.271 | Fast mode disponibile anche nelle sessioni Claude Code Remote. `allowed_domains` per-comando in Bash/PowerShell/Monitor con sandboxing di rete. `omitClaudeMd` nel frontmatter agent per girare senza CLAUDE.md. Auto-archiviazione sessioni in VS Code (14 giorni di default). Fix: policy organizzative riusate dopo cambio account/API key, controlli permessi Bash che perdevano file con certe opzioni. |
| 15 set 2026 | **Claude Mods** | **Nuovo sistema di estensione "Function Hooks"** (Early Access): un mod e' un plugin il cui comportamento vive in un modulo hooks con un unico entry point `register(on, options)` che intercetta gli eventi del motore agentico (`$` interfaccia engine, `e` evento, `next` per il hook successivo) — puo' modificare una tool call, aggiungere componenti interattivi all'interfaccia o nuovi "nomi" tipizzati su `$` (es. `$.telemetry`, `$.diff`). Mod di esempio nel repo: `sec-default`, `diff`, `telemetry`. API instabile, breaking change attesi prima della GA — vedi [11.3b](./11-plugins-marketplace.md#claude-mods-function-hooks-per-estendere-lengine-early-access-da-v21270). |
| 15 set 2026 | v2.1.272 | Bug fix e miglioramenti di affidabilita'. |
| 15 set 2026 | v2.1.273 | Header di richiesta gateway per monitoring LLM (`x-claude-code-request-class`, `x-claude-code-agent-type`). Forking di sessioni avviate con `--remote-control`. Fix: skill che restavano disponibili dopo la disattivazione della feature Skills a livello org, permesso Read su macOS che rifiutava screenshot trascinati, client Remote Control con permission mode non aggiornato. |
| 17 set 2026 | v2.1.274 | Warning visibile per uso critico della memoria con istruzioni per liberarla o riavviare in sicurezza. `CLAUDE_CODE_MCP_STARTUP_WAIT_MS` per il tempo di attesa di connessione ai server MCP. Fix: sessioni bloccate in retry infinito su errori "unexpected tool_use_id" (i transcript corrotti ora si auto-riparano), prompt/risorse MCP che non si aggiornavano alle notifiche list-changed dei server, `/goal` che perdeva contesto dopo la compaction. |

<sub>Aggiornato 2026-09-17 via daily what's new. Fonte: [@bcherny](https://x.com/bcherny/status/2099551291601248485) · [repo mods/](https://github.com/anthropics/claude-code/tree/main/mods) · [GitHub Releases v2.1.270](https://github.com/anthropics/claude-code/releases/tag/v2.1.270) · [v2.1.271](https://github.com/anthropics/claude-code/releases/tag/v2.1.271) · [v2.1.272](https://github.com/anthropics/claude-code/releases/tag/v2.1.272) · [v2.1.273](https://github.com/anthropics/claude-code/releases/tag/v2.1.273) · [v2.1.274](https://github.com/anthropics/claude-code/releases/tag/v2.1.274). Unica voce editoriale: Claude Mods, nuovo sistema di estensione Function Hooks (Early Access). Il resto (fix permessi git, fast mode Remote, `allowed_domains` sandboxing, auto-archiviazione VS Code, self-healing transcript, header gateway) resta sotto la soglia editoriale. Nessun annuncio Anthropic blog dedicato su Claude Code nelle ultime 24 ore.</sub>
