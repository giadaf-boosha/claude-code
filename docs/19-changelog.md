...
| 8 set 2026 | v2.1.265 | Telemetria `user.email`/`user.groups` per sessioni Desktop e Cowork via Claude apps gateway. `--plugin-dir` accetta ora anche una cartella che contiene piu' plugin (ogni sottocartella con manifest viene caricata come plugin separato). Limite di 1 GB sui risultati dei tool salvati su disco. Archiviazione automatica delle sessioni VS Code inattive (default 14 giorni). Fix: resume di subagent foreground che perdeva tool list/system prompt dalla prompt cache, `/model opusplan[1m]` rigettato per errore, MCP `http` legacy senza fallback a SSE, working directory non piu' persistente tra i turni nelle sessioni non interattive. |
| 8 set 2026 | — | **Blog Anthropic "Reducing cost and improving performance with Claude Platform"**: introduce due nuovi sotto-comandi della skill `/claude-api` — `hillclimb` (ottimizzazione iterativa di modello, effort e prompt su train/test set) e `prompt-audit` (rimozione di anti-pattern da prompt e tool description; +14,6% risparmio di costo e +5,3% accuratezza nei test citati) — vedi [03](./03-slash-commands.md#31-tabella-completa). |
| 8 set 2026 | v2.1.266 | Fix di una regressione introdotta in v2.1.265: `CLAUDE_CODE_USE_GATEWAY` non forza piu' da solo il login al Cloud gateway, ripristinando le configurazioni con API key, `apiKeyHelper` o header di autenticazione custom. |

<sub>Aggiornato 2026-09-09 via daily what's new. Fonte: [Anthropic blog](https://claude.com/blog/reducing-cost-and-improving-performance-with-claude-platform) · [GitHub Releases v2.1.263](https://github.com/anthropics/claude-code/releases/tag/v2.1.263) · [v2.1.265](https://github.com/anthropics/claude-code/releases/tag/v2.1.265) · [v2.1.266](https://github.com/anthropics/claude-code/releases/tag/v2.1.266). Unica voce editoriale: i due nuovi sotto-comandi `/claude-api hillclimb`/`prompt-audit` (annuncio blog Anthropic su Claude Code). Le release v2.1.263/265/266 restano sotto la soglia editoriale (solo fix e feature minori). Nessun post team dedicato su X nelle ultime 24 ore.</sub>
