...
| 23 set 2026 | **v2.1.281** | `"attribution": false` in `settings.json` per disattivare in un colpo solo l'attribuzione su commit e PR — vedi [18.2](./18-settings-auth.md#campi-principali-selezione). Hardening auto mode: prompt `rm` pericolosi in sessioni unattended attendono 2 minuti poi negano con suggerimento di riscrittura, comandi shell read-only/sandboxed aggiunti alla review server-side — vedi [4.3](./04-modalita-permessi.md#43-auto-mode-da-v2183-default-promaxteam-da-14-ago-2026). Supporto Claude apps gateway per `assume_role`/`guardrail` Bedrock e `telemetry.resource_attributes`, elicitation MCP in URL-mode, validazione server MCP in `claude plugin validate`, stima permission prompt auto mode in `/insights`, tasto "invia subito" che ora sposta i tool in esecuzione in background invece di cancellarli. Release enorme (oltre 200 voci) di fix su resume sessione, prompt cache, stream handling, Vim mode, VS Code, Claude Tag/Slack, Remote Control e performance di avvio. |

<sub>Aggiornato 2026-09-24 via daily what's new. Fonte: [GitHub Releases v2.1.281](https://github.com/anthropics/claude-code/releases/tag/v2.1.281). Due voci editoriali: `"attribution": false` (scorciatoia settings) e l'hardening di auto mode (timeout `rm` pericolosi + review server-side di comandi read-only/sandboxed). Il resto della release v2.1.281 (oltre 200 voci: gateway Bedrock, elicitation MCP, `claude plugin validate`, `/insights`, "invia subito" in background, fix vari) resta sotto la soglia editoriale. Nessun annuncio Anthropic blog dedicato a Claude Code ne' post team rilevante (Boris, Cat, Noah, Thariq, @claudeai, @ClaudeDevs, @alistaiir, @ClaudeCodeLog) nelle ultime 24-48 ore.</sub>
