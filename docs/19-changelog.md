...
| 4 ago 2026 | **v2.1.221** | **Focus view** su VS Code (`Ctrl+Alt+F`): nasconde l'attivita' dei tool dietro un riepilogo per turno con indicatore live del tool in esecuzione. Mascheramento credenziali sandbox su Linux/WSL (`mode: "mask"`): i comandi sandboxati leggono una copia sentinella, il proxy sostituisce il valore reale in uscita. Sessioni background ora fanno commit e push in autonomia per preservare il lavoro, aprendo draft PR solo quando il task lo richiede — vedi [8.6c](./08-subagents.md#86c-background-agents--auto-delivery-da-v21198). `/plugin install` aggiorna il catalogo marketplace stale prima di segnalare "not found". Fix: bypass permission-check Bash su condizionali regex zsh in `[[ ]]`, permission-check PowerShell su path con virgolette, yank register Vim mode che non sopravviveva a dialog/history search. |
