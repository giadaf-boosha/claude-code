...
Cronologia completa di Claude Code dalla research preview (24 febbraio 2025, v0.2.0) all'ultima versione (6 agosto 2026, v2.1.223). 7 fasi storiche + tabella versione per versione + post-mortem aprile 2026.
...
| 7 ago 2026 | **v2.1.224** | **`claude self-hosted-runner`** (Team/Enterprise): trasforma macchine o container proprietari in ambienti dove far girare sessioni Claude Code web, mobile e desktop. **Plugin source `archive`**: installa plugin da zip via HTTPS senza git/npm, pinning SHA-256 opzionale. **Cross-session `SendMessage`/`ListAgents`** (macOS/Linux): le sessioni si scambiano messaggi tra loro; `crossSessionInbound` trattiene per approvazione i messaggi verso sessioni a permessi bypassati. Sandbox credential masking JWT (`decode: "jwt"`) e AWS SigV4 (`awsPairs`/`sigv4`). Rimosso il tetto di 200 subagent per sessione (restano i limiti di concorrenza/profondita'). Fix sicurezza: regole `denyRead` con trailing slash bypassabili, dettagli di violazione sandbox ora visibili nei risultati Bash. |
