...
| 24 giu 2026 | v2.1.190 | Bug fixes e miglioramenti di affidabilita'. |
| 24 giu 2026 | **v2.1.191** | **`/rewind` + `/clear` resume**: `/rewind` supporta il resume da prima dell'ultimo `/clear`. **CPU streaming -37%** (coalescing 100ms); ridotto memory growth sessioni lunghe. Fix hook comma matchers (usare `|` non `,`); fix background agents stop permanente; **MCP retry logic**: capability discovery + OAuth retry su errori transitori + HTTP 404 mostra URL; fix `/permissions` Recently-denied persiste approvazioni; sandbox network dialog ricorda host permessi per sessione. Fix scroll streaming, `/voice` policy message, `/login` URL truncation Windows Terminal, Cmd+click Ghostty ssh/tmux. |
