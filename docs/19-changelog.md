...
| 11 ago 2026 | v2.1.228 | Fix: redraw delle sessioni interattive, discovery di git/Git Bash su Windows, regressione modello in `/tui`, messaggistica cross-session. Self-hosted runner salta repository non accessibili con warning invece di fallire. |
| 12 ago 2026 | v2.1.229 | Sorgente `command` per marketplace plugin con ri-risoluzione dinamica. SSE keepalive ping sullo streaming gateway. `ListAgents` marca come `offline` le sessioni Remote Control disconnesse. Fix: streaming di risposte lunghe che duplicava output, crash su tool call con valori non-stringa, OAuth MCP su `127.0.0.1` invece di `localhost`. |
| 13 ago 2026 | v2.1.231 | Fix: redirect URI OAuth MCP non combaciante per client pre-registrati (es. Slack) — bloccava il sign-in. |
| 13 ago 2026 | **v2.1.232** | **Subagent fork on by default**: un subagent `subagent_type: "fork"` eredita l'intera conversazione e la prompt cache; gli spawn non-teammate nelle sessioni interattive girano ora in background di default — vedi [08 § 8.5](./08-subagents.md#85-forking-di-sessione). **`@`-mention cross-session**: digitare `@` nel prompt referenzia un'altra sessione Claude per nome, instradato via `SendMessage` — vedi [17.6](./17-ide-surface.md#remote-control). **GitLab nei plugin marketplace**: URL `gitlab.com` (incluse subgroup annidate) si clonano come `github.com`; redazione token GitLab (`glrt-`, `gloas-`, `glptt-`, ecc.) — vedi [11](./11-plugins-marketplace.md). Fix sicurezza: bypass permessi PowerShell via `$PSDefaultParameterValues`, symlink Cygwin su Windows che aggiravano la validazione path, repo git annidati che ereditavano il trust del genitore, connessioni MCP che restavano appese 30s su risposta malformata. |

<sub>Aggiornato 2026-08-14 via daily what's new. Fonte: [GitHub Releases v2.1.232](https://github.com/anthropics/claude-code/releases/tag/v2.1.232) · [v2.1.231](https://github.com/anthropics/claude-code/releases/tag/v2.1.231) · [v2.1.229](https://github.com/anthropics/claude-code/releases/tag/v2.1.229) · [v2.1.228](https://github.com/anthropics/claude-code/releases/tag/v2.1.228) · [changelog ufficiale](https://code.claude.com/docs/en/changelog).</sub>
