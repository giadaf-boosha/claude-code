...
| 5 set 2026 | v2.1.262 | Bug fix e miglioramenti di affidabilita'. |
| 6 set 2026 | v2.1.263 | Bug fix e miglioramenti di affidabilita'. |
| 7 set 2026 | v2.1.264 | Regole permessi Bash estese a valori di opzione e operandi file. Fix: reset del workspace trust in sessioni concorrenti, loop di rifiuto del thinking sui turni successivi, invalidazione prompt cache al refresh OAuth, rendering del transcript fullscreen con molte tool call, resume di sessioni in background/Remote Control/MCP. |
| 8 set 2026 | v2.1.265 | Cap di 1 GB sui risultati dei tool salvati su disco (con indicatore di troncamento). Supporto `--plugin-dir` su cartelle con piu' plugin (ogni sottocartella con manifest viene caricata). Telemetria email/gruppi utente estesa a Claude Desktop e Cowork via apps gateway. Fix: prompt-cache rotta al resume di subagent in foreground, denial di accesso al modello messo in cache in modo scorretto, errori di publish degli artifact. |
| 8 set 2026 | v2.1.266 | Fix critico: la variabile `CLAUDE_CODE_USE_GATEWAY` forzava il sign-in Cloud-gateway anche con API key o auth custom configurate, rompendo i setup LLM-gateway/proxy introdotti in v2.1.265. |
| 9 set 2026 | v2.1.267 | Setting `maxEffortLevel` per limitare l'effort massimo su tutti i provider (Bedrock, Vertex, Foundry). Flag `--system-prompt-snapshot off` per rendering fresco del system prompt ad ogni richiesta. Fix: task schedulati Cowork che fallivano all'avvio per requisiti di sandboxing, output di `/context` e comandi locali vuoto su mobile, bypass di sicurezza nei marketplace plugin. |
| 10 set 2026 | v2.1.268 | Sincronizzazione pricing gateway (`pricing:` in `gateway.yaml`) cosi' i client autenticati vedono le stesse tariffe dell'org. Nuovo setting `gatewayInternalNetworks` per allowlist IP org. `claude self-hosted-runner --remove-session-state` per ripulire lo stato di sessioni self-hosted. Output JSON per i comandi plugin. Fix: errori HTTP 400 su endpoint compatibili di terze parti, timeout WebFetch ora a 300s, regole permessi su directory symlinkate. |

<sub>Aggiornato 2026-09-11 via daily what's new. Fonte: [GitHub CHANGELOG.md](https://github.com/anthropics/claude-code/blob/main/CHANGELOG.md) · [code.claude.com/docs/en/changelog](https://code.claude.com/docs/en/changelog) · [Anthropic blog](https://claude.com/blog/reducing-cost-and-improving-performance-with-claude-platform). Nessuna voce editoriale: le release v2.1.262-268 (5-10 set) sono quasi tutte fix/affidabilita' o settings enterprise (gateway pricing, `maxEffortLevel`, telemetria), sotto la soglia editoriale. L'annuncio Anthropic del periodo riguarda il Claude Platform/API in generale (ottimizzazione costi), non Claude Code specificamente. Nessun nuovo comando, tool o feature di sistema rilevata nelle fonti (X team Anthropic, changelog, GitHub releases, whats-new digest fermo a Week 34).</sub>
