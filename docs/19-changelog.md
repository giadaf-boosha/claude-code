...
| 1 lug 2026 | **v2.1.198** | **Claude in Chrome GA**; hook `Notification` (`agent_needs_input` / `agent_completed`) per sessioni `claude agents`; skill `/dataviz` bundled (design grafici + validatore palette); `anthropicAws` come provider upstream nel gateway (failover su model-not-found); background agents da `claude agents` auto-committano, pushano e aprono draft PR al completamento; Explore agent eredita modello sessione (non piu' haiku fisso); subagenti ereditano extended thinking config della sessione padre; retry con backoff su ECONNRESET mid-response. 25+ fix inclusi: `/diff` panel su cambio branch, tabelle markdown in fullscreen, STS token refresh AWS/Mantle, rete locale macOS per background agents. |
