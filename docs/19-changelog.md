...
Cronologia completa di Claude Code dalla research preview (24 febbraio 2025, v0.2.0) all'ultima versione (9 settembre 2026, v2.1.267). 7 fasi storiche + tabella versione per versione + post-mortem aprile 2026.
...
| 8 set 2026 | v2.1.265 | Aggiunti `user.email`/`user.groups` alla telemetria che Desktop e Cowork inviano via Claude apps gateway. `--plugin-dir` ora accetta anche una cartella di plugin (ogni sottocartella con manifest viene caricata, i cambi a runtime vengono rilevati). Cap di 1 GB sui risultati dei tool salvati su disco, con avviso di troncamento nell'anteprima. Ampi fix su prompt-cache, resume di subagent foreground, `/model opusplan`, MCP e UI. |
| 8 set 2026 | v2.1.266 | Fix critico di una regressione introdotta in v2.1.265: la env var non documentata `CLAUDE_CODE_USE_GATEWAY` forzava il sign-in al Cloud gateway anche con API key, `apiKeyHelper` o header di auth custom configurati, rompendo setup con LLM gateway/proxy. |
| 9 set 2026 | v2.1.267 | Nuovo setting `maxEffortLevel` per limitare l'effort level su tutti i provider, incluso Bedrock/Vertex/Foundry. Flag `--system-prompt-snapshot off` per rigenerare il system prompt ad ogni richiesta invece di riusare quello registrato in conversazione. Ampi fix: Cowork scheduled task falliti all'avvio per org con sandboxing obbligatorio, output `/context` vuoto su mobile, shortcut tastiera dopo reconnect tmux/ssh, retry eccessivi su credenziali AWS/GCP scadute, publish artifact interrotti da connessione persa, riuso prompt-cache con MCP e model switching. |

<sub>Aggiornato 2026-09-10 via daily what's new. Fonte: [GitHub Releases v2.1.263](https://github.com/anthropics/claude-code/releases/tag/v2.1.263) · [v2.1.265](https://github.com/anthropics/claude-code/releases/tag/v2.1.265) · [v2.1.266](https://github.com/anthropics/claude-code/releases/tag/v2.1.266) · [v2.1.267](https://github.com/anthropics/claude-code/releases/tag/v2.1.267) · [code.claude.com/docs/en/changelog](https://code.claude.com/docs/en/changelog). Nessuna voce editoriale: solo settings minori (`maxEffortLevel`, `--plugin-dir`, telemetria), un fix di regressione e fix di affidabilita', tutti sotto soglia. Nessun annuncio Anthropic blog dedicato su Claude Code nelle ultime 24 ore.</sub>
