...
Cronologia completa di Claude Code dalla research preview (24 febbraio 2025, v0.2.0) all'ultima versione (4 settembre 2026, v2.1.261). 7 fasi storiche + tabella versione per versione + post-mortem aprile 2026.
...
| 8 set 2026 | v2.1.265 | Telemetria utente/gruppo per sessioni gateway. `--plugin-dir` per cartelle di plugin con caricamento dinamico. Cap di 1 GB sui risultati dei tool salvati su disco. Fix: invalidazione della prompt cache su cambi di tool list/system prompt dei subagent, `/model opusplan[1m]` rifiutato erroneamente, elaborazione immagini ora affidata al supporto nativo del runtime. |
| 8 set 2026 | v2.1.266 | Fix critico: la regressione introdotta in v2.1.265 forzava il sign-in al Cloud gateway tramite `CLAUDE_CODE_USE_GATEWAY`, rompendo le configurazioni con API key dirette — ripristinato il comportamento precedente (variabile usata solo insieme ad altre credenziali gateway). |
| 9 set 2026 | v2.1.267 | Nuovo setting `maxEffortLevel` per limitare l'effort su tutti i provider. Flag `--system-prompt-snapshot off` per rigenerare il system prompt da zero ogni turno. Fix: task schedulati di Cowork che richiedevano sandboxing, invalidazione della prompt cache su cambi di tool/MCP a meta' sessione, cambio modello che ri-inviava tutte le definizioni dei tool. Migliorato il rendering del pannello `/diff`. |
| 10 set 2026 | v2.1.268 | Supporto pricing gateway allineato a `/cost`, nuovo setting org `gatewayInternalNetworks` per i blocchi IP gestiti. `--json` sui comandi di gestione plugin. Fix: errori HTTP 400 su endpoint compatibili Anthropic, `WebFetch` che restava appeso all'infinito (ora timeout di 300s), uso CPU elevato nelle sessioni idle. |
| 11 set 2026 | **v2.1.269** | **`claude plugin eval`**: esegue la eval suite di un plugin/skill con risultati riproducibili e report JSON+HTML, per misurare se una skill si attiva, sopravvive a un edit/nuovo modello e batte il modello nudo — vedi [11.1](./11-plugins-marketplace.md#111-struttura-di-un-plugin) e [9.1](./09-skills.md#91-bundled-skills-sempre-disponibili). **`/output-style [nome]`**: elenca e cambia output style; nuovo **`/focus`** (solo prompt, riassunto one-line, risposta) — vedi [3.1](./03-slash-commands.md#31-tabella-completa). Diff del tool Bash sui file modificati, tag repository OpenTelemetry. Fix: invalidazione parziale della prompt cache dopo tagli per limite di output-token, tasti F1/F2/F4 in kitty e Delete in st, regole di permesso con negazione `!` applicate oltre la sorgente prevista. Numerosi miglioramenti VS Code (agent map, dialog hook, dialog regole di permesso). |

<sub>Aggiornato 2026-09-12 via daily what's new. Fonte: [GitHub Releases v2.1.263](https://github.com/anthropics/claude-code/releases/tag/v2.1.263) · [v2.1.265](https://github.com/anthropics/claude-code/releases/tag/v2.1.265) · [v2.1.266](https://github.com/anthropics/claude-code/releases/tag/v2.1.266) · [v2.1.267](https://github.com/anthropics/claude-code/releases/tag/v2.1.267) · [v2.1.268](https://github.com/anthropics/claude-code/releases/tag/v2.1.268) · [v2.1.269](https://github.com/anthropics/claude-code/releases/tag/v2.1.269) · [MarkTechPost su plugin eval](https://www.marktechpost.com/2026/09/11/anthropic-adds-plugin-evals-to-claude-code-6-grader-types-a-no-plugin-baseline-and-a-ci-gate-for-skills/) · [code.claude.com/docs/en/changelog](https://code.claude.com/docs/en/changelog). Due voci editoriali: `claude plugin eval` (nuovo comando + CI gate skill) e i comandi `/output-style`/`/focus` (v2.1.269). Il resto (v2.1.263/265/266/267/268: telemetria gateway, `--plugin-dir`, fix critico gateway, `maxEffortLevel`, pricing gateway) resta sotto soglia. Nessun annuncio Anthropic blog dedicato nelle ultime 24 ore. Nota: `main` era fermo dal 2026-09-05 per PR daily non ancora mergiate (#86-#92, copertura 2026-09-05→09-11) — questo aggiornamento copre solo la finestra 24h richiesta (11→12 set), non l'intero gap accumulato.</sub>
