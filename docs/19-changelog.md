...
| 8 lug 2026 | **v2.1.205** | **Auto mode blocca tampering sui file di transcript** della sessione. **Notifiche background task dichiarano esplicitamente "nessun input umano ricevuto"**, per impedire che testo iniettato nel transcript venga letto come conferma/approvazione dell'utente. **`/doctor` diventa full setup checkup** (alias `/checkup`): oltre a diagnosticare versione, auth e connettivita', ora ripulisce skill/MCP/plugin inutilizzati, deduplica `CLAUDE.md` locale contro quello committato, spezza un `CLAUDE.md` root troppo grande in file annidati + skill, disabilita hook lenti, abilita auto mode di default e pre-approva comandi read-only spesso negati — con conferma prima di ogni modifica. Auto mode chiede conferma prima di `rm -rf` su variabili non risolvibili dal contesto. |
| 9-10 lug 2026 | v2.1.206 | `EnterWorktree` chiede conferma prima di entrare in un worktree fuori da `.claude/worktrees/`. `/commit-push-pr` auto-allow su `git push` verso il remote configurato del repo. `/cd` con path suggestions come `/add-dir`. Gateway: `/login` supporta endpoint gateway pubblici gestiti da Anthropic. Background agent si aggiornano in automatico subito dopo un update CLI. Miglioramenti qualita' `/code-review` su Opus 4.8. |
