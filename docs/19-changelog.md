...
Cronologia completa di Claude Code dalla research preview (24 febbraio 2025, v0.2.0) all'ultima versione (17 settembre 2026, v2.1.275). 7 fasi storiche + tabella versione per versione + post-mortem aprile 2026.
...
| ~25 giu 2026 | — | **Claude Tag** (beta Enterprise/Team): Claude Code reso multiplayer in Slack — si integra come team member condiviso, visibile a tutto il canale; async e proattivo. Gira su Opus 4.8. 65% del codice del product team Anthropic generato dalla versione interna. [@claudeai](https://x.com/claudeai/status/2069468694552461375) · [@ClaudeDevs](https://x.com/ClaudeDevs/status/2069468913264644419) · [Anthropic blog](https://www.anthropic.com/news/introducing-claude-tag). Vedi [12.17 Claude Tag](./12-agent-teams.md#1217-claude-tag). |
...
| 8 lug 2026 | **v2.1.205** | **`/checkup`**: nuovo alias di `/doctor`, diventa un setup checkup completo — diagnostica e propone fix per skill/MCP/plugin inutilizzati (per costo di context), deduplica `CLAUDE.md` locali contro le versioni committate, segnala hook lenti; riporta i risultati e chiede conferma prima di modificare. Auto mode: nuova regola blocca la manomissione dei file transcript di sessione. Notifiche task background dichiarano esplicitamente l'assenza di input umano (mitigazione approvazioni fabbricate nel transcript). Agent view: riga con stato colorato e headline scritta dal classifier al posto del raw tool call. Nome MCP server `Claude Browser` riservato lato client. |
...
| 14 lug 2026 | v2.1.210 | Contatore tempo trascorso live sulla riga riassuntiva dei tool collassati. Warning allo startup per regole di permesso `Write(path)`/`NotebookEdit(path)`/`Glob(path)` (usare `Edit`/`Read`). Fix: subagent con `isolation: 'worktree'` che eseguivano comandi git-mutating contro il repo principale, keyword `ultracode` innescata da input non-umano (webhook, commenti PR), crash sessione su risultati tool bigint/plain text, timeout hook callback riportato al modello come rifiuto utente. |
...
| 20 lug 2026 | v2.1.216 | `sandbox.filesystem.disabled`: nuova opzione salta l'isolamento filesystem mantenendo il controllo di egress network. Fix: rallentamento quadratico nelle sessioni lunghe (normalizzazione messaggi cresceva col numero di turni, causava stall e resume lenti), auto mode che negava comandi con errori classifier "HTTP 401" dopo rotazione/scadenza token OAuth mid-sessione. |
| 21 lug 2026 | v2.1.217 | Autocomplete emoji shortcode nel prompt (`:heart:` → ❤️, disattivabile con `emojiCompletionEnabled`). Warning quando la scrittura del transcript fallisce (es. disco pieno) invece di perdere silenziosamente la sessione. **Cap su subagent concorrenti** (default 20, `CLAUDE_CODE_MAX_CONCURRENT_SUBAGENTS`) e subagent che non spawnano piu' subagent annidati di default (`CLAUDE_CODE_MAX_SUBAGENT_SPAWN_DEPTH`). Fix: memory leak su output MCP troncati, auto-update Windows che poteva lasciare `claude.exe` mancante, isolamento sessioni background su working directory symlinkate, `--max-budget-usd` che non fermava i subagent background. |
| 22 lug 2026 | **v2.1.218** | **`/code-review` gira ora come subagent in background** — la review non riempie piu' la conversazione principale e mantiene come target gli stacked slash command. Fix: `/ultrareview` con argomenti descrittivi (es. "review my auth changes") ora lancia la review sul branch corrente usando il testo come nota ai findings; `/code-review ultra` non gira piu' silenziosamente in locale nelle sessioni non interattive. Miglioramenti accessibilita' (screen reader su cancellazioni di testo), sicurezza hook (richiedono trust della cartella dell'agent file) e numerosi fix di stabilita'. |
...
| 4 ago 2026 | **v2.1.221** | **Focus view VS Code**: toggle (`Ctrl+Alt+F`) che nasconde l'attivita' dei tool dietro un riassunto per-turno espandibile — vedi [17.1](./17-ide-surface.md#171-vs-code). Sandbox credential masking (`mode: "mask"`) su Linux/WSL: comandi sandboxati leggono una copia sentinella del file, il proxy sostituisce il valore reale in uscita. Background sessions ora committano/pushano sempre per preservare il lavoro, aprendo una draft PR solo se serve. Fix sicurezza: bypass permessi Bash via regex zsh in `[[ ]]`, gestione quote PowerShell su Windows. |
| 4 ago 2026 | **v2.1.222** | **Rimosso `/ultraplan`**: la feature di planning in cloud (lanciata v2.1.91, apr 2026) e' stata eliminata senza sostituto dedicato — vedi [15.1](./15-ultraplan-ultrareview.md#151-ultraplan--rimosso-da-v21222). Fix sicurezza: sessioni worktree-isolated potevano eseguire comandi git distruttivi contro il checkout principale; hook PreToolUse in background agent potevano bypassare le restrizioni sui tool. |
| 6 ago 2026 | **v2.1.223** | **`/review` torna alias di `/code-review`**: revisiona diff corrente o PR (`/code-review <level> <pr#>`), inverte la scelta single-pass di v2.1.202 — vedi [03](./03-slash-commands.md#31-tabella-completa). Wildcard `"owner/*"` per marketplace allow/blocklist. `/teleport` hint nelle sessioni cloud. Fix sicurezza: padding tab/Unicode nei permission prompt, `import()` dinamico nei workflow script fuori sandbox, `bypassPermissions` che ignorava policy org. |
| 7 ago 2026 | **v2.1.224** | **Ambienti self-hosted** (beta, Team/Enterprise): `claude self-hosted-runner` sposta le sessioni web/mobile/desktop dall'infrastruttura cloud Anthropic a macchine o container del team, in modalita' fixed o on-demand — vedi [13.7b](./13-routines-cloud.md#137b-self-hosted-runner-beta-da-v21224-7-ago-2026). **Cross-session messaging** (`SendMessage` + `ListAgents`): le sessioni Claude Code indipendenti possono scambiarsi messaggi tra loro, su una macchina, tra macchine o verso il web via Remote Control — vedi [17.6](./17-ide-surface.md#remote-control). Installazione plugin da archivio zip via HTTPS (senza git/npm, pinning SHA-256). Rimosso il tetto di 200 subagent per sessione. |
...
| 10 ago 2026 | v2.1.227 | Fix: feature flag valutati senza il tier di abbonamento su login token scaduto (falso prompt upgrade Fable per utenti Max), comandi Bash falliti sotto `claude-code-action` con `allowed_non_write_users` su GitHub-hosted runner, `/tui` che ripristinava conversazioni riportate indietro (`rewind`) a prima del primo messaggio. Menu slash command: solo la riga selezionata resta blu, i caratteri corrispondenti sono in bold invece che ricolorati. Riduzione stall dell'event loop su suggerimenti file-not-found e controlli dimensione at-mention. |
| 11 ago 2026 | v2.1.228 | Fix: sessioni interattive che smettevano di ridisegnare per un raro errore di layout, `git`/Git Bash non trovato su Windows se lanciato dalla cartella padre, `/tui` che tornava a un modello precedente, cross-session messaging senza inbox dopo install/upgrade, self-hosted runner che fallivano se l'hook `checkout` falliva su repo inutilizzati, cleanup sessione che cancellava contenuti nella memory folder del progetto. `Write` puo' ora sovrascrivere file senza lettura preventiva (come `Edit`). |
| 12 ago 2026 | v2.1.229 | Documentato `claude remote-control --continue` per riprendere sessioni recenti. Aggiunto supporto hook Claude Code lato server per sessioni self-hosted runner, keepalive SSE sul gateway streaming, sorgenti `command` nei plugin marketplace (ri-risolte a ogni sessione). `ListAgents` marca ora le sessioni Remote Control disconnesse come `offline` e quelle cloud come `cloud`. Fix vari su streaming, crash su tool call con valori non-string, MCP OAuth, Claude Code Review workflow. |
| 13 ago 2026 | v2.1.231 | Fix: MCP OAuth che falliva il sign-in per redirect URI mismatch su client OAuth pre-registrati (es. Slack). |
| 13 ago 2026 | **v2.1.232** | **Cross-session messaging con mention `@nome-sessione`**: basta digitare `@` nel prompt per raggiungere un'altra sessione, `SendMessage` recapita subito se il nome e' univoco — vedi [17.6](./17-ide-surface.md#remote-control). **Subagent forking on by default**: `subagent_type: "fork"` eredita conversazione e prompt cache complete, agent non-teammate in sessioni interattive girano ora in background di default — vedi [8.5](./08-subagents.md#85-forking-di-sessione). Aggiunto supporto GitLab nei plugin marketplace (URL bare `gitlab.com`, redazione token `glrt-`/`gloas-`/ecc.), alias settings `additionalMarketplaces`/`allowedMarketplaces`, Fable 5 di nuovo disponibile come advisor per le org con accesso. Fix sicurezza: bypass permessi PowerShell su parametri che scrivono variabili, bypass Windows via symlink Cygwin-style in Git Bash, trust ereditato da repo git annidati. |
| 14 ago 2026 | v2.1.233 | Estensione integrazione GitLab: URL merge request in `--worktree` e agents view, marketplace plugin, redazione token (`glrt-`, `gloas-`, `glptt-`, ecc.). Setting opt-in `forward_user_identity` per l'apps gateway. Supporto memory cgroup per comandi Bash su Linux. Fix: sessioni cloud segnate perse allo shutdown, connessioni MCP v2 che si riaprivano all'infinito, hook Notification per i permission prompt, sessioni idle al 100% CPU su Linux, alias skill bundled che segnalavano "Unknown command", bypass permessi PowerShell/Git Bash su Windows. |
| 17 ago 2026 | v2.1.234 | Continuazione automatica della sessione al reset dei limiti d'uso (configurabile da `/config`). Badge merge request GitLab in footer/statusline. Nuova env var `CLAUDE_CODE_PROJECT_DIR_NAME`, keybinding `selection:clear`. Hardening sicurezza: rifiutati i path Windows NT-namespace (`\??\`) su letture remote, session restore, include di CLAUDE.md. Skill `claude-api` ridotta da ~200k a ~25k token di contesto. Fix: risposte di permesso session-scoped perse nei prompt di subagent in background, crash su risposte API malformate, rallentamenti di rendering markdown con Unicode insoliti. |
| 18 ago 2026 | v2.1.235 | Setting opzionale `spellcheck` (aspell/hunspell/ispell). Fix: invalidazione prompt cache su disconnessione language server, allineamento liste markdown annidate in profondita', comportamento campo permission prompt con Shift+Tab. Migliorati uso memoria/CPU delle sessioni cloud in background e i dialoghi di permesso. |
| 19 ago 2026 | v2.1.236 | Nuova env var `ANTHROPIC_DEFAULT_MODEL` per fissare il modello di partenza delle nuove sessioni (il pick da `/model` continua a prevalere e persiste). `notify_when_idle` per `SendMessage` cross-session. Regole wildcard read-deny con precedenza nel sandbox macOS. Fix: fallback permanente del fullscreen renderer, rendering del model picker, vari problemi di cross-session messaging e Remote Control. |
| 20 ago 2026 | v2.1.237 | Nuovo output style built-in **"Concise"**: Claude va dritto ai risultati senza preamboli ne' narrazione, selezionabile da `/config` → Output style. Fix: prompt caching per sessioni con LLM gateway o base URL custom. |
| 20 ago 2026 | v2.1.238 | Setting `keybindingFlavor` per keybinding stile readline (cancellazione parola). `headersHelper` nei plugin marketplace per generare header HTTP dinamici su catalog/archive fetch. `claude self-hosted-runner --defer-shutdown-max-min` per shutdown graduale preservando le sessioni attive. Opzioni `--proxy-authorization-command`/`--proxy-authorization-file` per proxy di egress. Fix: memory leak non limitato nelle sessioni interattive lunghe, output style custom che tornavano al default a meta' sessione, vari fix MCP/permission/Remote Control. |
| 21 ago 2026 | v2.1.239 | Release ampia: le stime di costo includono ora il premium 1,1x per workspace data-residency US-only, `/claude-api upgrade` per migrare progetti Python dall'SDK `anthropic` 0.x a 1.x, plugin sincronizzati da claude.ai mostrati come `name@synced` nelle sessioni cloud, supporto Alpine/musl per paste immagini/clipboard/cattura audio native. Numerosi fix: streaming Bedrock dietro proxy, avvio bloccato con proxy HTTPS + profilo SSO, memoria di WebFetch su pagine scadute, resume di sessioni cloud fuori da plan mode dopo restart worker, form MCP, riconnessione server MCP remoti. |
| 22 ago 2026 | v2.1.240 | Bug fix e miglioramenti di affidabilita'. |
| 23 ago 2026 | v2.1.241 | Bug fix e miglioramenti di affidabilita'. |
...
| 26 ago 2026 | v2.1.247 | Tool `SendFeedback`: Claude prepara una bozza di feedback report da rivedere e inviare via `/feedback` (disattivabile con `feedbackDrafts`). `/claude-api cost-optimize` per profilare la spesa API di un progetto esistente e lavorare sulle leve di costo (caching, token hygiene, batch, effort, scelta modello). Auto-compact di Sonnet 5 esteso alla finestra 1M piena (~967K invece di ~934K token). Fix: sub-agent che morivano su un 404 al primo tentativo di modello, overflow della conversazione da output hook/agent molto grande, vari fix tastiera/terminale. |
...
| 2 set 2026 | v2.1.259 | Setting `managedMcpServers` per fornire server MCP HTTP/SSE gestiti dall'organizzazione a tutti gli utenti. Flag `--permission-prompts none` per operativita' unattended: nega automaticamente i prompt di permesso mantenendo attiva la modalita' impostata (utile su host headless). Fix: sessioni concorrenti che si sovrascrivevano a vicenda le modifiche a `~/.claude.json`, conversazioni rifiutate di nuovo dopo un primo rifiuto del thinking, invalidazione della prompt cache durante il refresh dei token OAuth. Rafforzata la regola "Containment Escape" in auto mode. |
| 3 set 2026 | v2.1.260 | **Diff panel in fullscreen mode**, apribile con `/diff`, mostra le modifiche non committate mentre Claude edita il codice — vedi [3.1](./03-slash-commands.md#31-tabella-completa). `/reload-plugins` disponibile in sessioni headless; `/advisor` accessibile in modalita' testuale su desktop e headless. Fix: pattern di permesso file con parentesi scartati come invalidi, revert della modifica alle regole `Read()` deny introdotta in v2.1.259 (era troppo restrittiva). Estesa la rilevazione di comandi `rm -rf` pericolosi. |
| 4 set 2026 | **v2.1.261** | **`/skill-doctor`**: nuovo comando diagnostico che individua le skill caricate ma mai invocate nella sessione e il loro costo di context — vedi [9.1](./09-skills.md#91-bundled-skills-sempre-disponibili). Nuovi settings `bashOutputMaxChars`/`taskOutputMaxChars` per alzare fino a 128K caratteri l'output inline di comandi prima che venga scritto su file. Nuovo flag `--append-subagent-system-prompt-file` per system prompt di subagent troppo lunghi per la CLI. `/status` mostra ora eventuali problemi di caricamento delle policy organizzative. Ampi fix su Remote Control (permission mode e spinner non aggiornati), gestione sessioni (tool call paralleli, preservazione del context al resume) e compatibilita' con proxy TLS-inspecting. |
| 6 set 2026 | v2.1.263 | Bug fix e miglioramenti di affidabilita'. |
| 8 set 2026 | v2.1.265 | Attributi telemetria `user.email`/`user.groups` per sessioni gateway. `--plugin-dir` accetta ora una cartella di piu' plugin (auto-caricati tutti i figli). Tetto di 1 GB sui risultati di tool salvati su disco. Fix: cache dei tool list/system prompt dei subagent al resume, parsing di `/model opusplan[1m]`, bypass di validazione sul path dei plugin via symlink, fallback trasporto MCP HTTP+SSE, permessi file su Windows AppContainer. |
| 8 set 2026 | v2.1.266 | Fix critico: revert della regressione 2.1.265 che forzava il sign-in gateway cloud (`CLAUDE_CODE_USE_GATEWAY`) anche con API key o header custom configurati. |
| 9 set 2026 | v2.1.267 | Setting `maxEffortLevel` per un tetto all'effort su tutti i provider (Bedrock, Vertex, Foundry). Flag `--system-prompt-snapshot off` per rendering fresco del system prompt. Fix: task schedulati Cowork che fallivano per requisiti di sandboxing, `/context` che appariva vuoto su mobile, retry su credenziali AWS/Google scadute. |
| 10 set 2026 | v2.1.268 | Pricing gateway per le Claude apps via `pricing:` in `gateway.yaml`, managed setting `gatewayInternalNetworks`. `claude self-hosted-runner --remove-session-state`. Fix: timeout WebFetch che restava bloccato all'infinito (ora fallisce dopo 300s, override con `CLAUDE_CODE_WEBFETCH_DEADLINE_MS`), CPU alta su sessioni idle lunghe, sessioni cloud che perdevano i file dopo 6 ore (ora persistono fino a 24h). |
| 11 set 2026 | **v2.1.269** | **`claude plugin eval`**: nuovo comando per eseguire la eval suite di un plugin con risultato riproducibile e punteggio (report JSON/HTML) — vedi [11.3](./11-plugins-marketplace.md#claude-plugin-eval-testare-un-plugin-con-risultati-riproducibili-da-v21269). **`/output-style [name]`** per elencare e cambiare output style anche da Remote Control e sessioni headless. Diff dei file modificati incluso nel risultato del tool Bash (`bashEditDiffEnabled`). `CLAUDE_CODE_WORKFLOW_MAX_CONCURRENT_AGENTS` (1-256) per il limite di agent concorrenti nei Dynamic Workflows. VS Code: agent map con card e transcript per-agente, dialog dedicati per hook e permission rules. |
| 12 set 2026 | v2.1.270 | Fix: comandi git read-only in Bash che chiedevano permesso inaspettatamente in sessioni aperte da tempo. |
| 14 set 2026 | **v2.1.271** | Fast mode esteso alle sessioni **Remote** (cloud e self-hosted runner): si applica l'impostazione fast mode dell'host, o `/fast` digitato in sessione dove l'organizzazione lo consente — vedi [13.7b](./13-routines-cloud.md#137b-self-hosted-runner-beta-da-v21224-7-ago-2026). Nuovo `allowed_domains` per-comando su Bash, PowerShell e Monitor in auto mode con sandboxing: solo gli host che servono a quel comando vengono aperti, gli altri sono rifiutati. Mouse support nel pannello `/config` in fullscreen. `claude self-hosted-runner --drain-marker-file <path>`. `omitClaudeMd` in agent frontmatter/`--agents` JSON per far girare subagent custom/plugin senza i CLAUDE.md di user/project/local. |
| 15 set 2026 | v2.1.272 | Bug fix e miglioramenti di affidabilita'. |
| 16 set 2026 | v2.1.273 | Sessioni **Remote Control** forkabili in sessioni locali in background, preservando lo stato e liberando la UI — vedi [17](./17-ide-surface.md#remote-control). Notifica quando un server MCP si disconnette a meta' sessione e la riconnessione fallisce, con diagnostica `/mcp`. |
| 17 set 2026 | v2.1.274 | Avviso visibile quando la memoria e' a livello critico, con passi per liberarla o riavviare in sicurezza. Fix: sessioni che entravano in loop su errori 400 "unexpected tool_use_id"; transcript che ora si autoripara o mostra un errore chiaro su `/rewind`. |
| 17 set 2026 | **v2.1.275** | **Sync skill/plugin da claude.ai al terminale**: skill e plugin abilitati sull'account claude.ai si sincronizzano nelle sessioni CLI loggate con lo stesso account, opt-out via `syncClaudeAiSkills: false` / `syncClaudeAiPlugins: false` — vedi [9.2b](./09-skills.md#92b-sync-skill-da-claudeai-al-terminale-da-v21275) e [11.4b](./11-plugins-marketplace.md#114b-sync-plugin-da-claudeai-al-terminale-da-v21275). Account collegato mostrato nel sign-in gateway Claude apps, con conferma prima di salvare le credenziali. Tasto "invia subito" (`ctrl+enter` / `ctrl+x ctrl+s`) per interrompere il turno corrente e inviare tutti i messaggi in coda. `CLAUDE_CODE_MCP_STARTUP_WAIT_MS` per limitare l'attesa dei server MCP al primo turno non interattivo. |
| 17 set 2026 | — | **Projects redesign**: nuova esperienza "Projects" in beta, annunciata sul blog Anthropic — una conversazione persistente in cui Claude fa da coordinatore, delega il lavoro a thread paralleli con memoria condivisa e una libreria comune di file/artifact, e assembla il risultato finale; steering possibile anche da telefono. Beta da oggi per un gruppo selezionato di utenti Pro/Max con sessioni cloud e senza progetti esistenti su web/desktop, rollout esteso nei prossimi giorni poi a Team/Enterprise — vedi [12.18](./12-agent-teams.md#1218-projects-redesign-da-cartella-a-conversazione-beta-17-set-2026). Fonte: [Anthropic blog](https://claude.com/blog/projects-redesigned). |
| 18 set 2026 | v2.1.276 | Fix critico: revert della regressione 2.1.275 che rompeva ogni richiesta con errore `400 ... Input tag 'advisor_20260301'` quando `ANTHROPIC_BASE_URL` punta a un proxy o gateway. |
| 18 set 2026 | **v2.1.277** | **Supporto nativo ad AGENTS.md**: nei progetti senza CLAUDE.md, Claude Code legge ora automaticamente AGENTS.md — lo standard cross-tool gia' usato in decine di migliaia di repo condivisi tra piu' agenti AI — al posto del vecchio workaround manuale via `@AGENTS.md`; file preferito configurabile da "Project instructions" in `/config`, non ancora su Bedrock/Vertex/Foundry — vedi [6.2](./06-claude-md-memory.md#agentsmd-interop-nativo-da-v21277). `CLAUDE_GATEWAY_PROXY_IS_EGRESS_BOUNDARY=1` per gateway Claude apps con egress via forward proxy; mappa opzionale `headers:` sugli upstream gateway per header statici verso il proxy. Notifiche di aggiornamento task in background quando un pannello come `/tasks` e' aperto. Release enorme: oltre 80 fix su sessioni, MCP, editing file, plugin, VS Code, Claude Code on the web e Claude Tag. |
| 19 set 2026 | v2.1.278 | Auto mode per utenti Claude API/Enterprise e su Bedrock/Vertex/Foundry/gateway passa di default al classificatore lato server, non piu' fatturato per l'overhead di classificazione (`CLAUDE_CODE_AUTO_MODE_SERVER=0` per l'opt-out su Bedrock/Vertex/Foundry/gateway); avviso in caso di fallback fatturato. Nuova riga "Auto mode server" in `/status`. |
| 22 set 2026 | **v2.1.280** | **Claude Opus 5.5** diventa il nuovo modello Opus di default in Claude Code — 1M context, $4/$20 per MTok (cache read $0.20/MTok, -60%), output oltre il 30% piu' veloce e il 40% piu' economico di Opus 5; su Pro e Team Standard il modello di default passa da Sonnet a Opus, allineandosi a Max/Premium/Enterprise — vedi [05.10](./05-fast-mode-1m-context.md#510-claude-opus-55-da-v21280). Aggiunto supporto mouse a piu' liste in fullscreen e `CLAUDE_CODE_MAX_MCP_DESCRIPTION_LENGTH` per il cap sulla lunghezza delle description dei tool MCP. Release enorme di fix su auto mode, dialog, MCP, subagent e sessioni Remote/cloud. |
| 23 set 2026 | v2.1.281 | Gateway Claude apps: `assume_role` e `guardrail: {id, version}` sugli upstream Bedrock (IAM STS, Amazon Bedrock guardrails), `telemetry.resource_attributes` per label fisse. Nuovo `"attribution": false` in `settings.json` per nascondere le righe di attribuzione su commit/PR. MCP URL-mode elicitation per flussi browser-based. Fix su crash nei retry di richieste API, turni che ignoravano `--max-turns`, sessioni riprese che reinviavano turni in forma alterata rompendo il prompt cache. |
| 24 set 2026 | v2.1.282 | `maxProseWidth` per limitare la larghezza del testo prosa nei terminali wide mantenendo tabelle e code block a larghezza piena. Avviso di avvio con elenco delle variabili di telemetria ignorate/disabilitate nei file di settings. `allowClaudeInChromeWithManagedMcp` per eseguire `claude --chrome` con `managed-mcp.json` esclusivo. Fix su errori 400 con risultati di web search non decifrabili, sessioni riprese che reinviavano messaggi precedenti in forma alterata, extended thinking scartato con slash command immediati. |
| 23 set 2026 | — | **Cloud sessions GA**: le sessioni cloud di Claude Code (ex "Claude Code on the web") escono dalla research preview e diventano generalmente disponibili per Pro, Max, Team e per Enterprise con seat premium o Chat + Claude Code — vedi [17.4](./17-ide-surface.md#174-web-enclaude-code-on-the-web). Fonte: [Claude blog](https://claude.com/blog/claude-code-on-the-web). |
| 25 set 2026 | v2.1.283 | Header di gateway hint (`x-claude-code-prompt-id`, opt-in via `CLAUDE_CODE_GATEWAY_HINT_HEADERS=1`) per raggruppare le richieste lato LLM gateway. Managed settings `availableModelsMatch: "exact"` (blocca i modelli non esplicitamente elencati) e `deniedModels` (nega modelli specifici anche se `availableModels` li permetterebbe). Subcomando `/doctor prompt-audit` (`/checkup prompt-audit`) che estende la diagnostica esistente per individuare pattern di prompting datati in CLAUDE.md, skill, agent e comandi. Blocco gateway `load_test_mode` per load testing senza richieste upstream reali, upstream `mantle` per l'endpoint Bedrock Mantle. Output dei tool (MCP, WebFetch, WebSearch) tracciati negli span OpenTelemetry con `OTEL_LOG_TOOL_CONTENT=1`. Fix: notifiche di progresso MCP scartate su tool lunghi, server MCP stdio rimasti attivi dopo chiusura sessione in fase di startup, sessioni SDK che perdevano tool call differite o risultati completati, dynamic workflow avviati durante un fallback di modello che restavano sul modello di fallback invece di ritentare quello configurato. |

<sub>Aggiornato 2026-09-26 via daily what's new. Fonte: [Claude blog](https://claude.com/blog/claude-code-on-the-web) · [GitHub Releases v2.1.283](https://github.com/anthropics/claude-code/releases/tag/v2.1.283). Unica voce editoriale: Cloud sessions GA (23 set), non ancora coperta nei giorni precedenti nonostante l'annuncio risalga a qualche giorno fa. La release v2.1.283 (25 set) resta sotto la soglia editoriale: settings enterprise/gateway, il subcomando `/doctor prompt-audit` (estensione di un comando esistente, non un comando nuovo) e fix vari. Nessun nuovo slash command, tool primitivo o altro annuncio di sistema nelle ultime 24-48 ore.</sub>

<sub>Aggiornato 2026-09-25 via daily what's new. Fonte: [GitHub Releases v2.1.281](https://github.com/anthropics/claude-code/releases/tag/v2.1.281) · [v2.1.282](https://github.com/anthropics/claude-code/releases/tag/v2.1.282). Nessuna voce editoriale: entrambe le release restano sotto la soglia (settings enterprise/gateway, `maxProseWidth`, fix su sessioni/resume/vim mode/prompt cache). I due post del blog Anthropic del 22 e 24 settembre su Opus 5.5 (costo per task, ottimizzazione per sessioni di coding lunghe) approfondiscono un modello gia' coperto il 23 settembre, senza annunciare feature nuove. Nessun nuovo slash command, tool primitivo o annuncio di sistema nelle ultime 24-48 ore.</sub>

<sub>Aggiornato 2026-09-23 via daily what's new. Fonte: [Anthropic](https://www.anthropic.com/claude-opus-5-5) · [GitHub Releases v2.1.280](https://github.com/anthropics/claude-code/releases/tag/v2.1.280). Unica voce editoriale: Claude Opus 5.5 come nuovo modello Opus di default in Claude Code, con Pro e Team Standard che passano da Sonnet a Opus. Il resto della release v2.1.280 (mouse support, `CLAUDE_CODE_MAX_MCP_DESCRIPTION_LENGTH`, oltre un centinaio di fix) resta sotto la soglia editoriale. Nessun altro annuncio Anthropic blog dedicato a Claude Code ne' post team rilevante oltre l'hype di lancio Opus 5.5 nelle ultime 24-48 ore.</sub>

<sub>Aggiornato 2026-09-20 via daily what's new. Fonte: [GitHub Releases v2.1.277](https://github.com/anthropics/claude-code/releases/tag/v2.1.277) · [v2.1.276](https://github.com/anthropics/claude-code/releases/tag/v2.1.276) · [v2.1.278](https://github.com/anthropics/claude-code/releases/tag/v2.1.278). Unica voce editoriale: supporto nativo ad AGENTS.md (v2.1.277), che rende obsoleto il vecchio workaround documentato in [docs/06-claude-md-memory.md](./06-claude-md-memory.md). Il resto (fix regressione gateway in v2.1.276, classificatore auto mode lato server in v2.1.278) resta sotto la soglia editoriale. Nessun annuncio Anthropic blog dedicato su Claude Code nelle ultime 24-48 ore.</sub>

<sub>Aggiornato 2026-09-18 via daily what's new. Fonte: [Anthropic blog](https://claude.com/blog/projects-redesigned) · changelog mirror community [marckrenn/claude-code-changelog](https://github.com/marckrenn/claude-code-changelog/releases) per i dettagli v2.1.270-275. Due voci editoriali: **Projects redesign** (annuncio Anthropic blog, nuova feature di sistema — coordinatore + thread paralleli con memoria condivisa) e **sync skill/plugin claude.ai → terminale** (v2.1.275, feature di sistema cross-surface). Il resto delle release v2.1.270-274 (fast mode su Remote, `allowed_domains` per-comando, fork Remote Control → locale, diagnostica MCP, avviso memoria critica, fix vari) resta sotto la soglia editoriale. Nota fonti: `code.claude.com/docs/en/changelog` e `github.com/anthropics/claude-code/releases` hanno risposto 503 per l'intera finestra di ricerca odierna; i dettagli versione per versione sono incrociati su piu' aggregatori indipendenti in assenza dell'accesso diretto, l'annuncio Projects e' invece confermato direttamente dal blog Anthropic e da piu' testate tech indipendenti (VentureBeat, MarkTechPost, XenoSpectrum).</sub>

<sub>Aggiornato 2026-09-12 via daily what's new. Fonte: [GitHub Releases v2.1.265](https://github.com/anthropics/claude-code/releases/tag/v2.1.265) · [v2.1.266](https://github.com/anthropics/claude-code/releases/tag/v2.1.266) · [v2.1.267](https://github.com/anthropics/claude-code/releases/tag/v2.1.267) · [v2.1.268](https://github.com/anthropics/claude-code/releases/tag/v2.1.268) · [v2.1.269](https://github.com/anthropics/claude-code/releases/tag/v2.1.269) · [@ClaudeDevs](https://x.com/ClaudeDevs/status/2093742321473065266). Due voci editoriali: `claude plugin eval` (nuovo tool/comando) e il cambio dei limiti settimanali dal 14 set (+25% permanente che sostituisce il +50% temporaneo, netto -17% per chi ha il bonus). Il resto (gateway pricing, `maxEffortLevel`, `/output-style`, agent map VS Code, fix vari) resta sotto la soglia editoriale. Nessun annuncio Anthropic blog dedicato nelle ultime 24 ore; digest ufficiale settimanale fermo alla Week 34 (17-21 ago).</sub>

<sub>Aggiornato 2026-09-05 via daily what's new. Fonte: [@claudeai](https://x.com/claudeai/status/2095226833293685100) · [GitHub Releases v2.1.259](https://github.com/anthropics/claude-code/releases/tag/v2.1.259) · [v2.1.260](https://github.com/anthropics/claude-code/releases/tag/v2.1.260) · [v2.1.261](https://github.com/anthropics/claude-code/releases/tag/v2.1.261) · [code.claude.com/docs/en/changelog](https://code.claude.com/docs/en/changelog). Due voci editoriali: computer use in background (Cowork + Code, feature di sistema) e `/skill-doctor` (nuovo comando). Il resto (managedMcpServers, diff panel fullscreen, settings di output, fix minori) resta sotto soglia. Nessun annuncio Anthropic blog dedicato nelle ultime 24 ore.</sub>
...
<sub>Aggiornato 2026-08-28 via daily what's new. Fonte: [GitHub Releases v2.1.247](https://github.com/anthropics/claude-code/releases/tag/v2.1.247) · [v2.1.248](https://github.com/anthropics/claude-code/releases/tag/v2.1.248) · [v2.1.250](https://github.com/anthropics/claude-code/releases/tag/v2.1.250) · [CHANGELOG.md](https://github.com/anthropics/claude-code/blob/main/CHANGELOG.md). Nessuna voce editoriale: `--restricted` e le altre novita' restano feature/flag opt-in sotto soglia, nessun annuncio Anthropic dedicato.</sub>
...
<sub>Aggiornato 2026-08-24 via daily what's new. Fonte: [GitHub Releases v2.1.235](https://github.com/anthropics/claude-code/releases/tag/v2.1.235) · [v2.1.236](https://github.com/anthropics/claude-code/releases/tag/v2.1.236) · [v2.1.237](https://github.com/anthropics/claude-code/releases/tag/v2.1.237) · [v2.1.238](https://github.com/anthropics/claude-code/releases/tag/v2.1.238) · [v2.1.239](https://github.com/anthropics/claude-code/releases/tag/v2.1.239) · [v2.1.240](https://github.com/anthropics/claude-code/releases/tag/v2.1.240) · [v2.1.241](https://github.com/anthropics/claude-code/releases/tag/v2.1.241) · [CHANGELOG.md](https://github.com/anthropics/claude-code/blob/main/CHANGELOG.md). Nessuna voce editoriale: solo settings minori, output style e fix, sotto soglia.</sub>
<sub>Aggiornato 2026-08-18 via daily what's new. Fonte: [GitHub Releases v2.1.233](https://github.com/anthropics/claude-code/releases/tag/v2.1.233) · [v2.1.234](https://github.com/anthropics/claude-code/releases/tag/v2.1.234). Nessuna voce editoriale: solo fix e feature minori, sotto soglia.</sub>
<sub>Aggiornato 2026-08-14 via daily what's new. Fonte: [GitHub Releases v2.1.228](https://github.com/anthropics/claude-code/releases/tag/v2.1.228) · [v2.1.229](https://github.com/anthropics/claude-code/releases/tag/v2.1.229) · [v2.1.231](https://github.com/anthropics/claude-code/releases/tag/v2.1.231) · [v2.1.232](https://github.com/anthropics/claude-code/releases/tag/v2.1.232).</sub>
<sub>Aggiornato 2026-08-11 via daily what's new. Fonte: [GitHub Releases v2.1.227](https://github.com/anthropics/claude-code/releases/tag/v2.1.227). Nessuna voce editoriale: solo bug fix/UI, sotto soglia. Nota: fetch di anthropic.com/news bloccato dal proxy di rete, ricerca completata via WebSearch/changelog/GitHub releases/whats-new digest sulle altre fonti.</sub>
...
<sub>Aggiornato 2026-07-26 via daily what's new. Fonte: [GitHub Releases v2.1.216](https://github.com/anthropics/claude-code/releases/tag/v2.1.216) · [v2.1.217](https://github.com/anthropics/claude-code/releases/tag/v2.1.217) · [v2.1.218](https://github.com/anthropics/claude-code/releases/tag/v2.1.218) · [v2.1.219](https://github.com/anthropics/claude-code/releases/tag/v2.1.219) · [v2.1.220](https://github.com/anthropics/claude-code/releases/tag/v2.1.220) · [Anthropic news](https://www.anthropic.com/news/claude-opus-5).</sub>
