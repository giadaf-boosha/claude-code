...
| 12 set 2026 | v2.1.270 | Fix: regressione che faceva chiedere permesso ai comandi Bash git read-only dopo sessioni lunghe (introdotta in v2.1.269). |
| 14 set 2026 | **v2.1.271** | **Fast mode nelle sessioni Claude Code Remote** (cloud e self-hosted runner) — vedi [5](./05-fast-mode-1m-context.md). **`allowed_domains` per-comando** su Bash, PowerShell e Monitor in auto mode con sandboxing — vedi [4.3](./04-modalita-permessi.md#43-auto-mode-da-v2183-default-promaxteam-da-14-ago-2026). **`omitClaudeMd`** in agent frontmatter e `--agents` JSON per subagent che non devono ereditare CLAUDE.md — vedi [8](./08-subagents.md). Mouse support nel pannello `/config` fullscreen. `--accept-command <sha256>` per `claude plugin install/update`. Supporto `multiplier` fino a 10x in `modelPricing`. Ampi fix: policy organizzativa cache non invalidata dopo cambio account, sessioni cloud che rifiutavano tool call di subagent per errori di schema, `/fast off`, permessi Bash su wildcard, `.git/config.lock` stantio, MCP OAuth, tool search su tool MCP, messaggi cross-session senza traccia. |
...
<sub>Aggiornato 2026-09-15 via daily what's new. Fonte: [changelog](https://code.claude.com/docs/en/changelog) · [GitHub Releases v2.1.270](https://github.com/anthropics/claude-code/releases/tag/v2.1.270) · [v2.1.271](https://github.com/anthropics/claude-code/releases/tag/v2.1.271) · [v2.1.272](https://github.com/anthropics/claude-code/releases/tag/v2.1.272) · [Anthropic blog](https://claude.com/blog/agentic-coding-is-straining-ci-heres-how-we-scaled-test-impact-analysis-at-anthropic). Voce editoriale principale: v2.1.271 (fast mode Remote, sandboxing auto mode, omitClaudeMd). Il resto (mouse support `/config`, `--accept-command`, `modelPricing` multiplier, v2.1.270/272) resta sotto soglia.</sub>
