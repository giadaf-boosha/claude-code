...
| 4 ago 2026 | v2.1.222 | Fix sicurezza: sessioni/subagent isolati in worktree potevano eseguire comandi git distruttivi contro il checkout principale (isolamento ora copre file edit e Bash in ogni tipo di sessione); hook `PreToolUse` auto-allow non piu' aggirabile nei task background. **Rimossa la feature Ultraplan** (era research preview) — vedi [15](./15-ultraplan-ultrareview.md). Fix: blocco duplicati `/usage-credits` per membri Team/Enterprise, connectivity check bloccante dietro proxy HTTPS, model alias org-restricted (es. `model: opus`) che ricadevano sul parent invece che sul piu' recente consentito. `/diff`, Remote Control diff e diff web session ora su raw git blob content. |
| 6 ago 2026 | **v2.1.223** | Wildcard `"owner/*"` in `strictKnownMarketplaces`/`blockedMarketplaces` per marketplace plugin org-wide. Hint `/teleport <session id>` nelle sessioni cloud per continuare in locale. Fix sicurezza: bypass dei permission check su comandi Bash crafted e su padding con tab/Unicode invisibile nei prompt di conferma; workflow script non piu' in grado di uscire dal sandbox via `import()` dinamico. **`/review` diventa alias di `/code-review`** (riusa l'ultimo effort level digitato). `CLAUDE_CODE_DISABLE_1M_CONTEXT` copre ora ogni modello con finestra nativa 1M, non solo una lista fissa. |
