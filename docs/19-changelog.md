...
Cronologia completa di Claude Code dalla research preview (24 febbraio 2025, v0.2.0) all'ultima versione (19 settembre 2026, v2.1.278). 7 fasi storiche + tabella versione per versione + post-mortem aprile 2026.
...
| 18 set 2026 | v2.1.276 | Fix critico: regressione v2.1.275 che faceva fallire ogni richiesta con errore `400 ... Input tag 'advisor_20260301'` quando `ANTHROPIC_BASE_URL` punta a un proxy o gateway. |
| 18 set 2026 | **v2.1.277** | **AGENTS.md come alternativa a CLAUDE.md**: quando un progetto non ha un CLAUDE.md, Claude Code legge AGENTS.md al suo posto, preferenza configurabile in `/config` sotto "Project instructions" (non ancora su Bedrock, Vertex, Foundry) — vedi [6.2](./06-claude-md-memory.md#62-claudemd-locations-e-precedence). `CLAUDE_GATEWAY_PROXY_IS_EGRESS_BOUNDARY=1` per gateway Claude apps con unico egress via forward proxy (hostname passato al proxy invece che risolto localmente), header statici opzionali sugli upstream gateway. Oltre 70 fix: hang di `claude -p`/Agent SDK dopo errori interni (ora riportano l'errore ed escono con codice 1), errori "text content blocks must be non-empty", logout inatteso con build piu' vecchie sulla stessa macchina, avvio sessione piu' rapido per SDK/headless (niente piu' attesa sul lookup CLAUDE.md per-directory al primo turno), fix reinstallazione plugin e crash di sessioni background. |
| 19 set 2026 | **v2.1.278** | **Auto mode: classifier lato server di default per API/Enterprise e provider cloud**: per utenti Claude API ed Enterprise, e su Bedrock/Vertex/Foundry/gateway, l'auto mode passa di default al classifier lato server (non fattura piu' l'overhead di classificazione) — vedi [4.3](./04-modalita-permessi.md#43-auto-mode-da-v2183-default-promaxteam-da-14-ago-2026). Opt-out su Bedrock/Vertex/Foundry/gateway con `CLAUDE_CODE_AUTO_MODE_SERVER=0` (warning se si ricade sul fallback fatturato). Nuova riga "Auto mode server" in `/status`. |

<sub>Aggiornato 2026-09-19 via daily what's new. Fonte: [GitHub Releases v2.1.276](https://github.com/anthropics/claude-code/releases/tag/v2.1.276) · [v2.1.277](https://github.com/anthropics/claude-code/releases/tag/v2.1.277) · [v2.1.278](https://github.com/anthropics/claude-code/releases/tag/v2.1.278) · [code.claude.com/docs/en/changelog](https://code.claude.com/docs/en/changelog) (corrisponde a GitHub Releases). Due voci editoriali: AGENTS.md come alternativa a CLAUDE.md (v2.1.277, nuova feature di sistema — Project instructions) e default del classifier lato server per l'auto mode su API/Enterprise/Bedrock/Vertex/Foundry/gateway (v2.1.278, cambio di billing/comportamento). Il fix critico v2.1.276 (regressione proxy/gateway della v2.1.275) resta sotto la soglia editoriale. `code.claude.com/docs/en/whats-new` e' un digest settimanale fermo alla Week 37 (7-11 set), non copre ancora questa finestra. Nessun annuncio Anthropic blog dedicato su Claude Code nelle ultime 24 ore (`claude.com/blog` controllato, ultimi post rilevanti del 17 set gia' coperti; `anthropic.com/news` non raggiungibile dal proxy di rete). Nessun post nuovo confermato negli account team tracciati (@bcherny, @_catwu, @noahzweben, @trq212, @claudeai, @ClaudeDevs, @alistaiir, @ClaudeCodeLog) nella finestra 18-19 settembre, verificato via WebSearch (non esaustivo, x.com non fetchabile direttamente).</sub>
