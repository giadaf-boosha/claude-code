...
Cronologia completa di Claude Code dalla research preview (24 febbraio 2025, v0.2.0) all'ultima versione (28 agosto 2026, v2.1.250). 7 fasi storiche + tabella versione per versione + post-mortem aprile 2026.
...
| 28 ago 2026 | **v2.1.251** | **Hook `PreModelSwitch`/`PostModelSwitch`**: bloccano, confermano o annotano un cambio modello; `SessionStart` di resume riceve staleness sessione e costo stimato di re-cache. **Live streaming dei tool call** di un subagent in foreground verso i client Remote Control. **Spend limit bar** in `/usage` + campo status line `rate_limits.spend_limit` (Claude apps gateway). **Metriche prompt-cache per sessione** in `/cost` (hit ratio, miss, warm/cold) e oggetto `prompt_cache` per script status line. `CLAUDE_CODE_SUBAGENT_MODEL` diventa default (non piu' override assoluto): `model:` di agent definition e modello esplicito allo spawn prevalgono. Modello default Enterprise seat-based passa a Opus 5. `/effort` ora salva un default per-modello. Fix sicurezza: Read/Write/Edit non seguono piu' un symlink scambiato dopo il controllo permessi, path traversal nei comandi plugin dichiarati in marketplace bloccato, Grep/Glob applicano le regole deny anche su symlink; settings gestiti che toccano TLS/proxy/credenziali della sandbox richiedono ora approvazione esplicita. Fix: errore "text content blocks must be non-empty" dopo turni solo-thinking, Opus 5 con effort xhigh/max e thinking disattivato (ora scende a `high` invece di fallire). |

<sub>Aggiornato 2026-08-29 via daily what's new. Fonte: [GitHub Releases v2.1.251](https://github.com/anthropics/claude-code/releases/tag/v2.1.251) · [CHANGELOG.md](https://github.com/anthropics/claude-code/blob/main/CHANGELOG.md). Nessun annuncio Anthropic dedicato nelle ultime 24 ore; nessun post X datato oggi dai profili monitorati.</sub>
