...
| 26 giu 2026 | **v2.1.195** | **`CLAUDE_CODE_DISABLE_MOUSE_CLICKS`**: env var per disabilitare click/drag/hover del mouse in fullscreen mode mantenendo lo scroll a rotella. **Fix hook matchers con trattino** (es. `code-reviewer`, `mcp__brave-search`): ora fanno exact-match invece di substring-match — usa `mcp__brave-search__.*` per matchare tutti i tool di un server MCP con trattino. Fix voice dictation macOS su sessioni lunghe e su lingue senza spazi (giapponese, cinese, thai). Fix plugin esterni non richiedevano consenso install; fix `/plugin` Enable/Disable quando il `name` in `plugin.json` differisce dal marketplace. Fix: background jobs scomparsi da `claude agents` su versione piu' nuova; blank screen 5s su task crash-restart; daemon background irraggiungibili quando il control socket non parte. Miglioramento `claude agents` completed list (usa spazio verticale disponibile) e Remote session startup con checklist di provisioning. |
| 27 giu 2026 | — | **Mythos 5 parzialmente ripristinato** (export control parziale revocato): il Dipartimento del Commercio US (lettera del Segretario Lutnick ad Anthropic) revoca parzialmente il blocco export su Claude Mythos 5 in vigore dal 12 giugno — il modello torna accessibile a ~100 istituzioni US di infrastruttura critica (inclusi i loro dipendenti stranieri). Fable 5 resta bloccato; nessun impatto su account standard e utenti non-US. [Fortune](https://fortune.com/2026/06/27/anthropic-mythos-5-ai-model-us-commerce-department-clearance-fable/) · [Semafor](https://www.semafor.com/article/06/27/2026/us-releases-powerful-anthropic-model-mythos-to-some-us-companies) |

<sub>Aggiornato 2026-06-28 via daily what's new. Fonte: [GitHub Releases v2.1.195](https://github.com/anthropics/claude-code/releases/tag/v2.1.195).</sub>
