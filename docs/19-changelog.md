...
| 4 ago 2026 | **v2.1.221** | **Focus view (VS Code)**: toggle nel chat-menu (`Ctrl+Alt+F`) che nasconde l'attivita' dei tool dietro un riassunto per-turno espandibile con indicatore live del tool in corso — vedi [17.1](./17-ide-surface.md#171-vs-code). Sandbox: nuovo `mode: "mask"` per i file di credenziali su Linux/WSL (i comandi sandboxati leggono una copia sentinel, il valore reale viene sostituito solo in uscita dal proxy) — vedi [4.4](./04-modalita-permessi.md#44-sandbox-mode-sandbox). Nuovo subcommand `prompt-audit` per verificare prompt e tool description scritti per modelli piu' vecchi. Le sessioni background ora fanno commit+push di default per preservare il lavoro, aprono una draft PR solo se il task lo richiede, e seguono le istruzioni git di `CLAUDE.md`. Fix di sicurezza: bypass del permission check su Bash con condizionali regex zsh in `[[ ]]`, gestione dei path con apici in PowerShell su Windows. |
