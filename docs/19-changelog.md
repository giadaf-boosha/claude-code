...
| 11 ago 2026 | v2.1.228 | Fix: sessioni interattive che smettevano di ridisegnarsi dopo un raro errore di layout interno (processo restava attivo), `git`/Git Bash non trovato su Windows se Claude Code e' lanciato da una cartella genitrice dell'installazione git, `/tui` che riportava la sessione a un modello precedente quando `/model` era stato cambiato dopo l'ultima risposta, cross-session messaging che a volte partiva senza inbox alla prima sessione dopo install/upgrade, `/resume` in Remote Control che poteva far trapelare titolo/cronologia della conversazione ripresa nella sessione connessa. |
| 12 ago 2026 | **v2.1.229** | **Plugin marketplace, sorgenti `command`**: un comando locale (es. l'IDE) stampa la directory del plugin, ri-risolta ad ogni sessione senza restart (`mode: "link"`) — vedi [11.3](./11-plugins-marketplace.md#113-comandi-plugin). **VS Code, gruppi di sessioni** nella sidebar — vedi [17.1](./17-ide-surface.md#171-vs-code). Self-hosted runner: hook Claude Code forniti dal server, come negli ambienti managed — vedi [13.7b](./13-routines-cloud.md#137b-self-hosted-runner-beta-da-v21224-7-ago-2026). Documentato `claude remote-control --continue`. SSE keepalive sullo streaming del gateway durante pause di thinking lunghe (anti idle-timeout su Vertex/Bedrock). `ListAgents` marca le sessioni Remote Control disconnesse come `offline` e le sessioni cloud come `cloud`. Fix: workflow "Claude Code Review" che non pubblicava piu' le review sulle PR, risposte lunghe che sparivano parzialmente e si stampavano due volte durante lo streaming, crash su valori non-stringa (`glob`/`file_path`/`command`) nelle tool call, RangeError su terminali molto stretti, path Windows estesi (`\\?\`)/UNC, auto mode rotto per chi disabilita l'attribution header, `/model` che rifiutava Sonnet/Opus 1M per abbonati claude.ai, OAuth MCP con authorization server che richiedono `127.0.0.1` invece di `localhost`. |

<sub>Aggiornato 2026-08-13 via daily what's new. Fonte: [GitHub Releases v2.1.229](https://github.com/anthropics/claude-code/releases/tag/v2.1.229) · [CHANGELOG.md](https://github.com/anthropics/claude-code/blob/main/CHANGELOG.md).</sub>
