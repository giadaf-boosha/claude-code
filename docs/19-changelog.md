...
Cronologia completa di Claude Code dalla research preview (24 febbraio 2025, v0.2.0) all'ultima versione (18 settembre 2026, v2.1.276). 7 fasi storiche + tabella versione per versione + post-mortem aprile 2026.
...
| 16 set 2026 | v2.1.273 | Nuovi attributi OpenTelemetry per il tracing (span `claude_code.llm_request`) e per le sorgenti dei managed settings risolti. Client MCP v2 come default su Bedrock, Vertex e Foundry. Rifiniture sull'applicazione delle regole di permesso. |
| 17 set 2026 | v2.1.274 | Avviso visibile quando la memoria e' a livello critico, con passi per liberarla o riavviare in sicurezza. `CLAUDE_CODE_MCP_STARTUP_WAIT_MS` per limitare l'attesa di connessione degli MCP server al primo turno non interattivo (`0` = nessuna attesa). Attributo `effort` sullo span OTel `claude_code.llm_request`. Fix: sessioni bloccate in retry infinito su errori 400 "unexpected tool_use_id" (i transcript corrotti ora si auto-riparano dove possibile), MCP server `http` legacy (HTTP+SSE) che non si connettevano se rispondevano 4xx alla prima richiesta, timeout a ~5 minuti sulle chiamate MCP Streamable HTTP anche con `timeout` per-server piu' alto. |
| 17 set 2026 | **v2.1.275** | **Tasto "send-now"** (`ctrl+enter` o `ctrl+x ctrl+s`): interrompe il turno corrente e invia subito tutti i messaggi in coda — vedi [3.1](./03-slash-commands.md#31-tabella-completa). **Sync di skill e plugin da claude.ai**: le skill e i plugin abilitati sull'account claude.ai si sincronizzano ora anche nelle sessioni terminal collegate allo stesso account, opt-out con `syncClaudeAiSkills: false` / `syncClaudeAiPlugins: false` — vedi [9.1](./09-skills.md#91-bundled-skills-sempre-disponibili). Nuovo `/plugin install <plugin> --marketplace <source>` che offre di aggiungere il marketplace prima di installare. Conferma dell'account al sign-in del gateway Claude apps, con avviso in avvio se `otelHeadersHelper` fallisce. Oltre 70 fix aggiuntivi su terminal, VS Code, Claude Code on the web, Claude Tag e Code Review (incluso un fix di sicurezza: log e messaggi plugin/marketplace non mostrano piu' password o token nelle URL git/ssh/marketplace). |
| 18 set 2026 | **v2.1.276** | Fix critico: corregge l'errore `400 ... Input tag 'advisor_20260301'` che bloccava ogni richiesta quando `ANTHROPIC_BASE_URL` punta a un proxy o gateway — regressione introdotta da v2.1.275. |

<sub>Aggiornato 2026-09-18 via daily what's new. Fonte: [GitHub Releases v2.1.273](https://github.com/anthropics/claude-code/releases/tag/v2.1.273) · [v2.1.274](https://github.com/anthropics/claude-code/releases/tag/v2.1.274) · [v2.1.275](https://github.com/anthropics/claude-code/releases/tag/v2.1.275) · [v2.1.276](https://github.com/anthropics/claude-code/releases/tag/v2.1.276) · [code.claude.com/docs/en/changelog](https://code.claude.com/docs/en/changelog) · [Anthropic blog](https://claude.com/blog/projects-redesigned) · [@ClaudeDevs](https://x.com/ClaudeDevs/status/2100633571543367691). Tre voci editoriali: il fix critico v2.1.276 (proxy/gateway), la release v2.1.275 (send-now key, sync skill/plugin da claude.ai) e l'annuncio "Projects redesigned" (feature cloud, non ancora un numero di versione CLI dedicato — vedi [13-routines-cloud.md](./13-routines-cloud.md)). Il resto (v2.1.273-274: telemetria OTel, client MCP v2, fix vari) resta sotto la soglia editoriale. Nota: date esatte di v2.1.270-272 non confermate dalle fonti consultate in questo run.</sub>
