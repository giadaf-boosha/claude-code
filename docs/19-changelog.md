...
| 20 lug 2026 | **v2.1.216** | **`sandbox.filesystem.disabled`**: disattiva l'isolamento filesystem del sandbox mantenendo il controllo dell'egress di rete. Fix rallentamento quadratico nella normalizzazione messaggi nelle sessioni lunghe (stall multi-secondo, resume lento). Fix: auto mode negava comandi con errori classifier "HTTP 401" dopo scadenza/rotazione token OAuth mid-sessione; `AskUserQuestion` non forzava piu' la continuazione quando la risposta free-text chiedeva di attendere/spiegare; sessioni background che tornavano all'agente di default dopo resume; worktree isolati con redirect git errato. |
| 21 lug 2026 | **v2.1.217** | **Autocomplete shortcode emoji** nel prompt input (`:heart:` → ❤️, disattivabile con `emojiCompletionEnabled`). **Tetto di concorrenza subagent** di default 20, override `CLAUDE_CODE_MAX_CONCURRENT_SUBAGENTS`; nesting subagent limitato di default. Avviso quando la scrittura del transcript fallisce (disco pieno o session saving disattivo). Fix: `--max-budget-usd` non fermava piu' i subagent in background; memory leak sugli output MCP troncati; auto-update Windows che rimuoveva `claude.exe`; auto-compact Opus 4.8 su Bedrock; impostazioni mTLS/TLS-verify/OAuth scope/proxy corporate ripristinate su Claude Desktop. |
...
<sub>Aggiornato 2026-07-22 via daily what's new. Fonte: [GitHub Releases v2.1.216](https://github.com/anthropics/claude-code/releases/tag/v2.1.216) · [v2.1.217](https://github.com/anthropics/claude-code/releases/tag/v2.1.217).</sub>
