...
| 19 ago 2026 | **v2.1.236** | Nuova env var `ANTHROPIC_DEFAULT_MODEL` (modello di default per le nuove sessioni, sovrascritta da scelte esplicite `/model`). `notify_when_idle` nel cross-session `SendMessage`: chiede a un'altra sessione di avvisare quando torna idle (macOS/Linux, opt-in, one-shot). Sandbox macOS: le regole wildcard read-deny (es. `**/.env`) hanno ora precedenza dentro le region consentite e non sono piu' aggirabili rinominando i file. |
| 20 ago 2026 | **v2.1.237** | **Output style integrato "Concise"**: Claude va dritto al risultato, saltando preamboli e narrazione, selezionabile da `/config` → Output style o `outputStyle: "Concise"` in settings.json — annunciato da [@ClaudeDevs](https://x.com/ClaudeDevs/status/2090245922685063634). Fix: prompt caching rotto per sessioni che usano un LLM gateway o una base URL custom. |
| 20 ago 2026 | **v2.1.238** | Nuovo setting `keybindingFlavor: "readline"` (Ctrl+W in stile Bash, cancella fino allo spazio precedente). `claude self-hosted-runner --defer-shutdown-max-min <minuti>`: su SIGTERM continua a servire le sessioni gia' agganciate prima di fermarsi — vedi [13.7b](./13-routines-cloud.md#137b-self-hosted-runner-beta-da-v21224-7-ago-2026). Supporto proxy authorization (`--proxy-authorization-command`/`--proxy-authorization-file`) per proxy di egress con header di autorizzazione a scadenza breve. Fix: memory leak nelle sessioni interattive lunghe, output style custom/plugin che tornava al default a meta' sessione. |

<sub>Aggiornato 2026-08-21 via daily what's new. Fonte: [changelog](https://code.claude.com/docs/en/changelog) · [GitHub Releases v2.1.236](https://github.com/anthropics/claude-code/releases/tag/v2.1.236) · [v2.1.237](https://github.com/anthropics/claude-code/releases/tag/v2.1.237) · [v2.1.238](https://github.com/anthropics/claude-code/releases/tag/v2.1.238) · [@ClaudeDevs](https://x.com/ClaudeDevs/status/2090245922685063634).</sub>
