...
| 19 giu 2026 | **v2.1.183** | **Sicurezza auto mode**: `git reset --hard`, `checkout -- .`, `clean -fd`, `stash drop` bloccati se non esplicitamente richiesti; `git commit --amend` bloccato se il commit non e' dell'agente nella sessione; `terraform/pulumi/cdk destroy` bloccati a meno che lo stack specifico non sia stato richiesto. **`attribution.sessionUrl`**: omette il link sessione claude.ai da commit e PR nelle sessioni web/Remote Control. **`/config --help`**: elenca shorthand key disponibili; Esc ora salva e chiude il pannello. Fix: thinking blocks silenziosi (riprova automatica), WebSearch vuoto in subagenti, cursor vim mode, TUI corruption Windows Terminal, skill duplicate nell'autocomplete, MCP stub auth esposti in headless, tmux teammate launch/background tasks/scheduled triggers. |
