...
| 8 lug 2026 | v2.1.205 | Auto mode: nuova regola blocca il tampering dei file di transcript della sessione; chiede conferma prima di `rm -rf` su variabili non risolvibili dal contesto. `/doctor` diventa checkup completo (diagnosi + fix); `/checkup` e' il suo alias. Agent view: righe con state word colorata e headline scritta dal classifier; sessioni che editano/mergiano/commentano una PR la collegano automaticamente. Auto-update binari in streaming su disco (-~400MB picco memoria). 15+ fix minori (Windows worktree/NTFS junction, `--json-schema` con schema invalido, `--max-turns` che perdeva messaggi, plugin LSP). |
| 8 lug 2026 | — | **VS Code 1.128 — Multi-chat sessions**: una sessione Claude Code puo' contenere piu' chat correlate (fork da un turno, peer chat in parallelo, subagent come transcript peer read-only). Fonte: [VS Code 1.128](https://code.visualstudio.com/updates/v1_128). Vedi [docs/17-ide-surface.md](./17-ide-surface.md#171-vs-code).

<sub>Aggiornato 2026-07-09 via daily what's new. Fonte: [GitHub Releases v2.1.205](https://github.com/anthropics/claude-code/releases/tag/v2.1.205) · [VS Code 1.128](https://code.visualstudio.com/updates/v1_128).</sub>
