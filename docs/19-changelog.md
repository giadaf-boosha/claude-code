...
Cronologia completa di Claude Code dalla research preview (24 febbraio 2025, v0.2.0) all'ultima versione (13 agosto 2026, v2.1.232). 7 fasi storiche + tabella versione per versione + post-mortem aprile 2026.
...
| 18 ago 2026 | v2.1.235 | Nuovo setting `spellcheck` (sottolinea parole errate nel prompt via aspell/hunspell/ispell). Fix: invalidazione prompt cache alla disconnessione/riconnessione di un language server, allineamento liste markdown annidate, highlight del prompt sfalsati su multi-riga, Shift+Tab nel campo commento del permission prompt che approvava per errore, task list espansa (ctrl+t) che ripartiva sempre chiusa. Migliorie memoria/CPU per sessioni cloud in background (`/ultrareview`, `/autofix-pr`) e ai permission dialog. |
| 19 ago 2026 | v2.1.236 | **`ANTHROPIC_DEFAULT_MODEL`** env var (imposta il modello di partenza, sovrascrivibile da `/model`). **`notify_when_idle`** su `SendMessage` cross-session: chiede a un'altra sessione locale una notifica quando torna idle. Sandbox macOS: regole wildcard read-deny (es. `**/.env`) ora prioritarie e non aggirabili con rename. `/goal` fa check-in automatico dopo 30 min di inattivita' su lavoro in background. Numerosi fix su fullscreen renderer, `/model` picker, `SendMessage`, Remote Control, self-hosted runner; screen reader support per il transcript in VS Code. |
| 20 ago 2026 | v2.1.237 | **Output style built-in "Concise"**: Claude parte dai risultati e salta preamboli/narrazione mantenendo lo stesso lavoro (attivabile da `/config` → Output style). Fix prompt caching per sessioni dietro LLM gateway o base URL custom. |
| 20 ago 2026 | v2.1.238 | **`keybindingFlavor: "readline"`**: Ctrl+W nel prompt cancella in stile Bash. Plugin marketplace: `headersHelper` per generare header HTTP (es. token) per i fetch dei catalog. `claude self-hosted-runner --defer-shutdown-max-min` e opzioni proxy authorization. Fix: memoria non limitata nelle sessioni interattive lunghe, output style custom che tornava alla voce di default a meta' sessione, numerosi fix Remote Control/MCP/cross-session messaging. |
| 21 ago 2026 | v2.1.239 | Cost estimate (`/cost`, statusline, `--max-budget-usd`) include ora il premio 1.1x US-only-inference per workspace data-residency. **`/claude-api upgrade`**: migra progetti Python da `anthropic` 0.x a 1.x. Cloud sessions: plugin sincronizzati da claude.ai mostrati come `name@synced`. Supporto Alpine/musl per paste immagini/clipboard/audio. Fix rilevanti: streaming Bedrock dietro proxy che raddoppiava silenziosamente le chiamate API fatturate, hang all'avvio dietro proxy HTTPS con Bedrock SSO, `/goal` check-in con backoff (30min/1h/2h), cross-session messaging ora disponibile anche su Windows. |
| 22 ago 2026 | v2.1.240 | Bug fix e miglioramenti di affidabilita' (nessun dettaglio pubblicato in release notes). |
| 23 ago 2026 | v2.1.241 | Bug fix e miglioramenti di affidabilita' (nessun dettaglio pubblicato in release notes). |

<sub>Aggiornato 2026-08-23 via daily what's new. Fonte: [GitHub Releases v2.1.235](https://github.com/anthropics/claude-code/releases/tag/v2.1.235) · [v2.1.236](https://github.com/anthropics/claude-code/releases/tag/v2.1.236) · [v2.1.237](https://github.com/anthropics/claude-code/releases/tag/v2.1.237) · [v2.1.238](https://github.com/anthropics/claude-code/releases/tag/v2.1.238) · [v2.1.239](https://github.com/anthropics/claude-code/releases/tag/v2.1.239) · [v2.1.240](https://github.com/anthropics/claude-code/releases/tag/v2.1.240) · [v2.1.241](https://github.com/anthropics/claude-code/releases/tag/v2.1.241). Nessuna voce editoriale nelle ultime 24h: v2.1.240/241 sono solo fix generici, sotto soglia.</sub>
