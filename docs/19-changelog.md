...
| 14 ago 2026 | **v2.1.233** | **Supporto GitLab merge request** in `--worktree` e nella vista `claude agents` (mostrate come `!N`) — vedi [2.5](./02-cli-installazione.md#worktree--tmux). **`CLAUDE_CODE_TOOL_MEMORY_LIMIT`**: limite opzionale di memoria cgroup per i comandi Bash su Linux, previene build che esauriscono la RAM. **Tool di task-tracking disattivati di default** su Opus 4.8+/Sonnet 5+ (`TaskCreate`/`TaskUpdate`/`TaskList`/`TodoWrite`); si riattivano con `CLAUDE_CODE_ENABLE_TODO_TOOLS=1` — vedi [12.6](./12-agent-teams.md#126-task-list). Aggiunto `forward_user_identity` opt-in per attribuzione utente sul gateway, `CLAUDE_CODE_WEBFETCH_CACHE_TTL_MS`. Fix: idle sessions Linux al 100% CPU con sandboxing attivo, alias skill bundled (`/checkup`, `/review`) "Unknown command" in modalita' `-p`, bypass Windows via prefisso path NT `\??\`, regressione Windows che bloccava l'auto mode su comandi Bash standard con redirect. |

<sub>Aggiornato 2026-08-15 via daily what's new. Fonte: [GitHub Releases v2.1.233](https://github.com/anthropics/claude-code/releases/tag/v2.1.233).</sub>
