...
| 14 lug 2026 | **v2.1.208** | **Screen reader mode** (`claude --ax-screen-reader` o setting): rendering plain-text opt-in per utenti screen reader. **`vimInsertModeRemaps`**: mappa sequenze a due tasti (es. `jj`) a Escape in modalita' vim insert. **`CLAUDE_CODE_PROCESS_WRAPPER`**: env var per requisiti di launcher aziendali attorno al processo CLI. Fix: Fast Mode restava disabilitato dopo cambio modello; risposte background perse su delivery fallita; reset del context window dopo auto-update CLI. |
| 14 lug 2026 | v2.1.209 | Fix dialog bloccati nelle sessioni background di `claude agents`; fix `/model` e dialog correlati non disponibili durante task in background. |
| 14 lug 2026 | v2.1.210 | Contatore tempo trascorso live nei tool summary collassati per operazioni lunghe. Startup alert sui parametri delle permission rule per `Write`/`NotebookEdit`/`Glob`. Fix subagent isolati che leggevano comandi dal repo del parent anziche' dal proprio worktree; fix trigger `ultracode` attivato erroneamente da input non umani (es. payload webhook); fix crash su renderer di tool result con tipi di dato inattesi; migliorati i messaggi Bash per comandi che superano il time limit e passano in background automaticamente. |
| 15 lug 2026 | **v2.1.211** | **`--forward-subagent-text`** (+ `CLAUDE_CODE_FORWARD_SUBAGENT_TEXT`): include testo e thinking dei subagent nello stream-json output. **Fix di sicurezza**: le permission preview inoltrate a canali chat ora neutralizzano caratteri bidirezionali, zero-width e look-alike quote — l'input di un tool non puo' piu' alterare visivamente il messaggio di approvazione. **Fix auto mode**: un hook `PreToolUse` con decisione `ask` su Bash non sandboxato ora blocca sempre con un prompt, l'auto mode non puo' piu' scavalcarlo. Fix: sessioni parallele disconnesse in blocco al wake-from-sleep con credential store condiviso; MCP server dei plugin non si riconnettevano dopo il risveglio di una sessione web idle; `/clear` non azzerava il contatore costo in statusline. |
| 17 lug 2026 | **v2.1.212** | **`/fork`**: clona la conversazione in una nuova sessione background dedicata (riga propria in `claude agents`) mantenendo il lavoro corrente in corso — il subagent in-session che lanciava prima diventa `/subtask`. **Limiti per sessione**: WebSearch (default 200, `CLAUDE_CODE_MAX_WEB_SEARCHES_PER_SESSION`) e spawn di subagenti (default 200, `CLAUDE_CODE_MAX_SUBAGENTS_PER_SESSION`, reset con `/clear`) contro loop runaway. **MCP auto-background**: tool call MCP oltre 2 minuti passano automaticamente in background (soglia configurabile via `CLAUDE_CODE_MCP_AUTO_BACKGROUND_MS`). **`/resume`** apre un picker di sessioni passate, incluse quelle cancellate dalla lista. **`claude auto-mode reset`**: ripristina la config auto mode di default (conferma richiesta, `--yes` per saltarla). **Fix di sicurezza**: plan mode eseguiva comandi Bash che modificano file (`touch`, `rm`) senza prompt di permesso ne' callback `canUseTool` SDK. |
