...
| 20 giu 2026 | v2.1.185 | Hint stream-stall aggiornato ("Waiting for API response · will retry in …"); retry trigger portato da 10s a 20s di silenzio. |
| 22 giu 2026 | **v2.1.186** | **`claude mcp login/logout <name>`** (auth MCP da CLI — flusso OAuth senza UI `/mcp`); **`respondToBashCommands`** (bash `!` → risposta auto Claude, configurabile); sezione "Skills" nel tab Installed di `/plugin`; **`teammateMode: "iterm2"`** per agent teams con split pane iTerm2; AWS credentials refresh in `/login`; `/review <pr>` allineato a `/code-review medium`; skill frontmatter kebab/snake/camelCase; memory management con compaction reminders; fix streaming post-wake; fix feature-gated tools bypass; fix dark-theme flash, mouse selection, session cost Enterprise/Team, agent teams effort inheritance. |
