...
| 12 set 2026 | v2.1.270 | Fix di una regressione introdotta in v2.1.269: i comandi Bash `git` di sola lettura (`git status`, `git log`, ecc.) smettevano di essere auto-approvati dopo che una sessione girava da un po' e tornavano a chiedere permesso. |
| 14 set 2026 | v2.1.271 | **Fast mode nelle sessioni Claude Code Remote** (cloud e self-hosted runner). Nuovo `allowed_domains` per-comando su Bash, PowerShell e Monitor in auto mode con sandboxing di rete. Supporto mouse nel pannello `/config` in fullscreen. |
| 15 set 2026 | v2.1.272 | Bug fix e miglioramenti di affidabilita'. |
| 15 set 2026 | v2.1.273 | Nuovi header richiesta per gateway LLM (`x-claude-code-request-class`, `x-claude-code-agent-type`, ecc., opt-in con `CLAUDE_CODE_GATEWAY_HINT_HEADERS=1`). Notifica quando un server MCP si disconnette a meta' sessione e la riconnessione automatica rinuncia. **Fork di una sessione avviata con `--remote-control`** dall'app Claude, eseguito come sessione background sul computer d'origine. Ampio set di fix su permessi Bash, sync skill, autenticazione Bedrock/Vertex/Foundry, Code Review e Claude Tag (Slack). |

<sub>Aggiornato 2026-09-16 via daily what's new. Fonte: [GitHub Releases v2.1.270](https://github.com/anthropics/claude-code/releases/tag/v2.1.270) · [v2.1.271](https://github.com/anthropics/claude-code/releases/tag/v2.1.271) · [v2.1.272](https://github.com/anthropics/claude-code/releases/tag/v2.1.272) · [v2.1.273](https://github.com/anthropics/claude-code/releases/tag/v2.1.273). Nessuna voce editoriale: tutte le release 12-15 set sono fix/estensioni incrementali (fast mode esteso a Remote/self-hosted, `allowed_domains` per-comando, fork di sessioni Remote Control, header gateway) sotto la soglia per un bullet in TLDR. Nessun annuncio Anthropic blog dedicato nelle ultime 24 ore; digest ufficiale settimanale fermo alla Week 37 (7-11 set).</sub>
