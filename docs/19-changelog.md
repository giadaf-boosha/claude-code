...
| 18 ago 2026 | v2.1.235 | Setting opzionale `spellcheck` (correzione refusi via `aspell`/`hunspell`/`ispell` locale). Fix: invalidazione prompt cache su disconnessione language server, liste markdown annidate a profondita' 3+, permission prompt Shift+Tab. |
| 19 ago 2026 | **v2.1.236** | **`ANTHROPIC_DEFAULT_MODEL`** env var per il modello di default delle nuove sessioni. **`notify_when_idle`** in `SendMessage` cross-session (macOS/Linux): avviso one-shot quando un'altra sessione locale torna idle. Sandbox macOS: wildcard read-deny rafforzate (precedenza dentro le read region consentite). |
| 20 ago 2026 | v2.1.237 | **Output style built-in "Concise"**: Claude va dritto al risultato, salta preamboli e narrazione. Fix prompt caching per sessioni con LLM gateway o base URL custom. |
| 20 ago 2026 | v2.1.238 | `keybindingFlavor: "readline"` (keybinding stile Bash per modifica parola). `headersHelper` su marketplace URL per header HTTP dinamici. `self-hosted-runner --defer-shutdown-max-min`. Fix memory growth su sessioni interattive lunghe. |
| 21 ago 2026 | **v2.1.239** | Stima costi (`/cost`, statusline, `--max-budget-usd`) include il premio 1.1× US-only-inference per workspace data-residency. **`/claude-api upgrade`** migra progetti Python dall'SDK `anthropic` 0.x a 1.x. Fullscreen renderer esteso a Bedrock/Vertex/Foundry. Plugin cloud sync mostrati come `name@synced`. |
| 22 ago 2026 | v2.1.240 | Bug fix e miglioramenti di affidabilita'. |
| 23 ago 2026 | v2.1.241 | Bug fix e miglioramenti di affidabilita'. |

<sub>Aggiornato 2026-08-24 via daily what's new. Fonte: [GitHub Releases v2.1.235](https://github.com/anthropics/claude-code/releases/tag/v2.1.235) · [v2.1.236](https://github.com/anthropics/claude-code/releases/tag/v2.1.236) · [v2.1.237](https://github.com/anthropics/claude-code/releases/tag/v2.1.237) · [v2.1.238](https://github.com/anthropics/claude-code/releases/tag/v2.1.238) · [v2.1.239](https://github.com/anthropics/claude-code/releases/tag/v2.1.239) · [v2.1.240](https://github.com/anthropics/claude-code/releases/tag/v2.1.240) · [v2.1.241](https://github.com/anthropics/claude-code/releases/tag/v2.1.241). Routine ferma dal 18 ago: entry riepiloga il gap 19-23 ago.</sub>
