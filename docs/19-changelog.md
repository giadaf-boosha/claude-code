...
| 21 ago 2026 | **v2.1.239** | Le stime di costo (`/cost`, status line, `--max-budget-usd`) includono ora il premio 1,1× per inferenza solo-USA sui workspace a data residency. Nuovo comando `/claude-api upgrade` migra progetti Python dal pacchetto `anthropic` 0.x alla 1.x. Nelle sessioni cloud i plugin sincronizzati da claude.ai appaiono come `nome@synced` e non sovrascrivono mai un plugin locale omonimo. Build Alpine/musl: supporto nativo a paste immagini, clipboard e cattura audio. Fix: streaming Bedrock dietro proxy, hang all'avvio con SSO Bedrock, pause ~5s su Edit/Write nel terminale JetBrains, form MCP elicitation tagliati in fullscreen, titoli sessione persi oltre 64KB, testo spurio da movimento mouse in fullscreen. |

<sub>Aggiornato 2026-08-22 via daily what's new. Fonte: [GitHub Releases v2.1.239](https://github.com/anthropics/claude-code/releases/tag/v2.1.239).</sub>
