...
| 18 ago 2026 | v2.1.235 | Setting opzionale `spellcheck` che sottolinea errori di battitura nel prompt input. Migliorati memoria/CPU per sessioni cloud in background (es. `/ultrareview`, `/autofix-pr`). Fix: cache invalidata alla disconnessione/riconnessione di language server, allineamento liste markdown a profondita' 3+, highlight del prompt spostato su prompt multi-riga, Shift+Tab nei campi commento dei permission prompt, dialog di approvazione notebook cell che omettevano contenuto illeggibile, entity HTML mostrate nelle risposte a slash command. |

<sub>Aggiornato 2026-08-19 via daily what's new. Fonte: [GitHub Releases v2.1.235](https://github.com/anthropics/claude-code/releases/tag/v2.1.235) · [@ClaudeDevs](https://x.com/ClaudeDevs/status/2089798442306711646). Nessuna voce editoriale su release: solo fix e una feature minore (spellcheck), sotto soglia; segnalata la proroga dei limiti settimanali +50% al 31 agosto. Nota: fetch di anthropic.com e claude.com bloccato dal proxy di rete, ricerca completata via WebSearch/GitHub changelog raw/releases/whats-new digest sulle altre fonti.</sub>
