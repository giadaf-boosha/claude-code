...
| 8 set 2026 | v2.1.265 | `--plugin-dir` accetta ora una cartella con piu' plugin — ogni sottocartella con manifest viene caricata, incluse quelle aggiunte/rimosse a runtime — vedi [11 § Caricamento da archivio o URL](./11-plugins-marketplace.md#caricamento-da-archivio-o-url-da-v21128129). Tetto di 1 GB sui tool results salvati su disco, con troncamento segnalato in-conversazione. Telemetria `user.email`/`user.groups` per Claude Desktop e Cowork via Claude apps gateway. |
| 8 set 2026 | v2.1.266 | Fix: regressione sui setup LLM-gateway/proxy che usano `CLAUDE_CODE_USE_GATEWAY`. |
| 9 set 2026 | **v2.1.267** | **`maxEffortLevel`**: nuovo setting (top-level o per modello sotto `modelSettings`) che mette un tetto all'effort level su tutti i provider incluso Bedrock/Vertex/Foundry — vedi [5.3](./05-fast-mode-1m-context.md#53-opus-47--effort-xhigh) e [18.2](./18-settings-auth.md#model--effort). `--system-prompt-snapshot off` rigenera il system prompt ad ogni richiesta invece di riusare quello registrato in conversazione. Ampi fix su prompt cache (switch modello, resume, MCP/plugin mid-sessione), Remote Control, VS Code extension e artifact publish (retry su upload interrotto). |

<sub>Aggiornato 2026-09-10 via daily what's new. Fonte: [GitHub Releases v2.1.263](https://github.com/anthropics/claude-code/releases/tag/v2.1.263) · [v2.1.265](https://github.com/anthropics/claude-code/releases/tag/v2.1.265) · [v2.1.266](https://github.com/anthropics/claude-code/releases/tag/v2.1.266) · [v2.1.267](https://github.com/anthropics/claude-code/releases/tag/v2.1.267) · [code.claude.com/docs/en/changelog](https://code.claude.com/docs/en/changelog). Tre voci editoriali: `maxEffortLevel` + `--system-prompt-snapshot off` (v2.1.267, release), avviso deprecazione config managed legacy (10 set, annuncio), `--plugin-dir` su cartella (v2.1.265, feature). Il resto (fix di affidabilita', telemetria gateway) resta sotto soglia. Nessun annuncio Anthropic blog dedicato nelle ultime 24 ore.</sub>
