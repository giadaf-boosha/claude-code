...
Cronologia completa di Claude Code dalla research preview (24 febbraio 2025, v0.2.0) all'ultima versione (2 settembre 2026, v2.1.259). 7 fasi storiche + tabella versione per versione + post-mortem aprile 2026.
...
| 2 set 2026 | **v2.1.259** | **`managedMcpServers`**: setting gestito per distribuire server MCP HTTP/SSE a tutta l'organizzazione, con `toolPolicy` per bloccare l'approvazione per-tool. **`--permission-prompts none`**: nega automaticamente ogni prompt su host headless, mantenendo attiva la logica del permission mode corrente. Riconoscimento comandi `glab mr` (badge `MR !N` nel riepilogo tool, come per le PR GitHub). `--json` su `claude plugin validate` per report machine-readable. Fix: sessioni concorrenti che si sovrascrivevano a vicenda `~/.claude.json`, conversazioni con thinking rifiutato che venivano rifiutate di nuovo ad ogni turno successivo, regole deny Bash `Read()` che non coprivano file passati come valore di un'opzione, invalidazione della prompt cache al refresh OAuth in sessioni con telemetria disattivata, fullscreen con conversazione vuota dopo turni lunghi con centinaia di tool call, auto mode che eseguiva modelli non supportati dal frontmatter di comandi/skill. VS Code: quick filter "Active" e menu filtro stato nella sidebar delle sessioni. |

<sub>Aggiornato 2026-09-03 via daily what's new. Fonte: [GitHub Releases v2.1.259](https://github.com/anthropics/claude-code/releases/tag/v2.1.259). Voci editoriali: `managedMcpServers`, `--permission-prompts none`, riconoscimento `glab mr`, `--json` su `claude plugin validate` (feature di release). Il resto (fix di sessioni concorrenti, deny rules, auto mode, sidebar VS Code) resta sotto soglia ma e' incluso in tabella per completezza. Fuori da questa release: [Claude Commerce Agents](https://claude.com/blog/claude-for-commerce-agents) (blueprint open source con plugin Claude Code) ed [Enterprise Frontier Safeguards](https://www.anthropic.com/news/enterprise-frontier-safeguards) (annunci Anthropic blog, non versioni CLI) — vedi README "What's new today" del giorno.</sub>
