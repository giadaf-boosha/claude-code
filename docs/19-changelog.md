...
| 2 set 2026 | **v2.1.259** | **`managedMcpServers`**: managed setting che fornisce server MCP HTTP/SSE dell'organizzazione a tutti gli utenti senza configurazione locale — vedi [10.4](./10-mcp.md#104-scope-settings). `--permission-prompts none` per host headless non presidiati. Riconoscimento GitLab merge request (`MR !N`). `--json` per `claude plugin validate`. Fix: sessioni concorrenti che si sovrascrivevano `~/.claude.json` a vicenda, regole deny `Read()` su Bash non applicate a valori di opzione/operandi git, invalidazione prompt cache al refresh OAuth, fullscreen con conversazione vuota dopo turni con centinaia di tool call, auto mode che ignorava `model:` nel frontmatter di comandi/skill. |
| 3 set 2026 | **v2.1.260** | **Diff panel in fullscreen mode**: `/diff` apre un pannello laterale con le modifiche non committate mentre Claude edita — vedi [3.1](./03-slash-commands.md#31-tabella-completa). Diagnostica cache-miss in `/cost` e status line. `/reload-plugins` e `/advisor` testuale in sessioni headless. Timeout `/ultrareview` esteso a 45 min (era 30). Ampi fix di sicurezza/stabilita': regole permesso con parentesi, permessi Bash su comandi zsh, certificati Bedrock, managed settings non caricati per Enterprise/Team, e numerosi fix VS Code (session list, effort pill, tab rename). |

<sub>Aggiornato 2026-09-04 via daily what's new. Fonte: [GitHub Releases v2.1.259](https://github.com/anthropics/claude-code/releases/tag/v2.1.259) · [v2.1.260](https://github.com/anthropics/claude-code/releases/tag/v2.1.260) · [code.claude.com/docs/en/changelog](https://code.claude.com/docs/en/changelog). Due voci editoriali: `managedMcpServers` (feature di sistema MCP) e il diff panel fullscreen (nuova UX di `/diff`). Il resto (diagnostica cache-miss, `/reload-plugins`, riconoscimento GitLab MR, decine di fix di sicurezza/stabilita') resta sotto soglia. Nessun annuncio Anthropic blog dedicato su Claude Code nelle ultime 24 ore.</sub>
