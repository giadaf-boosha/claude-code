...
| 3 lug 2026 | **v2.1.200** | **Permission mode default → "Manual"** in CLI/VS Code/JetBrains (era `default`); **AskUserQuestion** senza auto-continue (opt-in via `/config`); fix background sessions stop-silenzioso/`daemon.lock` stale/subagent-rate-limit vuoto/roster corruption; fix crash MCP startup con `disabledMcpServers`/`enabledMcpServers` non-array; fix plugin scoped-project da git worktree; accessibilita' screen reader (glyph nascosti, label brevi, tabelle come `Header: value`); fix focus `/mcp`, fix voice dictation. |
| 3 lug 2026 | **v2.1.201** | **Sonnet 5**: rimosso system role mid-conversazione per reminder harness — riduce interferenze nei prompt lunghi. |

<sub>Aggiornato 2026-07-04 via daily what's new. Fonte: [GitHub Releases v2.1.201](https://github.com/anthropics/claude-code/releases/tag/v2.1.201).</sub>
