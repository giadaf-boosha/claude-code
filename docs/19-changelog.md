...
Cronologia completa di Claude Code dalla research preview (24 febbraio 2025, v0.2.0) all'ultima versione (17 luglio 2026, v2.1.212). 7 fasi storiche + tabella versione per versione + post-mortem aprile 2026.
...
| 14 lug 2026 | **v2.1.208** | **Screen reader mode** (`--ax-screen-reader` / `CLAUDE_AX_SCREEN_READER=1` / `axScreenReader`): rendering plain-text opt-in per accessibilita'. `vimInsertModeRemaps` mappa sequenze come `jj` a Escape. `CLAUDE_CODE_PROCESS_WRAPPER` per launcher aziendali che richiedono un wrapper eseguibile su agent view e background service. Ottimizzazioni memoria (MCP stdio stderr, documenti LSP, transcript sessione) e performance (cache rule matcher, tool-pool assembly). Fix: fast mode che restava disattivato dopo switch modello, background agent che perdevano risposte su delivery failure, crash HTTP/2 su GOAWAY, context indicator resettato a 200k dopo auto-update, tabelle Markdown oltre 200 righe che stallavano il rendering. |
| 14 lug 2026 | v2.1.209 | Fix: `/model` e altri dialog bloccati nelle sessioni background di `claude agents` (revert di una guardia troppo ampia introdotta in precedenza). |
| 14 lug 2026 | **v2.1.210** | Contatore tempo trascorso live nelle righe tool collassate. Fix: subagent con `isolation: 'worktree'` che bypassavano le restrizioni del repo principale, paste marker che finivano negli editor esterni, `claude attach` che non attendeva la stabilizzazione del daemon prima di applicare il resize del terminale, dialog di approvazione in plan mode con snapshot stale. Migliorato crash recovery dei background worker. |
| 15 lug 2026 | **v2.1.211** | **`--forward-subagent-text`** / `CLAUDE_CODE_FORWARD_SUBAGENT_TEXT`: include testo e thinking dei subagent nell'output stream-json. Sicurezza: le anteprime permessi neutralizzano ora caratteri bidirectional-override, zero-width e quote look-alike (i tool input non possono piu' alterare visivamente il messaggio di approvazione). Auto mode: una decisione `ask` di un hook PreToolUse su Bash non sandboxato non viene piu' sovrascritta. Fix: sessioni parallele disconnesse simultaneamente dopo sleep/wake, subagent con modello esplicito che tornavano al modello del parent al resume, `/loop` che nascondeva sessioni da `/resume`. |
| 17 lug 2026 | **v2.1.212** | **`/fork` ridisegnato**: copia la conversazione in una nuova sessione background (riga propria in `claude agents`) mentre continui a lavorare in primo piano; il vecchio comportamento in-session diventa il nuovo comando **`/subtask`**. **`/resume`** apre un picker di sessioni passate (incluse quelle eliminate) e le riprende sempre come sessione background. Nuovi limiti per sessione: 200 chiamate WebSearch (`CLAUDE_CODE_MAX_WEB_SEARCHES_PER_SESSION`) e 200 spawn di subagent (`CLAUDE_CODE_MAX_SUBAGENTS_PER_SESSION`), reset da `/clear`. Chiamate MCP oltre i 2 minuti passano automaticamente in background. `claude auto-mode reset` ripristina la configurazione di default con conferma. Fix: plan mode che eseguiva comandi Bash file-modifying senza permission prompt, `/ultrareview` che rifiutava riferimenti PR, sessioni hosted che fallivano con certificati mTLS o scope OAuth. |

<sub>Aggiornato 2026-07-17 via daily what's new. Fonte: [GitHub Releases v2.1.208](https://github.com/anthropics/claude-code/releases/tag/v2.1.208) · [v2.1.209](https://github.com/anthropics/claude-code/releases/tag/v2.1.209) · [v2.1.210](https://github.com/anthropics/claude-code/releases/tag/v2.1.210) · [v2.1.211](https://github.com/anthropics/claude-code/releases/tag/v2.1.211) · [v2.1.212](https://github.com/anthropics/claude-code/releases/tag/v2.1.212).</sub>
