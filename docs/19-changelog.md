...
| 25 set 2026 | **v2.1.283** | **`/doctor prompt-audit`** (alias `/checkup prompt-audit`): audita `CLAUDE.md`, skill, agent e comandi custom per pattern di prompting scritti per modelli piu' vecchi — vedi [3.1](./03-slash-commands.md#31-tabella-completa). Managed setting `deniedModels` (blocca modelli specifici anche se `availableModels` li permette) e `availableModelsMatch: "exact"` (blocca le nuove versioni finche' non elencate esplicitamente) — vedi [18.2](./18-settings-auth.md). Header gateway `x-claude-code-prompt-id` per raggruppare le richieste di uno stesso prompt (`CLAUDE_CODE_GATEWAY_HINT_HEADERS=1`). Output di MCP/WebFetch/WebSearch nello span OpenTelemetry `tool.output` con `OTEL_LOG_TOOL_CONTENT=1`. Ampio giro di fix su SDK sessions, MCP, plugin loading, vim mode, permessi, Remote Control, VS Code e Claude Tag. |

<sub>Aggiornato 2026-09-26 via daily what's new. Fonte: [GitHub Releases v2.1.283](https://github.com/anthropics/claude-code/releases/tag/v2.1.283) · [Anthropic blog — Build plugins for Claude](https://claude.com/blog/build-plugins-for-claude). Due voci editoriali: `/doctor prompt-audit` (nuovo comando diagnostico) e i managed setting `deniedModels`/`availableModelsMatch: "exact"` per la governance dei modelli. Fuori dal changelog CLI, Anthropic apre un portale di sottomissione plugin/connector alla Claude directory (validazione automatica, review tracking, analytics), con unificazione della discovery Claude/Claude Code annunciata nelle prossime settimane. Il resto della release v2.1.283 (header gateway, logging OTel dei tool output, ampio giro di fix su SDK/MCP/plugin/vim mode/Claude Tag) resta sotto la soglia editoriale.</sub>
