...
| 20 lug 2026 | **v2.1.216** | Nuova opzione `sandbox.filesystem.disabled` (salta l'isolamento filesystem mantenendo il controllo egress di rete). Fix rallentamento quadratico nelle sessioni lunghe (normalizzazione messaggi), auto mode che negava comandi dopo scadenza/rotazione OAuth, sessioni background che perdevano prompt/tool restriction al resume, subagent worktree-isolati che reindirizzavano git nel checkout condiviso, MCP re-authenticate che revocava credenziali funzionanti prima del nuovo login, telemetria permessi che contava erroneamente rifiuti falliti come rejection utente. |
