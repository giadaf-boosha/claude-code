...
| 14 ago 2026 | v2.1.233 | Supporto URL merge request GitLab su `--worktree` e in `claude agents` (mostrate come `!N`). Memory cgroup opzionale per i comandi del tool Bash su Linux (`CLAUDE_CODE_TOOL_MEMORY_LIMIT`), env var `CLAUDE_CODE_WEBFETCH_CACHE_TTL_MS` per la cache di sessione di WebFetch. I tool di todo-tracking (`TaskCreate`/`TaskGet`/`TaskUpdate`/`TaskList`, `TodoWrite`) sono rimossi di default su Opus 4.8, Sonnet 5, Fable 5 e modelli successivi — riattivabili con `CLAUDE_CODE_ENABLE_TODO_TOOLS=1`. Fix sicurezza: path Windows con prefisso NT device (`\??\`) che bypassavano la validazione UNC. |

<sub>Aggiornato 2026-08-17 via daily what's new. Nessuna voce editoriale nelle ultime 24 ore (sotto soglia). Housekeeping: aggiunta riga v2.1.233 (14 ago) mancante dalla tabella. Fonte: [GitHub Releases v2.1.233](https://github.com/anthropics/claude-code/releases/tag/v2.1.233).</sub>
