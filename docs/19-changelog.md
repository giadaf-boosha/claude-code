...
Cronologia completa di Claude Code dalla research preview (24 febbraio 2025, v0.2.0) all'ultima versione (21 luglio 2026, v2.1.217). 7 fasi storiche + tabella versione per versione + post-mortem aprile 2026.
...
| 20 lug 2026 | v2.1.216 | **`sandbox.filesystem.disabled`**: nuova opzione per saltare l'isolamento filesystem mantenendo pero' il controllo dell'egress di rete. Fix prestazioni: rallentamento quadratico nelle sessioni lunghe dovuto alla normalizzazione messaggi. Vari fix worktree, MCP re-auth, telemetry permission denial. |
| 21 lug 2026 | **v2.1.217** | **iOS Simulator pane** in Claude Code Desktop (beta) — vedi [17.3 Desktop app](./17-ide-surface.md#173-desktop-app-endesktop). **Subagent: nesting disattivato di default** (era fino a 5 livelli da v2.1.172) + **tetto di concorrenza** default 20 (`CLAUDE_CODE_MAX_CONCURRENT_SUBAGENTS`) — vedi [08 Subagents § 8.4](./08-subagents.md). Emoji shortcode autocomplete nel prompt input. Warning su transcript write falliti (es. disco pieno). |

<sub>Aggiornato 2026-07-22 via daily what's new. Fonte: [GitHub Releases v2.1.216](https://github.com/anthropics/claude-code/releases/tag/v2.1.216) · [v2.1.217](https://github.com/anthropics/claude-code/releases/tag/v2.1.217) · [docs ufficiali iOS Simulator](https://code.claude.com/docs/en/desktop-ios-simulator).</sub>
