...
| 18 set 2026 | v2.1.276 | Fix regressione: ogni richiesta falliva con `400 … Input tag 'advisor_20260301'` quando `ANTHROPIC_BASE_URL` punta a un proxy o gateway (introdotta da v2.1.275). |
| 18 set 2026 | **v2.1.277** | **AGENTS.md come fallback di CLAUDE.md**: se una cartella non ha un CLAUDE.md, Claude Code legge e usa direttamente AGENTS.md (standard cross-tool), configurabile in `/config` → "Project instructions" — vedi [6.3](./06-claude-md-memory.md#agents-md-interop) (non ancora su Bedrock/Vertex/Foundry). Tasto "invia subito" (`ctrl+enter` / `ctrl+x ctrl+s`) per interrompere il turno corrente e inviare tutti i messaggi in coda. Mouse support nel pannello `/config` in fullscreen. 95+ fix minori (VS Code agent map e sign-out, Claude Tag/Slack, Code Review, Write/Edit tool, sessioni su macchine lente). |
| 19 set 2026 | **v2.1.278** | **Auto mode: classifier lato server senza sovrapprezzo**: su API diretta, Enterprise, Bedrock, Vertex, Foundry e Claude apps gateway, i controlli di sicurezza di auto mode girano di default lato server invece che come richieste extra al modello, e non vengono piu' fatturati come overhead classifier — vedi [4.3](./04-modalita-permessi.md#43-auto-mode-da-v2183-default-promaxteam-da-14-ago-2026) (opt-out `CLAUDE_CODE_AUTO_MODE_SERVER=0`). Nuova riga "Auto mode server" in `/status`. |

<sub>Aggiornato 2026-09-20 via daily what's new. Fonte: [changelog ufficiale](https://code.claude.com/docs/en/changelog) · [GitHub Releases v2.1.276](https://github.com/anthropics/claude-code/releases/tag/v2.1.276) · [v2.1.277](https://github.com/anthropics/claude-code/releases/tag/v2.1.277) · [v2.1.278](https://github.com/anthropics/claude-code/releases/tag/v2.1.278) · [@trq212](https://x.com/trq212/status/2101009392611278961) · [auto-mode-classifier-billing](https://code.claude.com/docs/en/auto-mode-classifier-billing). Due voci editoriali: **AGENTS.md come fallback di CLAUDE.md** (v2.1.277, feature di sistema cross-tool, annunciata da Thariq/@trq212) e **auto mode: classifier lato server senza sovrapprezzo** (v2.1.278, cambio di billing su API/Enterprise/cloud provider). Il resto (fix regressione proxy v2.1.276, tasto "invia subito", mouse support `/config` fullscreen, 95+ fix minori VS Code/Claude Tag/Code Review) resta sotto la soglia editoriale. Nessun annuncio Anthropic blog dedicato su Claude Code nelle ultime 24 ore.</sub>
