...
| 26 ago 2026 | v2.1.247 | **Tool `SendFeedback`**: quando qualcosa va storto in sessione, Claude puo' redigere da solo un feedback report da rivedere e inviare via `/feedback` (disattivabile col setting `feedbackDrafts`). **`/claude-api cost-optimize`**: profila la spesa API di un progetto esistente e propone leve di costo (caching, token hygiene, batch, effort, scelta modello) una alla volta. Skill `/claude-api` estesa con copertura Admin API (membri org, workspace, API key, rate limit report, workload identity federation, CMEK). Sonnet 5 in finestra 1M ora fa auto-compact a ~967K token invece di ~934K (usa tutta la context window). Messaggi cross-session ora collassati di default a una riga (`Ctrl+O` per espandere). Tip sui prompt di permesso Bash con scorciatoia a un tasto per passare ad auto mode. Numerosi fix: sub-agent che ripartono sul fallback model chain dopo un 404 sul primo tentativo, hook/agent che stampavano megabyte di errori senza piu' bloccare la sessione, tastiere non-Latin nei terminali kitty-protocol, sandbox Bash che non cancellava piu' i symlink dotfile-managed. |

<sub>Aggiornato 2026-08-27 via daily what's new. Fonte: [GitHub Releases v2.1.247](https://github.com/anthropics/claude-code/releases/tag/v2.1.247).</sub>
