...
| 29 ago 2026 | v2.1.252 | Fix: Bash che falliva con "task output swap refused" su alcuni Mac, setting "always allow" che non persisteva nelle configurazioni di progetto nuove, Remote Control che si bloccava su connessioni claude.ai degradate, notifiche di task in background che superavano i limiti di dimensione delle richieste API. |
| 1 set 2026 | **v2.1.257** | **Claude Fable 5.1** (`claude-fable-5-1`) diventa il modello Fable di default — 1M context, stesso pricing $10/$50 per MTok, cache read -75% ($1 → $0.25/MTok) — vedi [5.7](./05-fast-mode-1m-context.md#57-claude-fable-5-da-v21170). Nuova regola **Containment Escape** nell'auto mode contro fetch di credenziali da cloud metadata e cross-tenant reach — vedi [4.3](./04-modalita-permessi.md#43-auto-mode-da-v2183-default-promaxteam-da-14-ago-2026). Env var `CLAUDE_CODE_SUBAGENT_MODEL_FORCE` per forzare il modello su tutti i subagent — vedi [8.2](./08-subagents.md#82-frontmatter-completo). Settings `timeFormat`/`timeZone` per il formato orario (12h/24h/UTC/strftime custom), opzione `s` su `/effort` per applicare il livello alla sola sessione corrente, prompt one-time prima della prima lettura file fuori dalle working directory. Oltre 80 fix su permessi, sandbox, MCP, plugin e cross-session messaging. |
| 1 set 2026 | v2.1.258 | Fix: regressione che impediva l'avvio di Claude Code su macOS 12 Monterey (introdotta in v2.1.255); sessioni remote/schedulate che fallivano con errore "user messages must have non-empty content" quando una permission re-approval ripetuta non poteva essere applicata. |

<sub>Aggiornato 2026-09-02 via daily what's new. Fonte: [GitHub Releases v2.1.257](https://github.com/anthropics/claude-code/releases/tag/v2.1.257) · [v2.1.258](https://github.com/anthropics/claude-code/releases/tag/v2.1.258) · [CHANGELOG.md](https://github.com/anthropics/claude-code/blob/main/CHANGELOG.md) · [@claudeai](https://x.com/claudeai/status/2094848572143407483) · [@bcherny](https://x.com/bcherny/status/2094864060609376748) · [@ClaudeDevs](https://x.com/ClaudeDevs/status/2094851229734277228). Voce editoriale principale del giorno: lancio **Claude Fable 5.1** (annuncio Anthropic + CLI v2.1.257), che sostituisce Fable 5 come modello Fable di default. Resto della release (Containment Escape, `CLAUDE_CODE_SUBAGENT_MODEL_FORCE`, time format settings) sopra soglia per rilevanza sicurezza/orchestrazione. v2.1.258 solo fix.</sub>
