...
| 14 ago 2026 | v2.1.233 | Supporto GitLab merge request in `--worktree` e nella vista `claude agents` (mostrate come `!N`). Cgroup di memoria opt-in per comandi del Bash tool su Linux (`CLAUDE_CODE_TOOL_MEMORY_LIMIT`), variabile `CLAUDE_CODE_WEBFETCH_CACHE_TTL_MS` per la cache URL di WebFetch. Tool di task-tracking (`TaskCreate`/`TaskGet`/`TaskUpdate`/`TaskList`, `TodoWrite`) disabilitati di default su Opus 4.8, Sonnet 5, Fable 5 e modelli piu' recenti (riattivabili con `CLAUDE_CODE_ENABLE_TODO_TOOLS=1`). Fix sicurezza: percorsi Windows con NT device prefix che bypassavano la validazione UNC (vettore di leak credenziali NTLM). Fix: sessioni cloud segnate erroneamente come perse allo shutdown dell'ambiente, connessioni MCP v2 che riaprivano streaming all'infinito, notification hook per permission prompt che non scattavano, sessioni idle su Linux che saturavano una CPU al 100% con sandboxing attivo, alias skill bundled (`/checkup`, `/review`) che segnalavano "Unknown command" in modalita' `-p`. |

<sub>Aggiornato 2026-08-16 via daily what's new. Fonte: [GitHub Releases v2.1.233](https://github.com/anthropics/claude-code/releases/tag/v2.1.233). Nessuna voce editoriale in README: solo GitLab MR support, cgroup memoria e fix, sotto soglia. Copertura estesa dal 14 al 16 ago (routine non eseguita il 15 ago). Fetch di anthropic.com/news e claude.com/blog bloccati dal proxy di rete; ricerca completata via WebSearch/GitHub releases/changelog/whats-new digest sulle altre fonti.</sub>
