...
| 2 set 2026 | **v2.1.259** | **Setting `managedMcpServers`**: le organizzazioni distribuiscono server MCP HTTP/SSE a tutti gli utenti via settings gestiti, senza deploy file `managed-mcp.json` — vedi [10.4](./10-mcp.md#104-scope-settings). `--permission-prompts none` per host headless unattended. Riconoscimento GitLab merge request (`MR !N`) nei riepiloghi tool. `--json` su `claude plugin validate`. Fix importante: sessioni concorrenti che si sovrascrivevano a vicenda le modifiche a `~/.claude.json` (workspace trust e stato MCP/progetto non piu' persi). Altri fix: rifiuti di thinking che causavano rifiuti ripetuti nei turni successivi, regole deny `Bash Read()` su valori/operandi di opzione file, invalidazione prompt cache al refresh OAuth con telemetria disattivata. |

<sub>Aggiornato 2026-09-03 via daily what's new. Fonte: [GitHub Releases v2.1.259](https://github.com/anthropics/claude-code/releases/tag/v2.1.259) · [docs managed-mcp](https://code.claude.com/docs/en/managed-mcp). Unica voce editoriale: `managedMcpServers` (nuova feature di sistema/MCP). Il resto di v2.1.259 (flag CLI, riconoscimento GitLab, fix sessioni concorrenti) resta sotto la soglia editoriale. Nessun annuncio Anthropic blog dedicato su Claude Code nelle ultime 24 ore; il digest settimanale ufficiale resta fermo alla Week 34 (17-21 ago, gia' coperta).</sub>
